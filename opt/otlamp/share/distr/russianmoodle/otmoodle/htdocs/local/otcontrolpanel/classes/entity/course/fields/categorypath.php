...
class categorypath extends \local_otcontrolpanel\field\abstract_field {
    ...
    protected function get_plain_value($record, $suffix='')
    {
        ...
        if (array_key_exists($record->categorypathid, $categorieslist))
        {
            $path = $categorieslist[$record->categorypathid];
            return implode('/', explode('/', $path, -1));
        } else {
            return '?';
        }
    }
    ...
    public function register_joins() {
        try {
            ...
            $join->add_required_db_fields(['id AS "categorypathid"']);
