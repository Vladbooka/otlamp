...
abstract class abstract_comparison_operator {

    protected $args = [];

    abstract public static function define_args() : array;
    abstract public function execute();

    public function __construct() {
        $this->args = static::define_args();
        $countargs = count($this->args);

        if ($countargs != func_num_args()) {
            throw new \Exception('Wrong args num');
        }

        $values = func_get_args();
        for ($a=0; $a < $countargs; $a++) {
            $this->args[$a]->setValue($values[$a]);
        }
    }
    ...
    public static function get_displayname() {
        return get_string('comparison_operator_'.static::get_code(), 'local_opentechnology');
    }

    public static function get_description() {
        return get_string('comparison_operator_'.static::get_code().'_desc', 'local_opentechnology');
    }

    public static function get_user_description($a=null) {
        return get_string('comparison_operator_'.static::get_code().'_userdesc', 'local_opentechnology', $a);
    }
    ...
    public static function get_class_info() {
        $classinfo = [
            ...
            'argsnum' => count(static::define_args())
        ];
        return $classinfo;
    }
    ...
    public static function get_classes_info() {
        $classesinfo = [];
        $namespace = 'availability_condition\\comparison_operator';
