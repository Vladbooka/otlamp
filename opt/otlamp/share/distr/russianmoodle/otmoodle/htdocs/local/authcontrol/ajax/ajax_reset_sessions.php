 ...
 * 
 * Закрытие доступа пользователю в СДО по AJAX
 * 
...
else
{
    ...
    {
        if (  local_authcontrol_kill_user_sessions($userid) )
        {
            $data['complete'] = get_string('ajax_sessions_kill_success', 'local_authcontrol');
            $data['html'] = get_string('form_main_access_reset_sessions', 'local_authcontrol');
        }
        else 
        {
            $data['complete'] = get_string('ajax_sessions_kill_fail', 'local_authcontrol');
        }
    }
