...
/**
 * Юнит-тест сценария user_registered_long_ago (Уведомление ни разу не авторизовавшегося пользователя о регистрации, с момента которой прошло почти два месяца)
 *
 * @package    local
...
class local_pprocessing_s_user_registered_long_ago_testcase extends advanced_testcase
{

    /**
     * Уведомление ни разу не авторизовавшегося пользователя о регистрации, с момента которой прошло почти два месяца
    ...
    public function test_scenario()
    {
        ...
        // включаем сценарий
        set_config('user_registered_long_ago__status', true, 'local_pprocessing');
        set_config('user_registered_long_ago__message_subject', 'Тема письма', 'local_pprocessing');
        set_config('user_registered_long_ago__message_full', 'Полный текст письма', 'local_pprocessing');
        set_config('user_registered_long_ago__message_short', 'Короткий текст письма', 'local_pprocessing');
        ...
        $userrecord = [
            'confirmed' => 1,
            'firstaccess' => 0,
            'timecreated' =>  strtotime('-2 months, +6 days'),
            'deleted' => 0
        ];
        ...
        set_config('user_registered_long_ago__message_subject', '', 'local_pprocessing');
        ...
        set_config('user_registered_long_ago__message_full', '', 'local_pprocessing');
        ...
        $freshtimecreated['timecreated'] = strtotime('-2 months, +8 days');
        call_user_func($test, $freshtimecreated, 0, 'Проверка сценария, когда условие timecreated <= strtotime(\'-2 months, +7 days\') не исполняется');
