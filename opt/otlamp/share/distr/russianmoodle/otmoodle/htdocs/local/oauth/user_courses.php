<?php

require_once '../../config.php';
require_once __DIR__.'/lib.php';

$PAGE->set_context(null);

\core\session\manager::write_close();

$server = oauth_get_server();
if (!$server->verifyResourceRequest(OAuth2\Request::createFromGlobals())) {
    $logparams = array('other' => array('cause' => 'invalid_approval'));
    $event = \local_oauth\event\user_courses_request_failed::create($logparams);
    $event->trigger();

    $server->getResponse()->send();
    die();
}


$token = $server->getAccessTokenData(OAuth2\Request::createFromGlobals());
if (!empty($token['user_id'])) {

    $user = $DB->get_record('user', [
        'id' => $token['user_id'],
        'deleted' => '0'
    ], 'id,auth,username,idnumber,firstname,lastname,email,lang,country,phone1,address,description');
    if (!$user) {
        $logparams = array('other' => array('cause' => 'user_not_found'));
        $event = \local_oauth\event\user_courses_request_failed::create($logparams);
        $event->trigger();

        $response->send();
    }
    
    $courses = enrol_get_users_courses($user->id, true, [
        'id',
        'category',
        'shortname',
        'fullname',
        'idnumber'
    ]);

    $request = OAuth2\Request::createFromGlobals();
    $response = new OAuth2\Response();
    $scopeRequired = 'user_courses';
    if (!$server->verifyResourceRequest($request, $response, $scopeRequired)) {
        $logparams = array('relateduserid' => $user->id, 'other' => array('cause' => 'insufficient_scope'));
        $event = \local_oauth\event\user_courses_request_failed::create($logparams);
        $event->trigger();

          // if the scope required is different from what the token allows, this will send a "401 insufficient_scope" error
          $response->send();
    }

    $logparams = array('userid' => $user->id);
    $event = \local_oauth\event\user_courses_request::create($logparams);
    $event->trigger();
    echo json_encode($courses);
} else {
    $logparams = array('other' => array('cause' => 'invalid_token'));
    $event = \local_oauth\event\user_courses_request_failed::create($logparams);
    $event->trigger();

    $server->getResponse()->send();
}
