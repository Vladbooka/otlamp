...
function profile_has_required_custom_fields_set($userid) {
    ...
    $conditions = "required = 1 AND visible > 0 AND locked = 0";
    $fieldrecords = $DB->get_records_select('user_info_field', $conditions);
    
    foreach ($fieldrecords as $fieldrecord) {
        require_once ($CFG->dirroot . '/user/profile/field/' . $fieldrecord->datatype . '/field.class.php');
        $fieldclassname = 'profile_field_' . $fieldrecord->datatype;
        $field = new $fieldclassname($fieldrecord->id, $userid);
        if ($field->is_empty()) {
            return false;
        }
    }
