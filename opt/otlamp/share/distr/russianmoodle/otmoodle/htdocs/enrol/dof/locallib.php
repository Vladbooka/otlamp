...
class enrol_dof_potential_participant extends user_selector_base {
    ...
    protected function get_options() {
        ...
        $options['file']    = 'enrol/dof/locallib.php';
        return $options;
    }
}
...
class enrol_dof_editselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/dof:manage", $manager->get_context())) {
            return false;
        }
...
class enrol_dof_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        global $CFG;
        require_once($CFG->dirroot.'/enrol/dof/bulkchangeforms.php');
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/dof:unenrol", $manager->get_context())) {
            return false;
        }
        foreach ($users as $user) {
            foreach ($user->enrolments as $enrolment) {
