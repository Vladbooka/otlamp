...
class enrol_self_plugin extends enrol_plugin {
    ...
    public function enrol_self(stdClass $instance, $data = null) {
             ...
             switch ( $instance->customchar3 )
        {
            ...
            case 'localcrw':
                ...
                {
                    $destination = "$CFG->wwwroot/local/crw/course.php?id=$instance->courseid";
                } else
                {
                    $destination = "$CFG->wwwroot/course/view.php?id=$instance->courseid";
                }
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $message = format_text($message, FORMAT_MOODLE, array('context'=>$context, 'para'=>false, 'newlines'=>true, 'filter'=>true));
        } else {
            $message = get_string('welcometocoursetext', 'enrol_self', $a);
            $message = text_to_html($message, null, false, true);
        }
    ...
    public function get_instance_defaults() {
        ...
        $fields['customchar3']     = $this->get_config('return_url');
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_self'), $roles);
            ...
            // На какую страницу перенаправлять пользователя после успешной
            // записи на курс
        $returnurls = [
            'course' => get_string('return_url_course', 'enrol_self')
        ];
        $installlist = core_plugin_manager::instance()->get_installed_plugins('local');
        if ( array_key_exists('crw', $installlist) )
        {
            $returnurls['localcrw'] = get_string('return_url_localcrw', 'enrol_self');
        }
        $returnurls['wantsurl'] = get_string('return_url_wantsurl', 'enrol_self');
        $mform->addElement('select', 'customchar3', get_string('form_return_url', 'enrol_self'), $returnurls);
