...
require_once($CFG->dirroot . '/mod/quiz/report/otoverview/overview_options.php');
require_once($CFG->dirroot . '/mod/quiz/report/otoverview/overview_form.php');
require_once($CFG->dirroot . '/mod/quiz/report/otoverview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_otoverview_report extends quiz_attempts_report {
    
    /**
     * Таблицы вывода данных
     * @var quiz_otoverview_table
     */
    public $table = null;

    public function display($quiz, $cm, $course, $ajax = false, $questionattemptsids = [], $getcharts = false) {
        global $DB, $OUTPUT, $PAGE, $SESSION;

        if ( !$ajax )
        {
            // подключение JS кода
            $PAGE->requires->js_call_amd('quiz_otoverview/table_controller', 'init');
        }
        
        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'otoverview', 'quiz_otoverview_settings_form', $quiz, $cm, $course);

        $options = new quiz_otoverview_options('otoverview', $quiz, $cm, $course);
        ...
        $table = new quiz_otoverview_table($quiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $this->table = $table;
        if ( $ajax )
        {
            $table->set_flag_onlygetdata(true);
            $table->set_questionattemptsids($questionattemptsids);
        }
        $filename = quiz_report_download_filename(get_string('overviewfilename', 'quiz_otoverview'),
                $courseshortname, $quiz->name);
        $table->is_downloading($options->download, $filename,
                $courseshortname . ' ' . format_string(htmlspecialchars($quiz->name), true));
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
                ...
                if (has_capability('mod/quiz:regrade', $this->context)) {
                    ...
                    if ($currentgroup) {
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'quiz_otoverview', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'quiz_otoverview', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'quiz_otoverview', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'quiz_otoverview', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'quiz_otoverview');
                        $regradealllabel =
                                get_string('regradeall', 'quiz_otoverview');
                    }
            ...
            if (!$table->is_downloading() && has_capability('mod/quiz:regrade', $this->context) &&
                ...
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'quiz_otoverview');
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                ...
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    ...
                    $graphname = get_string('overviewreportgraphgroup', 'quiz_otoverview', groups_get_group_name($currentgroup));
                    $chartshtml .= $output->chart($chart, $graphname);
                }
            }
            ...
            if ($DB->record_exists('quiz_grades', array('quiz'=> $quiz->id))) {
                ...
                $hash .= '-' . md5(serialize($data));
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'quiz_otoverview');
                $chartshtml .= $output->chart($chart, $graphname);
            }
            if ( $ajax )
            {
                if ( !$getcharts || 
                        (! empty($SESSION->mod_quiz_report_quiz_otoverview_charts_hash) &&
                        ($SESSION->mod_quiz_report_quiz_otoverview_charts_hash == $hash)) )
                {
                    return '';
                }
                $SESSION->mod_quiz_report_quiz_otoverview_charts_hash = $hash;
            ...
            {
                $SESSION->mod_quiz_report_quiz_otoverview_charts_hash = $hash;
                echo html_writer::div($chartshtml, 'otoverview-charts');
            }
        }
        return true;
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'quiz_otoverview'));
    ...
    protected function regrade_attempts($quiz, $dryrun = false,
        ...
        $progressbar = new progress_bar('quiz_otoverview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'quiz_otoverview', $a));
        }
