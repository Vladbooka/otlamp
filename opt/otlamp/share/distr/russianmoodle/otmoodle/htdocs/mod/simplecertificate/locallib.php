...
class simplecertificate {
    ...
    public function delete_expired_shelf_life_issues() {
        global $DB;

        $certificate = $this->get_instance();

        $sql = 'timecreated <= :timecreated AND certificateid=:certificateid AND timedeleted IS NULL';
        $params = [
            'timecreated' => (time() - $certificate->shelflife),
            'certificateid' => $certificate->id
        ];
        $issues = $DB->get_records_select('simplecertificate_issues', $sql, $params);
        foreach($issues as $issue)
        {
            $this->remove_issue($issue, $certificate);
        }
    }
    ...
    public function get_issue($user = null) {
        ...
        } else
        {
            ...
            if ( empty($issuecert) || ! $this->issue_file_exists($issuecert) )
            {
                ...
                $needalternatecode=$this->get_instance()->alternatecode==1?true:false;

                // Avoiding not null restriction;
                $issuecert->pathnamehash = '';

                if (!has_capability('mod/simplecertificate:manage', $this->context, $userid) && is_enrolled($this->get_course_context(), $userid))
                {
                    $issuecert->id = $DB->insert_record('simplecertificate_issues', $issuecert);

                    if($needalternatecode) {
                        $issuecert->code = $this->get_alternate_code($issuecert->id);
                ...
                } else
                {
                    $issuecert->id = 0;
                    if($needalternatecode) {
                        $issuecert->code = $this->get_alternate_code();
                    }
                }
            }
        }
        ...
        if (!empty($issuecert->haschange) && !$created) { //Check haschange, if so, reissue
            ...
            $formated_certificatename = str_replace('-', '_', $this->get_instance()->name);
            $issuecert->certificatename = format_string($formated_coursename . '-' . $formated_certificatename, true);
            // несмотря на то, что здесь происходит только апдейт
            // сам файл будет позднее сформирован с новыми настройками и новыми исходными данными
            // следовательно это сравнительно с выпуском нового сертификата, у которого вполне может быть новая дата создания
            // в противном случае (если не обновлять дату создания) мы получаем неверные данные,
            // будто сертификат выпущен давно
            // при этом в сертификат попала оценка полученная позднее
            $issuecert->timereissue = time();
            $DB->update_record('simplecertificate_issues', $issuecert);
        }
    ...
    /**
     * Генерирует альтеративный код для сертификата
     *
     * @param number $issuecertid (or null for random value)
     * @return string alternate code
     */
    private function get_alternate_code($issuecertid=null)
    {
        if($issuecertid==null) {
            $issuecertid=rand(0,65535);
        }

        $alternatecode="A001-".
            str_pad( dechex( $issuecertid ), 4, "0", STR_PAD_LEFT).
            "-".
            substr( md5(time()), 0, 4 );
    ...
    public function print_attempts($attempts) {
        ...
        $html = $OUTPUT->heading(get_string('summaryofattempts', 'simplecertificate'));
     ...
     */
    protected function get_tablegrades($userid = null)
    {
        global $USER, $DB;
        ...
        $grades = grade_item::fetch_all(['courseid' => $this->get_course()->id]);
        $grades = $grades ? $grades : [];

        foreach ( $grades as $id => $item )
        {
            $name = $item->get_name();
            ...
            if ( ! empty($grade) )
            {
                ...
                $fullgrade->points = grade_format_gradevalue(
                    $grade->finalgrade,
                    $item,
                    true,
                    GRADE_DISPLAY_TYPE_REAL,
                    $this->get_instance()->gradedecimals
                );
                $fullgrade->percentage = grade_format_gradevalue(
                    $grade->finalgrade,
                    $item,
                    true,
                ...
                $fullgrade->letter = grade_format_gradevalue(
                    $grade->finalgrade,
                    $item,
                    true,
                ...
                $fullgrade->maxpoints = grade_format_gradevalue(
                    $item->grademax,
                    $item,
                    true,
                    GRADE_DISPLAY_TYPE_REAL,
                ...
                $strgrade = $this->get_formated_grade($fullgrade);

                $return .= '<tr><td>' . $name . ' : </td><td>' . $strgrade . '</td></tr>';
            }
        }
        $return .= '</table></div>';
        return $return;
    ...
    /**
     * Returns a table with grades to display for the certificate.
     *
     * @param int $userid
     * @return string the grade result
     */
        ...
        switch ($this->get_instance()->certgrade) {
            case self::COURSE_GRADE: //Course grade
                if  ($course_item = grade_item::fetch_course_item($this->get_course()->id)) {
                    ...
                    $coursegrade->percentage = grade_format_gradevalue($grade->finalgrade, $course_item, true,
                            GRADE_DISPLAY_TYPE_PERCENTAGE, $this->get_instance()->gradedecimals);
                    $coursegrade->letter = grade_format_gradevalue($grade->finalgrade, $course_item, true,
                            GRADE_DISPLAY_TYPE_LETTER, 0);
                    $coursegrade->maxpoints = grade_format_gradevalue($course_item->grademax, $course_item, true, GRADE_DISPLAY_TYPE_REAL, $this->get_instance()->gradedecimals);
                }
                break;
            ...
            default: // Module grade
                ...
                if ($modinfo = $this->get_mod_grade($this->get_instance()->certgrade, $userid)) {
                    ...
                    $coursegrade->maxpoints = $modinfo->maxpoints;
                    break;
                }
        }
    ...
    private function get_formated_grade(stdClass $coursegrade, $format=null) {
        $coursegradearr = (array)$coursegrade;
        if (empty($coursegradearr)) {
            return '';
        }
        if( is_null($format) )
        {
            $format = $this->get_instance()->gradefmt;
        }
        ...
        switch ( $format ) {
            ...
            case 3:
                ...
                break;
            case 4:
                return $coursegrade->points.' '.get_string('gradefmt_of', 'simplecertificate').' '.$coursegrade->maxpoints;
                break;
            case 5:
                return $coursegrade->points.' '.get_string('gradefmt_of', 'simplecertificate').' '.$coursegrade->maxpoints.' ('.$coursegrade->percentage.')';
                break;
            case 6:
                return $coursegrade->points.' '.get_string('gradefmt_of', 'simplecertificate').' '.$coursegrade->maxpoints.' ('.$coursegrade->letter.')';
                break;
            case 7:
                return $coursegrade->points.' '.get_string('gradefmt_of', 'simplecertificate').' '.$coursegrade->maxpoints.' ('.$coursegrade->percentage.') '.$coursegrade->letter;
                break;
    ...
    protected function get_mod_grade($moduleid, $userid) {
        ...
        if ($grade_item = grade_get_grades($this->get_course()->id, 'mod', $module->name, $cm->instance, $userid)) {
            ...
            if( ! empty($itemproperties) && is_object($itemproperties) )
            {
                ...
                $modinfo->points = grade_format_gradevalue($grade, $item, true, GRADE_DISPLAY_TYPE_REAL, $this->get_instance()->gradedecimals);
                $modinfo->percentage = grade_format_gradevalue($grade, $item, true, GRADE_DISPLAY_TYPE_PERCENTAGE, $this->get_instance()->gradedecimals);
                $modinfo->letter = grade_format_gradevalue($grade, $item, true, GRADE_DISPLAY_TYPE_LETTER, 0);

                $modinfo->maxpoints = grade_format_gradevalue($item->grademax, $item, true, GRADE_DISPLAY_TYPE_REAL, $this->get_instance()->gradedecimals);
    ...
    protected function create_pdf(stdClass $issuecert, $pdf = null, $isbulk = false) {
        ...
        //положение поля для текста по горизонтали
        $x = $this->get_instance()->certificatetextx;
        //положение поля для текста по вертикали
        $y = $this->get_instance()->certificatetexty;
        //ширина поля для текста
        $w = $this->get_instance()->certificatetextwidth;
        //высота поля для текста
        $h = $this->get_instance()->certificatetextheight;
        //сам текст в формате html
        $html = $this->get_certificate_text($issuecert, $this->get_instance()->certificatetext);
        //стиль границы для ячейки. Почему-то перетсал работать
        $border=0;
        //Indicates where the current position should go after the call
        $ln = 0;
        //должен ли быть заполнен фон ячейки
        $fill = false;
        //сбрасывать ли высоту последней ячейки
        $reseth = true;
        //выравнивание по умолчанию
        $align = 'C';
        $pdf->writeHTMLCell($w, $h, $x, $y, $html, $border, $ln, $fill, $reseth, $align);

        //border для writeHTMLCell перестал отображаться,
        //нарисуем его в виде простого прямоугольника отдельно если требуется
        if((int)$this->get_instance()->certificatetextborder==1)
        {
            $border = array(
                'LTRB' => array(
                    'width' => 0.3,
     ...
     */
    public function output_pdf(stdClass $issuecert, $markdelivered=true) {
        global  $DB;
        ...
        if ($file = $this->get_issue_file($issuecert)) {
            ...
            if (empty($issuecert->firstdeliverytime) && $markdelivered)
            {
            ...
            if (has_capability('mod/simplecertificate:manage', $this->context, $issuecert->userid) &&
                !is_enrolled($this->get_course_context(), $issuecert->userid)) {
                $file->delete();
            }
        } else {
            error_log(get_string('filenotfound', 'simplecertificate'));
    ...
    protected function get_certificate_text($issuecert, $certtext = null) {
        ...
        //меняем язык, если указано в настройках
        force_current_language($this->get_instance()->forcelanguage);
        ...
        $a->city = $user->city;
        $a->tablegrades = $this->get_tablegrades($user->id);

        //добавление оценок всех модулей в объект
        foreach(simplecertificate_get_mods() as $modid=>$modname)
        {
            //получение оценки за модуль
            $modgrade = $this->get_mod_grade($modid, $user->id);
            //формат для этого типа подстановки
            $grademodformat = $this->get_instance()->grademodfmt;
            //отформатированная оценка
            $a->{'grade_'.$modid} = $this->get_formated_grade($modgrade, $grademodformat);
        }
        ...
        if ($timestart = $DB->get_field_sql($sql, $params, IGNORE_MULTIPLE)) {
        ...
        foreach ($a as $key => $value) {
            ...
            {
                $replace[] = $value;
            } elseif (strtoupper($key) == 'TABLEGRADES' )
            {
 ...
 */
    ...
    protected function can_issue($user = null, $chkcompletation = true) {
        ...
        if ($chkcompletation) {
            ...
            if ($completion->is_enabled($this->get_course_module()) && $this->get_instance()->requiredtime) {
            ...
            if ($CFG->enableavailability) {
                $modinfo = get_fast_modinfo($this->get_course(), $user->id);
    ...
    protected function show_tabs(moodle_url $url) {
        ...
        return $OUTPUT->tabtree($tabs, $tab);
    ...
    public function view_default(moodle_url $url, $canmanage) {
        ...
        if (!$url->get_param('action')) {
            ...
            if (!empty($this->get_instance()->intro)) {
                $html .= $OUTPUT->box(format_module_intro('simplecertificate', $this->get_instance(),
                    $this->get_course_module()->id), 'generalbox', 'intro');
            }

            if ($attempts = $this->get_attempts()) {
                $html .= $this->print_attempts($attempts);
            }

            if (!$canmanage) {
               $this->add_to_log('view', $USER);
            }

            if ($this->get_instance()->delivery != self::OUTPUT_DISABLED || $canmanage) {
                ...
                $html .= html_writer::tag('p', $str, array('style' => 'text-align:center'));
                $linkname = get_string('getcertificate', 'simplecertificate');

                $link = new moodle_url('/mod/simplecertificate/view.php', array('id' => $this->get_course_module()->id, 'action' => 'get'));
                $button = new single_button($link, $linkname);
                $button->add_action(
                    new popup_action('click', $link, 'view' . $this->get_course_module()->id,
                                                    array('height' => 600, 'width' => 800)));

                $html .= html_writer::tag('div', $OUTPUT->render($button), array('style' => 'text-align:center'));
            }
            $html .= $OUTPUT->footer();
        } else { // Output to pdf
            if ($this->get_instance()->delivery != self::OUTPUT_DISABLED || $canmanage) {
                 $html .= $this->output_pdf($this->get_issue($USER));
                 if ($this->get_instance()->delivery != self::OUTPUT_SEND_EMAIL)
                 {
                     $html .= $OUTPUT->header();
                     $html .= $OUTPUT->box(get_string('emailsent', 'simplecertificate') . '<br>' .
                         $OUTPUT->close_window_button(), 'generalbox', 'notice');
                     $html .= $OUTPUT->footer(); //send email
                 }
            }
        }
        return $html;
    ...
    public function view_issued_certificates(moodle_url $url) {
        ...
        if (!$url->get_param('action')) {
            ...
            if (!$users) {
                echo $OUTPUT->notification(get_string('nocertificatesissued', 'simplecertificate'));
                echo $OUTPUT->footer();
                exit();
            }
    ...
    public function view_bulk_certificates(moodle_url $url, array $selectedusers = null) {
        ...
        if (!$action) {
            ...
            $selectoptions = [
                ...
                'delete' => get_string('deletissuedcertificates', 'simplecertificate')
            ];
            ...
            $issuedusers = $this->get_issued_certificate_users();
            foreach ($users as $user) {
                $canissue = $this->can_issue($user, $issuelist != 'allusers');
                if ( empty($canissue) || array_key_exists($user->id, $issuedusers) ) {
            ...
            switch ($type) {
                ...
                case 'delete':
                    ...
                    foreach ($users as $user)
                    {
                        $issuecert = $DB->get_record('simplecertificate_issues', [
                            'userid' => $user->id,
                            'certificateid' => $this->get_instance()->id,
                            'timedeleted' => null
                        ]);
                        if( ! empty($issuecert) )
                        {
                            $this->remove_issue($issuecert);
                            $issuesdeleted++;
                        }
                    }
                    $url->remove_params('action', 'type');
                    redirect($url, get_string('issuesdeleted', 'simplecertificate', $issuesdeleted), 5);
                    break;
            }
            exit();
        }
     ...
     */
    private function add_to_log($action, $user)
    {
        global $DB, $USER;
        ...
        switch ($action)
        {
            default:
                ...
                $instance = $this->get_course_module()->instance;
                $eventdata = [
                ...
                {
                    $eventdata['other'] = [
                        'certificatecode' => $this->get_issue()->code
                    ];
                }

                $event = \mod_simplecertificate\event\course_module_viewed::create($eventdata);
    ...
    /**
     * Автовыпуск сертификата по данному инстансу для указанного пользователя
     * @param object $user - пользователь, для которого необходимо запустить процесс автовыпуска сертификата
     */
    public function user_certificate_issue($user)
    {
        $curtime = time();

        $issuecondserror = $this->can_issue($user);
        if (is_null($issuecondserror))
        {
            $issuecert = $this->get_issue($user);
            $issuefile = $this->get_issue_file($issuecert);

            // проверка, что файл есть
            // проверка, что выпуск сертификата состоялся сейчас, а не ранее
            $timereissue = $issuecert->timereissue ?? 0;
            $timecreated = $issuecert->timecreated ?? 0;
            $lastissuetime = ($timereissue > $timecreated ? $timereissue : $timecreated);
            if ($issuefile && $lastissuetime >= $curtime)
            {
                if ($this->get_instance()->delivery == self::OUTPUT_SEND_EMAIL)
                {// если в качестве доставки сертификата настроена отправка учащемуся по email,
                    // то при автоматической генерации требуется выполнять отправку сразу
                    $this->output_pdf($issuecert, false);
                }

                // проверка, что настройка отправки уведомления включена
                if (!empty($this->get_instance()->autoissuenotification))
                {
                    // параметры по умолчанию для ссылки на доставку сертификата
                    $certificateurlparams = ['id' => $this->get_course_module()->id, 'action' => 'get'];
                    // в зависимости от настроенного способа доставки будет своя надпись, по умолчанию - показать
                    $deliveryverb = get_string('display_certificate', 'simplecertificate');
                    switch($this->get_instance()->delivery)
                    {
                        case self::OUTPUT_DISABLED:
                            // доставка сертификата отключена, учащемуся не нужна ссылка
                            $deliveryverb = null;
                            break;
                        case self::OUTPUT_FORCE_DOWNLOAD:
                            // скачать
                            $deliveryverb = get_string('download_certificate', 'simplecertificate');
                            break;
                        case self::OUTPUT_SEND_EMAIL:
                            // несмотря на то, что настроена доставка по email, предлагаем посмотреть в браузере
                            // для этого через параметры форсируем способ доставки
                            $certificateurlparams['fd'] = self::OUTPUT_OPEN_IN_BROWSER;
                            break;
                    }
                    ...
                    $messagedata->coursefullname = $this->get_course()->fullname;
                    $messagedata->certificatename = $this->get_instance()->name;

                    if (!is_null($deliveryverb))
                    {
                        $certificateurl = new moodle_url('/mod/simplecertificate/view.php', $certificateurlparams);
                        $messagedata->certificatelink = html_writer::link($certificateurl, $deliveryverb, ['target' => '_blank']);
                        $message->contexturl = $certificateurl->out();
                        $message->contexturlname = $issuefile->get_filename();
                    } else {
                        $messagedata->certificatelink = '';
                    }

                    $message->component = 'mod_simplecertificate';
                    $message->name = 'certificate_issued_by_system';
                    $message->userfrom = \core_user::get_noreply_user();
                    $message->userto = $user;
                    $message->subject = get_string('certificate_issued_by_system_subject', 'mod_simplecertificate', $messagedata);
                    $message->smallmessage = get_string('certificate_issued_by_system_smallmessage', 'mod_simplecertificate', $messagedata);
                    $message->fullmessagehtml = get_string('certificate_issued_by_system_fullmessage', 'mod_simplecertificate', $messagedata);
    ...
    public static function get_course_objects(int $courseid)
    {
        $result = [];

        $cmrecords = get_coursemodules_in_course('simplecertificate', $courseid);
        if (!empty($cmrecords))
        {
            foreach($cmrecords as $cmrecord)
            {
                $context = \context_module::instance($cmrecord->id);
                $result[$cmrecord->id] = new \simplecertificate($context, $cmrecord);
            }
        }
