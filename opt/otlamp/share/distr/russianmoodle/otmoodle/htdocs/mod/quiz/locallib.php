...
function quiz_start_new_attempt($quizobj, $quba, $attempt, $attemptnumber, $timenow,
    ...
    foreach ($quizobj->get_questions() as $questiondata) {
        ...
        if ($questiondata->qtype == 'random' || $questiondata->qtype == 'otrandom') {
            $randomfound = true;
            continue;
        }
    ...
    if ($randomfound) {
        ...
        foreach ($quizobj->get_questions() as $questiondata) {
            ...
            if ( $questiondata->qtype === 'otrandom' )
            {
                // Normal case, pick one at random.
                $questionid = $randomloader->get_next_question_id($questiondata->options->targetcategory,
                        (bool)$questiondata->options->includesubcategories, $tagids);
                if ( $questionid === null )
                {
