...
/**
 * Панель управления логикой курса. Обработка карты(обработчика)
 *
...
$processorid = optional_param('processor', 0, PARAM_INT);

// Получение модуля курса
$cm_data = get_course_and_cm_from_instance($instanceid, 'otcourselogic');

// Формирование ссылки
$url = new moodle_url('/mod/otcourselogic/apanel/process_card.php', ['instance' => $instanceid, 'action' => $action, 'processor' => $processorid]);
...
$form = $renderer->get_form_process_card($options);
$form->process();
...
$html .= html_writer::link($apanel_url, get_string('admin_panel', 'mod_otcourselogic'), ['class' => 'mod_otcourselogic btn btn-secondary']);
