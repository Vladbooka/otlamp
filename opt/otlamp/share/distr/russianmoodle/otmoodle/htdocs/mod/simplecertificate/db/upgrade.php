...
function xmldb_simplecertificate_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2015102600) {

        // Define field alternatecode to be added to simplecertificate.
        $table = new xmldb_table('simplecertificate');
        $field = new xmldb_field('alternatecode', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');

        // Conditionally launch add field alternatecode.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    }
    if ($oldversion < 2015111600) {
        ...
        $field = new xmldb_field('certificatetextwidth', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field certificatetextwidth.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('certificatetextheight', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field certificatetextheight.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    }
    if ($oldversion < 2015111700) {
        ...
        $field = new xmldb_field('certificatetextborder', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field certificatetextwidth.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    ...
    if ($oldversion < 2016022501) {
        ...
        $field = new xmldb_field('grademodfmt', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field certificatetextwidth.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    ...
    if ($oldversion < 2016030200) {
        ...
        $field = new xmldb_field('gradedecimals', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '2');
        // Conditionally launch add field certificatetextwidth.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    }
    if ($oldversion < 2016030201) {

        // Define field alternatecode to be added to simplecertificate.
        $table = new xmldb_table('simplecertificate');

        $field = new xmldb_field('forcelanguage', XMLDB_TYPE_CHAR, '2', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field certificatetextwidth.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    ...
    if ($oldversion < 2020060800) {
        ...
        $field = new xmldb_field('issueongrade', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('issueonotcl', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('otcltoissue', XMLDB_TYPE_INTEGER, '10', null, false, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('autoissuenotification', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '1');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('modalundelivered', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $field = new xmldb_field('firstdeliverytime', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, false, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $DB->execute('UPDATE {simplecertificate_issues} set firstdeliverytime=timecreated ');

    }
    if ($oldversion < 2020122402) {

        $table = new xmldb_table('simplecertificate');

        $field = new xmldb_field('shelflife', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $field = new xmldb_field('timereissue', XMLDB_TYPE_INTEGER, '10', null, false, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
