...
/**
 * Класс способа указания даты - относительная дата (относительно даты старта подписки)
 *
 * @package   mod_event3kl
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class relative_to_enrolment extends abstract_datemode implements datemode_interface {
    ...
    /**
     * Получение точки отсчета для формирования даты относительно даты начала подписки
     * @return string
     */
    protected function set_default_startingpoint() {

        if (isset($this->userid) && isset($this->event3kl)) {
            $enrolments = enrol_get_course_users($this->event3kl->get('course'), true, [$this->userid]);
            $datestart = null;
            foreach($enrolments as $enrolment) {
                if (is_null($datestart) || $enrolment->uetimestart < $datestart) {
                    $datestart = $enrolment->uetimestart;
                }
            }
        }

        // по подписке пользователя дата не определилась
        // если указана группа - берем за точку отсчета дату её старта
        if (empty($datestart) && isset($this->groupid)) {
        ...
        // по подписке пользователя дата не определилась
        // по группе дата не определилась
        // получаем дату старта курса
        if (empty($datestart)) {
