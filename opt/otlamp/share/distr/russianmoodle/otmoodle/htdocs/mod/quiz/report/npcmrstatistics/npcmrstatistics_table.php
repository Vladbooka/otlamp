...
/**
 * Quiz npcmrstatistics report, table for showing npcmrstatistics of each question in the quiz.
 *
 ...
 *
 * There are columns for the various item and position npcmrstatistics.
 *
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_npcmrstatistics_table extends flexible_table {
    ...
    public function __construct() {
        global $PAGE;
        parent::__construct('mod-quiz-report-npcmrstatistics-report');
        $PAGE->requires->js_call_amd('quiz_npcmrstatistics/qtext-switcher', 'init');
    }
     ...
     * @param int $s number of attempts included in the npcmrstatistics.
     */
    public function npcmrstatistics_setup($quiz, $cmid, $reporturl, $s) {
        $this->quiz = $quiz;
        $this->cmid = $cmid;
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'quiz_npcmrstatistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'quiz_npcmrstatistics');
        }
        ...
        $columns[] = 'qtext';
        $headers[] = get_string('questiontext', 'quiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'quiz_npcmrstatistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'quiz_npcmrstatistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'quiz_npcmrstatistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'quiz_npcmrstatistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'quiz_npcmrstatistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'quiz_npcmrstatistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'quiz_npcmrstatistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'quiz_npcmrstatistics');

        $this->define_columns($columns);
        $this->define_headers($headers);
        $this->sortable(false);
        
        $this->column_class('qtext', 'qtext-cell');
        ...
        $this->set_attribute('id', 'questionnpcmrstatistics');
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'quiz_npcmrstatistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'quiz_npcmrstatistics')));
                ...
                //                 var_dump($questionstat->get_variants(), $questionstat->get_sub_question_ids());
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                        $name,
                        array('title' => get_string('slotstructureanalysis', 'quiz_npcmrstatistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                        $name,
                        array('title' => get_string('detailedanalysis', 'quiz_npcmrstatistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'quiz_npcmrstatistics') . '<br />' . $name;
        }
     ...
     * @return string contents of this table cell.
     */
    protected function col_qtext($questionstat) {
        
        global $OUTPUT;
        
        $context = context_module::instance($this->cmid);
        
        $qtext = question_rewrite_question_preview_urls($questionstat->question->questiontext, $questionstat->question->id,
            $questionstat->question->contextid, 'question', 'questiontext', $questionstat->question->id,
            $context->id, 'quiz_npcmrstatistics');
        
        $text = format_text(
            $qtext,
            $questionstat->question->questiontextformat,
            [
                'noclean' => true,
                'para' => false,
                'overflowdiv' => false
        ...
        // очищаем текст от тегов, кроме безобидных
        $cleantext = strip_tags($text, '<b><i><br><p><span><strong>');
        // пустые span'ы удаляем
        $cleantext = preg_replace("/<span[^>]*><\\/span[^>]*>/", '', $cleantext);
        // пустые параграфы удаляем
        $cleantext = preg_replace("/<p[^>]*><\\/p[^>]*>/", '', $cleantext);
        
        // обрезанный текст с безобидными тегами (для проверки соответствия оригинальному тексту
        $shorttext = mb_substr($cleantext, 0, 150);
        // обрезанный и очищенный текст
        $shortcleantext = mb_substr(strip_tags($text), 0, 150);
        
        if ($questionstat->question->qtype == 'random')
        {
            $text = $shorttext = get_string('textforinslot', 'quiz_npcmrstatistics');
        }
        ...
        return (strip_tags($cleantext) == '' ? '' : html_writer::div($shortcleantext, 'qtext')) .
            ($shorttext == $text ? '' : html_writer::div($text, 'fulltext') . html_writer::div('', 'qtext-expander'));
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'quiz_npcmrstatistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'quiz_npcmrstatistics'),
    ...
    public function  wrap_html_start() {
        ...
        if (!$this->is_downloading()) {
            echo html_writer::start_tag('div', array('id' => 'tablecontainer',
                    'class' => 'npcmrstatistics-tablecontainer'));
        }
    }
