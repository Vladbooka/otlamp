...
/**
 * Standard plugin entry points of the quiz npcmrstatistics report.
 *
 ...
 */
function quiz_npcmrstatistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('quiz/npcmrstatistics:view', $context);
...
/**
 * Quiz npcmrstatistics report cron code. Deletes cached data more than a certain age.
 */
function quiz_npcmrstatistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quiz npcmrstatistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('quiz_npcmrstatistics', 'timemodified < ?', array($expiretime));
