...
require_once ($CFG->dirroot . '/mod/simplecertificate/backup/moodle2/restore_simplecertificate_stepslib.php'); // Because it exists (must)
...
class restore_simplecertificate_activity_task extends restore_activity_task {
    ...
    protected function define_my_steps() {
        ...
        $this->add_step(new restore_simplecertificate_activity_structure_step('simplecertificate_structure', 'simplecertificate.xml'));
    }
    ...
    static public function define_restore_log_rules() {
        ...
        $rules[] = new restore_log_rule('simplecertificate', 'verifyt', 'verify.php?code={simplecertificate_issues}', '{simplecertificate}');
	...
	/*
     * This function is called after all the activities in the backup have been restored. This allows us to get the new course module ids, as they may have been restored after the
    ...
    public function after_restore() {
        ...
        if ($certificate = $DB->get_record('simplecertificate', array('id' => $this->get_activityid()))) {
            if ($certificate->certdate  <= -1000) { // If less or equal -1000, is mark as not sucefully retored in stepslib
                ...
                if ($mapping = restore_dbops::get_backup_ids_record($this->get_restoreid(), 'course_module', $certificate->certdate)){
                    ...
                    $certificate->certdate = $mapping->newitemid;
                     ...
                     $this->get_logger()->process("Failed to restore dependency in simplecertificate 'certdate'. " .
            ...
            if ($certificate->certgrade <= -1000) {// If greater than 0, then it is a grade item value
                ...
                if ($mapping = restore_dbops::get_backup_ids_record($this->get_restoreid(), 'course_module', $certificate->certgrade)) {
                    $certificate->certgrade = $mapping->newitemid;
                } else {
                    $this->get_logger()->process("Failed to restore dependency in simplecertificate 'certgrade'. " .
                                                 "Backup and restore will not work correctly unless you include the dependent module.", 
                                                backup::LOG_ERROR);
                }
            }
            ...
            if ($issues = $DB->get_records('simplecertificate_issues', array('certificateid' => $certificate->id))) {
                ...
                foreach ($issues as $issued) {
                    try {
                        ...
                        if ($this->get_old_moduleversion() < 2014051000 && ($user = $DB->get_record("user", array('id' => $issued->userid)))) {
                            $filename = str_replace(' ', '_', clean_filename($issued->certificatename . ' ' . fullname($user) . ' ' . $issued->pathnamehash . '.pdf'));
                        } else {
                            $filename = str_replace(' ', '_', clean_filename($issued->certificatename . ' ' . $issued->pathnamehash . '.pdf'));
                        }
                        ...
                        if ($fs->file_exists($fileinfo['contextid'], $fileinfo['component'], $fileinfo['filearea'], 
                            ...
                            $newfileinfo['filename'] = str_replace(' ', '_', clean_filename($issued->certificatename . ' ' . $issued->id . '.pdf'));
