...
/**
 * Quiz npcmrstatistics report class.
 *
...
require_once($CFG->dirroot . '/mod/quiz/report/npcmrstatistics/npcmrstatistics_form.php');
require_once($CFG->dirroot . '/mod/quiz/report/npcmrstatistics/npcmrstatistics_table.php');
require_once($CFG->dirroot . '/mod/quiz/report/npcmrstatistics/npcmrstatistics_question_table.php');
require_once($CFG->dirroot . '/mod/quiz/report/npcmrstatistics/npcmrstatisticslib.php');
/**
 * The quiz npcmrstatistics report provides summary information about each question in
 * a quiz, compared to the whole quiz. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_npcmrstatistics_report extends quiz_default_report {

    /** @var context_module context of this quiz.*/
    protected $context;

    /** @var quiz_npcmrstatistics_table instance of table class used for main questions stats table. */
    protected $table;
    ...
    public function display($quiz, $cm, $course) {
        ...
        if (!quiz_has_questions($quiz->id)) {
            $this->print_header_and_tabs($cm, $course, $quiz, 'npcmrstatistics');
        ...
        $pageoptions['mode'] = 'npcmrstatistics';

        $reporturl = new moodle_url('/mod/quiz/report.php', $pageoptions);

        $mform = new quiz_npcmrstatistics_settings_form($reporturl, compact('quiz'));
        ...
        $qubaids = quiz_npcmrstatistics_qubaids_condition($quiz->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new quiz_npcmrstatistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'quiz_npcmrstatistics');
        } else {
            $report = get_string('questionstatsfilename', 'quiz_npcmrstatistics');
        }
        ...
        $this->table->is_downloading($download, $filename,
                get_string('quizstructureanalysis', 'quiz_npcmrstatistics'));
        ...
        if (!$nostudentsingroup) {
            ...
            $quizstats = new \quiz_npcmrstatistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->npcmrstatistics_setup($quiz, $cm->id, $reporturl, $quizstats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'quiz_npcmrstatistics'));
                }
            }

            if (!$this->table->is_downloading() && $quizstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'quiz_npcmrstatistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            if ($quizstats->s()) {
                ...
                if ($this->table->is_downloading() == 'html' && $quizstats->s() != 0) {
                    $this->output_npcmrstatistics_graph($quiz->id, $qubaids);
                }
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtoquizreport', 'quiz_npcmrstatistics') . '</a>',
...
//                 );
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'quiz_npcmrstatistics', $number), 3);
                }
            ...
            echo $OUTPUT->heading(get_string('quizinformation', 'quiz_npcmrstatistics'), 3);
            ...
            if ($quizstats->s()) {
                echo $OUTPUT->heading(get_string('quizstructureanalysis', 'quiz_npcmrstatistics'), 3);
                $this->output_quiz_structure_analysis_table($questionstats);
                $this->output_npcmrstatistics_graph($quiz, $qubaids);
            }
        }
    ...
    protected function output_individual_question_data($quiz, $questionstat) {
        ...
        // and the question npcmrstatistics.
        ...
        $questioninfotable->data[] = array(get_string('questionname', 'quiz_npcmrstatistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'quiz_npcmrstatistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'quiz_npcmrstatistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'quiz_npcmrstatistics'),
                $questionstat->positions);

        // Set up the question npcmrstatistics table.
        ...
        $labels = array(
            's' => get_string('attempts', 'quiz_npcmrstatistics'),
            'facility' => get_string('facility', 'quiz_npcmrstatistics'),
            'sd' => get_string('standarddeviationq', 'quiz_npcmrstatistics'),
            'random_guess_score' => get_string('random_guess_score', 'quiz_npcmrstatistics'),
            'intended_weight' => get_string('intended_weight', 'quiz_npcmrstatistics'),
            'effective_weight' => get_string('effective_weight', 'quiz_npcmrstatistics'),
            'discrimination_index' => get_string('discrimination_index', 'quiz_npcmrstatistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'quiz_npcmrstatistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'quiz_npcmrstatistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionnpcmrstatistics', 'quiz_npcmrstatistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'quiz_npcmrstatistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new quiz_npcmrstatistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'quiz_npcmrstatistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'quiz_npcmrstatistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'quiz_npcmrstatistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'quiz_npcmrstatistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'quiz_npcmrstatistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'quiz_npcmrstatistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the quiz with their npcmrstatistics.
     *
    ...
    protected function output_quiz_structure_analysis_table($questionstats) {
        ...
        foreach ($questionstats->get_all_slots() as $slot) {
            // Output the data for these question npcmrstatistics.
    ...
    /**
     * Return HTML for table of overall quiz npcmrstatistics.
     *
    ...
    /**
     * Download the table of overall quiz npcmrstatistics.
     *
     * @param array $quizinfo as returned by {@link get_formatted_quiz_info_data()}.
     */
    protected function download_quiz_info_table($quizinfo) {
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('quizinformation', 'quiz_npcmrstatistics'));
    ...
    /**
     * Output the HTML needed to show the npcmrstatistics graph.
     *
     ...
     */
    protected function output_npcmrstatistics_graph($quizorid, $qubaids) {
        global $DB, $PAGE;
        ...
        $questionnpcmrstatistics = $DB->get_records_select('question_statistics',
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'quiz_npcmrstatistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'quiz_npcmrstatistics')
        ];
        ...
        foreach ($questionnpcmrstatistics as $questionstatistic) {
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'quiz_npcmrstatistics'));
        ...
        $graphname = get_string('npcmrstatisticsreportgraph', 'quiz_npcmrstatistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the quiz and question npcmrstatistics, either by loading the cached results,
     ...
     * @return array with 2 elements:    - $quizstats The npcmrstatistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
        ...
        $quizcalc = new \quiz_npcmrstatistics\calculator($progress);
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'quiz_npcmrstatistics'));
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'quiz_npcmrstatistics'),
    ...
    protected function output_caching_info($lastcachetime, $quizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        ...
        // Find the number of attempts since the cached npcmrstatistics were computed.
        list($fromqa, $whereqa, $qaparams) = quiz_npcmrstatistics_attempts_sql($quizid, $groupstudentsjoins, $whichattempts, true);
        ...
        $output .= get_string('lastcalculated', 'quiz_npcmrstatistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'quiz_npcmrstatistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('quiz_npcmrstatistics', array('hashcode' => $qubaids->get_hash_code()));
