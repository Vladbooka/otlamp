...
/**
 * Quiz npcmrstatistics report, table for showing response analysis for a particular question (or sub question).
 *
...
/**
 * This table shows npcmrstatistics about a particular question.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_npcmrstatistics_question_table extends flexible_table {
     ...
     * Constructor.
     *
     * @param int $qid the id of the particular question whose npcmrstatistics are being
     * displayed.
     */
    public function __construct($qid) {
        parent::__construct('mod-quiz-report-npcmrstatistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata, $s, $responseanalysis) {
        ...
        if ($responseanalysis->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'quiz_npcmrstatistics');
        }

        if ($responseanalysis->has_multiple_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'quiz_npcmrstatistics');

            if ($responseanalysis->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'quiz_npcmrstatistics');
            }

        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'quiz_npcmrstatistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'quiz_npcmrstatistics');

        if (!$responseanalysis->has_multiple_tries_data()) {
            $columns[] = 'totalcount';
            $headers[] = get_string('count', 'quiz_npcmrstatistics');
        } else {
            $countcolumns = range(1, $responseanalysis->get_maximum_tries());
            foreach ($countcolumns as $countcolumn) {
                $columns[] = 'trycount'.$countcolumn;
                $headers[] = get_string('counttryno', 'quiz_npcmrstatistics', $countcolumn);
            }
        }

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'quiz_npcmrstatistics');
