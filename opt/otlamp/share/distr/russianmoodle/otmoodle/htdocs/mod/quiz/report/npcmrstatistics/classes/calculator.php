...
namespace quiz_npcmrstatistics;
defined('MOODLE_INTERNAL') || die();

/**
 * Class to calculate and also manage caching of quiz npcmrstatistics.
 *
 * These quiz npcmrstatistics calculations are described here :
 *
 * http://docs.moodle.org/dev/Quiz_npcmrstatistics_calculations#Test_npcmrstatistics
 *
...
class calculator {
    ...
    /**
     * Compute the quiz npcmrstatistics.
     *
     ...
     * @param float $sumofmarkvariance sum of mark variance, calculated as part of question npcmrstatistics
     * @return calculated $quizstats The npcmrstatistics for overall attempt scores.
     */
    public function calculate($quizid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                quiz_npcmrstatistics_attempts_sql($quizid, $groupstudentsjoins, $whichattempts);
            ...
            $quizstats->cache(quiz_npcmrstatistics_qubaids_condition($quizid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $quizstats;
    }

    /** @var integer Time after which npcmrstatistics are automatically recomputed. */
    const TIME_TO_CACHE = 900; // 15 minutes.

    /**
     * Load cached npcmrstatistics from the database.
     *
     * @param $qubaids \qubaid_condition
     * @return calculated The npcmrstatistics for overall attempt scores or false if not cached.
     */
    public function get_cached($qubaids) {
        global $DB;

        $timemodified = time() - self::TIME_TO_CACHE;
        $fromdb = $DB->get_record_select('quiz_npcmrstatistics', 'hashcode = ? AND timemodified > ?',
    ...
    /**
     * Find time of non-expired npcmrstatistics in the database.
     *
     * @param $qubaids \qubaid_condition
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('quiz_npcmrstatistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }
     ...
     *
     * Note internally we use the grading method constants to represent which attempts we are calculating npcmrstatistics for, each
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'quiz_npcmrstatistics');
    }

    /**
     * Given a particular quiz grading method return a string id for use as a field name prefix in mdl_quiz_npcmrstatistics or to
    ...
    protected function attempt_counts_and_averages($quizid, \core\dml\sql_join $groupstudentsjoins) {
        ...
        foreach (array_keys(quiz_get_grading_options()) as $which) {
            ...
            list($fromqa, $whereqa, $qaparams) = quiz_npcmrstatistics_attempts_sql($quizid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/Quiz_npcmrstatistics_calculations#Median_Score
     *
