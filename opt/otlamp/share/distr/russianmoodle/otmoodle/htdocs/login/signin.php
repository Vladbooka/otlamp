...
if (isloggedin() and !isguestuser()) {
    ...
    // если в теме есть метод, возвращающий страницу верстки, юзаем её, иначе стандарт
    $corerenderer = $PAGE->theme->get_renderer($PAGE, 'core');
    if(method_exists($corerenderer, 'get_login_form_path'))
    {
        $loginformpath = $corerenderer->get_login_form_path();
        if( ! is_null($loginformpath) )
        {
            include($loginformpath);
            if(method_exists($corerenderer, 'js_login_form_call'))
            {
                $corerenderer->js_login_form_call();
            }
        } else 
        {
            ...
            \core\notification::error(get_string('alternate_login_form_not_found', 'theme_opentechnology'));
        }
    } else 
    {
        $loginform = new \core_auth\output\login($authsequence, $frm->username);
