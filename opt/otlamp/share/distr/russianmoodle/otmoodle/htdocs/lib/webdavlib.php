...
EOD;
    ...
    private function get_respond($fp = null) {
        ...
        case preg_match('/Content\\-Length:\\s+([0-9]*)\\r\\n/',$header,$matches):
            ...
            // полученная длина
            $loadedlength = 0;
            // пытаемся получить контент пока не достигнем заявленной длины
            while ($loadedlength < $contentlength) {
                // остаток
                $remain = $contentlength - $loadedlength;
                // запрашивать будем фиксированную длину или остаток, если он меньше
                $requestlength = ($remain > $max_chunk_size ? $max_chunk_size : $remain);
                // результат запроса
                $chunk = fread($this->sock, $requestlength);
                // длина ответа
                $responselength = strlen($chunk);
                // пересчитываем полученную длину
                $loadedlength += $responselength;
                // выводим в лог сообщение для дебага
                $this->_error_log('loaded length: ' . $loadedlength . ' (requested length: ' . $requestlength . '; response length: ' . $responselength . ';)');
                // записываем полученный контент
                self::update_file_or_buffer($chunk, $fp, $buffer);
            }
