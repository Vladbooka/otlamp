...
foreach ($requests as $request) {
    $methodname = clean_param($request['methodname'], PARAM_ALPHANUMEXT).'_after_connection_closed';
    $function = $DB->get_record('external_functions', ['name' => $methodname]);
    if ($function != false)
    {
        external_api::call_external_function($methodname, $request['args'], true);
    }
}