...
/**
 * Роутер статусов для инвентаря
 *
 * @package    workflow
 * @subpackage invitems
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_invitems implements dof_workflow
{
    /**
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'invitems','eventcode'=>'insert'));
    }
    /** 
    ...
    {
		if ( $gentype==='storage' AND $gencode === 'invitems' AND $eventcode === 'insert' )
		{
    ...
    public function get_list()
    {
		return array('active'=>$this->dof->get_string('status:active',$this->get_storage(),NULL,'workflow'),
					 'notavailable'=>$this->dof->get_string('status:notavailable',$this->get_storage(),NULL,'workflow'),
		             'scrapped'=>$this->dof->get_string('status:scrapped',$this->get_storage(),NULL,'workflow'),
		             'deleted'=>$this->dof->get_string('status:deleted',$this->get_storage(),NULL,'workflow'),
		             'repairing'=>$this->dof->get_string('status:repairing',$this->get_storage(),NULL,'workflow'));
    }
    ...
    public function get_meta_list($type)
    {
        switch ( $type )
        {
            ...
            case 'actual':
                return array('active'=>$this->dof->get_string('status:active',$this->code(),NULL,'workflow'),
                             'notavailable'=>$this->dof->get_string('status:notavailable',$this->code(),NULL,'workflow'),
                             'repairing'=>$this->dof->get_string('status:repairing',$this->code(),NULL,'workflow'));
            case 'real':
                return array('active'=>$this->dof->get_string('status:active',$this->code(),NULL,'workflow'),
                             'notavailable'=>$this->dof->get_string('status:notavailable',$this->code(),NULL,'workflow'),
                             'repairing'=>$this->dof->get_string('status:repairing',$this->code(),NULL,'workflow'),
                             'scrapped'=>$this->dof->get_string('status:scrapped',$this->code(),NULL,'workflow'));  
            case 'junk':                
    ...
    public function get_available($id)
    {
		...
		switch ( $obj->status )
		{
			case 'active':       // переход из статуса "доступен"
				$statuses['scrapped']  = $this->get_name('scrapped');
				$statuses['repairing'] = $this->get_name('repairing');
				$statuses['deleted']  = $this->get_name('deleted');
				$statuses['notavailable'] = $this->get_name('notavailable');
            break;
            case 'scrapped':       // переход из статуса "списано"
				$statuses = array();
			break;
            case 'notavailable':       // переход из статуса "не доступен"
				$statuses['active'] = $this->get_name('active');
				$statuses['scrapped'] = $this->get_name('scrapped');
				$statuses['repairing'] = $this->get_name('repairing');
			break;
            case 'repairing':       // переход из статуса "в ремонте"
				$statuses['active'] = $this->get_name('active');
				$statuses['scrapped'] = $this->get_name('scrapped');
            case 'deleted':  // переход из статуса "удален"
                $statuses = array();
            break;
     ...
     * Возвращает массив всех состояний,   
	 * метастатусов
	 * @param integer $param - название метастатуса
     * @return array
     * @access public
     */
    public function get_list_param($param)
    {
        switch ( $param )
        {
            // реальный метастатус
            case 'real': return array('active','notavailable','scrapped','repairing');
            // актуальный
            case 'actual': return array('active','notavailable','repairing');
