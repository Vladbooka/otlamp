...
class dof_im_acl implements dof_plugin_im
{
    ...
    function get_section($name, $id = 1)
    {
        ...
        $this->dof->modlib('nvg')->add_css('im', 'acl', '/style.css');
        
        $result = '';
        switch ($name)
        {
            case "user_and_acl_link":
                $person = $this->dof->storage('persons')->get((int)$id);
                if( !empty($person) )
                {
                    //имя пользователя
                    $spanfullname = dof_html_writer::span(
                        $this->dof->storage('persons')->get_fullname($person->id),
                        'user_and_acl_fullname'
                    );
                    
                    //ссылка на список прав доступа
                    $spanlink = '';
                    if ( $this->is_access('aclwarrants:view', null, null, $depid) )
                    {//есть право на просмотр прав
                        //формируем ссылку
                        $link = dof_html_writer::link(
                            $this->dof->url_im($this->code(),'/personacl.php',[
                                'id' => $person->id,
                                'departmentid' => $depid
                            ]),
                            $this->dof->get_string('acl_link_title','acl')
                        );
                        //оборачиваем в span
                        $spanlink = dof_html_writer::span(" (".$link.")");
                    }
                    //результат - фио со ссылкой на просмотр прав доступа
                    $result = dof_html_writer::div($spanfullname.$spanlink, 'user_and_acl_link'); 
                }
                break;
            case "my_warrants":
                $person = $this->dof->storage('persons')->get((int)$id);
                ...
                {// Персона есть в деканате
                    $result = $this->get_warrants($person->id,$depid);
                }
                break;
        }
     ...
     * @return array
     */
    private function get_fields_description($type,$options=[])
    {
        switch ( $type )
		{
			case 'person':
            ...
            case 'warrant':
                ...
                if ( ! empty($options['addvars']) )
                {
                    ...
                    if ( ! empty($options['addvars']['sortfield']) )
                    { //поле для сортировки выбрано
                        $sortfield = $options['addvars']['sortfield'];
                        $sortdirection = 'ASC';
                        if ( ! empty($options['addvars']['sortdirection']) )
                        { //направление сортировки выбрано
                            $sortdirection = $options['addvars']['sortdirection'];
                            unset($addvars['sortdirection']);
                        }
                        unset($addvars['sortfield']);
                    }
                }
                
                //тексты заголовков
                $actionstext = $this->dof->modlib('ig')->igs('actions');
                $plugintypetext = $this->dof->get_string('plugintype', 'acl');
                $plugincodetext = $this->dof->get_string('plugincode', 'acl');
                $codetext = $this->dof->get_string('code', 'acl');
                $legendtext = $this->dof->get_string('legend', 'acl');
                $objectidtext = $this->dof->get_string('objectid', 'acl');
                
                //по умолчанию делаем ссылку на сортировку по возрастанию для всех сортируемых полей
                $sortbyplugintypeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                    array_merge($addvars, 
                        [
                            'sortfield' => 'plugintype',
                            'sortdirection' => 'ASC'
                        ]));
                $sortbyplugincodeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                    array_merge($addvars, 
                        [
                            'sortfield' => 'plugincode',
                            'sortdirection' => 'ASC'
                        ]));
                $sortbycodeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                    array_merge($addvars, 
                        [
                            'sortfield' => 'code',
                            'sortdirection' => 'ASC'
                ...
                if ( ! empty($sortfield) )
                { //выбрано поле для сортировки
                    //если отсортировано в определенном порядке - надо предложить сменить порядок на обратный
                    $changedsortdirection = 'DESC';
                    //обозначение текущего направления стрелкой
                    $currentdirection = '&darr;';
                    if ( $sortdirection == 'DESC' )
                    {
                        $changedsortdirection = 'ASC';
                        $currentdirection = '&uarr;';
                    }
                    
                    switch ( $sortfield )
                    { //в зависимости от выбранного поля и направления - меняем ссылку на сортировку
                        case 'plugintype':
                            $sortbyplugintypeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                                array_merge($addvars, 
                                    [
                            ...
                            $plugintypetext = $plugintypetext . " " . $currentdirection;
                            break;
                        case 'plugincode':
                            $sortbyplugincodeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                                array_merge($addvars, 
                            ...
                            $plugincodetext = $plugincodetext . " " . $currentdirection;
                            break;
                        case 'code':
                            $sortbycodeurl = $this->dof->url_im('acl', '/warrantacl.php', 
                                array_merge($addvars, 
                            ...
                            $codetext = $codetext . " " . $currentdirection;
                            break;
                    }
                }
                ...
                return [
                    $actionstext,
                    html_writer::link($sortbyplugintypeurl, $plugintypetext),
                    html_writer::link($sortbyplugincodeurl, $plugincodetext),
                    html_writer::link($sortbycodeurl, $codetext),
                    $legendtext,
                    $objectidtext
                ];
            case 'acl':
   ...
   /** Возвращает html-код таблицы
     ...
     */
    private function print_table($date,$type,$options=[])
    {
        $addvars = [];
        if ( ! empty($options['addvars']) )
        {
     ...
     * @return mixed string в string html-код или false в случае ошибки
     */
    public function get_table_right_warrant($list, $options=[])
    {
        if ( ! is_array($list))
        {// переданны данные неверного формата
        	return false;
    	...
    	foreach ($list as $obj)
    	{
    	    ...
    	    if ( $this->is_access('aclwarrantagents:view') )   
            {
                ...
                $obj->id,[]).'><img src="'.$this->dof->url_im('acl', '/icons/persons_acl.png').'" 
            ...
            if ( $this->is_access('acl:delete') ) 
            {
                ...
                $obj->id,[]).'><img src="'.$this->dof->url_im('acl', '/icons/delete.png').'" 
    ...
    public function get_warrants($intvar, $depid = 0) 
    {
        ...
        foreach ( $warrants as $warrant )
        {
            ...
            if ($this->is_access('aclwarrants:delegate', null, null, $depid))
            {
    ...
    public function show_one_warrant($addvars, $aclwarrantid)
    {
       ...
       // $table->size = array('20%', '10%', '20%', '5%', '5%', '10%', '15%', '15%');
        ...
        {// маcсив не пустой - заносим данные в таблицу
            ...
            foreach ( $obj as $elm )
            {
                $table->data[] = array('<b>'.array_shift($head).'</b>', $elm);
            } 
