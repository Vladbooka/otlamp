...
/**
 * Задача по обслуживанию Деканата. Исполнение срочных задач.
 *
 * @package    block
 * @subpackage dof
...
class execute_plugins_cron_loan_1 extends scheduled_task
{
    /**
    ...
    {
        return get_string('execute_plugins_cron_loan_1', 'block_dof');
    }
    ...
    public function execute()
    {
        ...
        dof_mtrace(1, 'Load plugins cron with loan 1: ');
        ...
        {// Плагины, требующие запуска задачи, найдены
            ...
            foreach ( $plugins as $plugin )
            {
                ...
                // Исполняем задание
                if ( $DOF->plugin($plugin->type, $plugin->code)->cron(1, 3) )
        ...
        // Исполняем задания
        dof_mtrace(1, "Load todo`s with loan 1: ");
        //  Получаем список неисполненных заданий
        $todos = $DB->get_records_select('block_dof_todo', "exdate=0 AND tododate < " . time() . " AND loan = 1");
