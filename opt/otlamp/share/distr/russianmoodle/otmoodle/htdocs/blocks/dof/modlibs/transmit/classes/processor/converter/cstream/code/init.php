...
/**
 * Обмен данных с внешними источниками. Конвертер кода учебного процесса в идентификатор
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_cstream_code extends dof_modlib_transmit_processor_base
{
    /**
    ...
    {
        $items = $dof->storage('cstreams')->get_records(['code' => $input['code']]);
        if ( ! empty($items) )
        {
            return ['id' => key($items)];
        }
