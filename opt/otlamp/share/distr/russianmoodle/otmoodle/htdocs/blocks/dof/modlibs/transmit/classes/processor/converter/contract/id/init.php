...
/**
 * Обмен данных с внешними источниками. Конвертер идентификатора договора в номер договора
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_contract_id extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_required = ['contractid' => null];
    ...
    {
        $num = $dof->storage('contracts')->get_field($input['id'], 'num');
        if ( $num )
        {// Контракт найден
            return ['num' => $num];
        }
