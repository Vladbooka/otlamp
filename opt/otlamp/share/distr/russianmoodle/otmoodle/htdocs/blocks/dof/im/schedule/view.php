...
$formeditinfo = optional_param('editinfo', '', PARAM_RAW_TRIMMED);
// Создание ссылки на историю статусов
$linkstatushistory = $DOF->modlib('widgets')->status_link(
    ['plugintype' => 'storage', 'plugincode' => 'schtemplates', 'objectid' => $tmid]
    );
//вывод на экран
...
// Информация после сохранения формы
if ( ! empty($formeditinfo) )
{
    echo dof_html_writer::div($DOF->get_string($formeditinfo, 'schedule'), 'block_dof_notice_message alert alert-notice');
}

$load = new dof_im_schedule_master_make($DOF, $addvars['departmentid'], null, $addvars);
// отобразим недогруз
echo $load->get_underload_cstream($template->cstreamid,true);
// вывод информации о периоде
echo '<br>'.$show->get_table_one($tmid);
// Вывод ссылки на историю статусов
echo $linkstatushistory;
