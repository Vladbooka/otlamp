...
class dof_storage_sync extends dof_storage
{
    ...
    {
        $url = $this->dof->plugin_path('storage', 'sync', '/connect.php');
        require_once($url);
    ...
    /**
     * Вычисление хэша объекта
     * 
     * Требуется для определения изменений в объектах
     * 
     * @param stdClass $object - Объект для хэширования
     * @param stdClass|null $hashmask - Маска хэширования
     *   
     * @return string|false - Хэш объекта
     */
    public function makeHash($object, $hashmask = null)
    {
        ...
        $clone = fullclone($object);
        
        // Фильтрация по маске
        if ( is_object($hashmask) && ! is_null($hashmask) )
        {// Маска определена
            
            // Очистка полей, указанных в маске как 'ignore'
            foreach ( $hashmask as $key => $value )
            {
                if ( property_exists($clone, $key) AND $value == 'ignore' )
