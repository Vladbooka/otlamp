...
class dof_storage_persons extends dof_storage implements dof_storage_config_interface
{
    ...
    public function list_catch_events()
    {
        return [
            ...
            ['plugintype'=>'sync', 'plugincode'=>'personstom', 'eventcode'=>'sync_finished']
        ];
    }
    ...
    public function catch_event($gentype,$gencode,$eventcode,$intvar,$mixedvar)
    {
        ...
        // Завершилась синхронизация персоны ЭД в Moodle
        if ( $gentype === 'sync' && $gencode === 'personstom' && $eventcode === 'sync_finished' )
        {
            $oldperson = $mixedvar;
            $mdluserid = $intvar;
            
            if ( empty($oldperson->mdluser) && ! empty($mdluserid) )
            {// Персона ранее не была синхронизирована, а теперь синхронизировалась
                
                // Подготовка объекта для обновления
                $newperson = new stdClass();
                $newperson->sync2moodle = 1;
                $newperson->mdluser = $intvar;
                // Сохраняем созданный идентификатор mdluser в объект персоны
                $this->dof->storage('persons')->update($newperson, $oldperson->id, true);
                
                $this->dof->send_event('storage', 'persons', 'person_sync_saved', $oldperson->id);
            }
        }
        return true;
    }
    ...
    public function config_default($code=null)
    {
        ...
        // Настройка, разрешающая менять профиль пользователя, зарегистрированного не через auth_dof
        // По умолчанию мы запрещаем такие операции, но есть моменты, когда это необходимо, например,
        // генерация нового пароля и отправка его пользователю при первичной синхронизации персоны ЭД
        // с пользователем Moodle
        $obj = new stdClass();
        $obj->type = 'checkbox';
        $obj->code = 'allow_change_not_auth_dof_users';
        $obj->value = '0';
        $config[$obj->code] = $obj;
     ...
     * @param bool $quiet флаг выброса события
    ...
    {
        $this->prepare_insert_object($dataobject);
        return parent::insert($dataobject, $quiet, $bulk, $options);
    }
    
    /**
     * Получение объекта персоны вместе со всеми его дополнительными полями
     *
     * @param int $personid - идентификатор персоны
     *
     * @return object
     */
    public function get_person_with_customs($personid)
    {
        $person = $this->get($personid);
        $personwithcustoms = $this->dof->storage('cov')->fill_object_with_all_customs(
            $this->type(),
            $this->code(),
            $person
        );
        return $personwithcustoms;
    }
    
    /**
     * Извлечение из сохраняемого объекта всех данных, подлежащих хранению в кастомных полях (по префиксу cov__)
     *
     * @param array|stdClass $data - объект персоны (будет очищен от кастомных полей)
     *
     * @return stdClass - объект с кастомными полями
     */
    public function extract_custom_data(&$data)
    {
    ...
    /**
     * Сохранение кастомных полей в привязке к персоне
     *
     * @param int $personid - Идентификатор персоны
     *
     * @param object $data - объект с данными персоны
     */
    public function save_custom_data($personid, $data)
    {
        if ($this->is_exists($personid))
        {
            foreach($data as $key => $fieldval)
            {
                $splittedkey = explode('__', $key);
                if ($splittedkey && count($splittedkey) > 1)
                {
                    $substorage = $splittedkey[0];
                    $fieldcode = $splittedkey[1];
                } else
                {
                    $fieldcode = $key;
                }
                $this->dof->storage('cov')->save_option(
                    $this->type(),
                    $this->code(),
                    $personid,
                    $fieldcode,
    ...
    {
        // Извлечение всех кастомных полей
        $customdata = $this->extract_custom_data($persondata);
        ...
        {// Обновление записи
            $personid = $normalized_data->id;
            $oldpersonwithcustoms = $this->get_person_with_customs($personid);
            $saveresult = $this->update($normalized_data, $personid, true);
        } else
        {// Создание записи
            $saveresult = $this->insert($normalized_data, true, false, $saveoptions);
            $personid = $saveresult;
        }
        ...
        {// Сохранение удалось
            ...
            $this->save_custom_data($personid, $customdata);
            
            if (empty($options['silent']))
            {// не требуется выполнять сохранение без генерации событий
                
                $newpersonwithcustoms = $this->get_person_with_customs($personid);
                
                // генерируем событие вставки/обновления вручную, чтобы они содержали кастомные поля
                $eventoptions = [
                    'old' => $oldpersonwithcustoms ?? null,
                    'new' => $newpersonwithcustoms,
                    'options' => $saveoptions
                ];
                $this->dof->send_event(
                    'storage',
                    'persons',
                    (isset($oldpersonwithcustoms) ? 'update' : 'insert'),
                    $personid,
                    $eventoptions
                );
                ...
                {// Требуется активация объекта
                    $this->dof->send_event('storage', 'persons', 'activate_request', (int)$personid);
                }
            }
        }
    ...
    {
        $this->prepare_update_object($dataobject);
    ...
    public function get_bu($userid = NULL,$create=false)
    {
        ...
        {
            $person = $this->get_record(array('mdluser'=>intval($userid)));
            if (!empty($person))
            {
   ...
   /** Получить список персон по запрашиваемой фамилии
    ...
    public function get_list_extendedsearch($conditions, $depid = 0, $limitfrom = 0, $limitnum = 0, $onlycount = false)
    {
        ...
        $sql = "FROM {$tblpersons} as per
                LEFT JOIN {$tblcontracts} as c
                    ON c.studentid = per.id
                LEFT JOIN {$tblpsbcs} as pbcs
                    ON pbcs.contractid = c.id
                WHERE " . implode(" AND ", $conds);
        $recordssql = "SELECT DISTINCT per.id, per.sortname " . $sql . " ORDER BY per.sortname ASC";
        $records = $this->get_records_sql($recordssql, null, $limitfrom, $limitnum);
        
        // Сразу же запрашиваем сколько было найдено строк до выполнения LIMIT
        $countsql = "SELECT COUNT(DISTINCT per.id) as \"count\" " . $sql;
        $foundrows = $this->get_records_sql($countsql);
        $countrows = 0;
    ...
    public function get_list_extended($personids, $params = null, $limitfrom = 0, $limitnum = 0)
    {
        ...
        $sql = sprintf("SELECT
                  CONCAT(
                    per.id,'_',
                    COALESCE(CAST(c.id AS CHAR(10)),'NA'),'_',
                    COALESCE(CAST(pbcs.id AS CHAR(10)),'NA'),'_',
                    COALESCE(CAST(p.id AS CHAR(10)),'NA')
                  ) as \"unique_id\",
     ...
     * @param bool $quiet - флаг оповещения о создании персоны
     * @param array $options опции (reset_password)
     *
     ...
     */
    public function reg_moodleuser($USER, $quiet = false, $options = [])
    {
        if (
                !is_object($USER)
                OR !isset($USER->id)
        ...
        return $this->insert($obj, $quiet, false, $options);
    }
    ...
    /**
     * Возвращает данные об имени персоны
     *
     * Данные об имени персоны включают в себя следующие поля
     *  array['fullname'] string|null - Полное имя персоны в установленном формате
     *  array['firstname'] string|null - Имя персоны
     *  array['lastname'] string|null - Фамилия персоны
     *  array['middlename'] string|null - Отчество персоны
     *
     * @param int|stdClass|null $id - ID персоны, либо объект персоны, либо NULL
     *
     * @return array - Данные об имени персоны
     */
    public function get_name_info($person = null)
    {
        // Получим персону
        ...
        {// Выполним действия в зависимости от типа
            ...
            case 'object' :
                if ( isset($person->firstname) AND isset($person->lastname) )
        ...
        {// Фамилия указана
            $personname['lastname'] = $person->lastname;
        }
        if ( !empty($person->firstname) )
        {// Имя указано
            $personname['firstname'] = $person->firstname;
        }
        if ( ! empty($person->middlename) )
        {// Отчество указано
            $personname['middlename'] = $person->middlename;
        }
        $personname['fullname'] = $this->get_fullname($person);
    ...
    /**
     * Проверка на владельца указанного объекта
     *
     * @param unknown $objectid
     * @param unknown $personid
     */
    public function is_owner($objectid, $personid)
    {
        // Проверка наличия объекта в БД
        $isexist = $this->is_exists($objectid);
        
        if ( $isexist )
        {// Персона существует
            if ( (int)$objectid === (int)$personid )
            {// Персона является владельцем для себя
                return true;
            }
        }
        return false;
    ...
    /**
     * Получить список персон, связанных с подразделением(-ями)
     * (имеющих подписку на программу или назначение на должность в подразделении(-ях))
     *
     * @param int|array $departmentids - идентификатор(-ы) подразделения(-ий)
     *
     * @return array - массив персон
     */
    public function get_persons_related_to_department($departmentids, $options=[])
    {
        $persondata = [];
        
        // Получение активных подписок на программы обучения
        $programmsbcstatuses = $this->dof->workflow('programmsbcs')->get_meta_list('active');
        $programmsbcs = $this->dof->storage('programmsbcs')->get_records([
            'departmentid' => $departmentids,
            'status' => array_keys($programmsbcstatuses)
        ]);
        if ( ! empty($programmsbcs) )
        {
            // Извлечение идентификаторов договоров
            $contractsprogrammsbcs = [];
            foreach($programmsbcs as $programmsbc)
            {
                if( ! array_key_exists($programmsbc->contractid, $contractsprogrammsbcs) )
                {
                    $contractsprogrammsbcs[$programmsbc->contractid] = [];
                }
                $contractsprogrammsbcs[$programmsbc->contractid][] = $programmsbc;
            }
            // Получение договоров в активном статусе, имеющих подписки
            $contractstatuses = $this->dof->workflow('contracts')->get_meta_list('active');
            $contracts = $this->dof->storage('contracts')->get_records([
                'id' => array_keys($contractsprogrammsbcs),
                'status' => array_keys($contractstatuses)
            ]);
            if( ! empty($contracts) )
            {
                // Извлечение учащихся из данных о договорах
                foreach($contracts as $contract)
                {
                    if( ! array_key_exists($contract->studentid, $persondata) )
                    {
                        $persondata[$contract->studentid] = [
                            'contracts' => [],
                            'programmsbcs' => [],
                            'appointments' => []
                        ];
                    }
                    if( ! array_key_exists($contract->id, $persondata[$contract->studentid]['contracts']) )
                    {
                        $persondata[$contract->studentid]['contracts'][$contract->id] = $contract;
                    }
                    foreach($contractsprogrammsbcs[$contract->id] as $psbc)
                    {
                        if( ! array_key_exists($psbc->id, $persondata[$contract->studentid]['programmsbcs']) )
                        {
                            $persondata[$contract->studentid]['programmsbcs'][$psbc->id] = $psbc;
                        }
                    }
                }
            }
        ...
        // Получение назначений на должность (табельных номеров) в подразделении в связке с договором и персоной
        $appointmentstatuses = $this->dof->workflow('appointments')->get_meta_list('active');
        $conds = new stdClass();
        $conds->departmentid = $departmentids;
        $conds->status = array_keys($appointmentstatuses);
        
        $appointments = $this->dof->storage('appointments')->get_teacher_list($conds);
        if ( ! empty($appointments) )
        {
            foreach($appointments as $appointment)
            {
                if( ! array_key_exists($appointment->personid, $persondata) )
                {
                    $persondata[$appointment->personid] =[
                        'contracts' => [],
                        'programmsbcs' => [],
                        'appointments' => []
                    ];
                }
                if( ! array_key_exists($appointment->id, $persondata[$appointment->personid]['appointments']) )
                {
                    $persondata[$appointment->personid]['appointments'][$appointment->id] = $appointment;
                }
            }
        }
        
        // Получение персон по собранным идентификаторам
        if( count($persondata) == 0 )
        {
            return [];
        } else
        {
            $personstatuses = $this->dof->workflow('persons')->get_meta_list('active');
            $persons = $this->get_records([
                'id' => array_keys($persondata),
                'status' => array_keys($personstatuses)
            ]);
            if( ! empty($options['detailed']) )
            {
                foreach($persons as $person)
                {
                    $persondata[$person->id]['data'] = $person;
                }
                return array_intersect_key($persondata, $persons);
            } else
            {
                return $persons;
            }
    ...
    protected function prepare_insert_object(&$dataobject)
    {
    ...
    protected function prepare_update_object(&$dataobject)
    {
        if (!isset($dataobject->sortname))
        {
    ...
    /**
     * Получить пользователей, имеющих достижения в портфолио
     * @param int $limitfrom с какой записи начать
     * @param int $limitnum количество записей, которые нужно вернуть
     * @param string $sort по каком полям сортировать
     * @param string $fields какие поля вернуть (у таблицы достижений префикс a, у таблицы пользователей префикс p)
     * @return array|false
     */
    public function get_persons_with_achievementins($limitfrom = null, $limitnum = null, $sort = '', $fields = '*') {
        if ((strpos($fields, 'p.id') === false || strpos($fields, 'p.*') === false) && $fields !== '*') {
            if (!empty($fields)) {
                $fields .= ', p.id';
            } else {
                $fields = '*';
        ...
        $ptable = $this->prefix() . $this->tablename();
        $atable = $this->prefix() . $this->dof->storage('achievementins')->tablename();
        $sql = "SELECT $fields 
                  FROM $ptable p
             LEFT JOIN $atable a
                    ON p.id = a.userid
              ...
              GROUP BY p.id" . 
              $this->get_orderby_listing($sort);
        return $this->get_records_sql($sql, [], $limitfrom, $limitnum);
    }
}
?>