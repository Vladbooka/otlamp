...
class dof_modlib_transmit_processor_validator_appointment_id_exists extends dof_modlib_transmit_processor_base
{
    /**
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        $exists = $dof->storage('appointments')->is_exists((int)$input['id']);
        if ( $exists )
        {
            return ['id' => $input['id']];
        }
