...
class dof_storage_contracts extends dof_storage implements dof_storage_config_interface
{
    ...
    public function upgrade($oldversion)
    {
        ...
        {
            // Проверка наличия поля ID организации
            $isexist = $dbman->field_exists($table, 'organizationid');
            if ( ! $isexist )
            {// Поле не найдено
                
                // Добавление поля в структуру БД
            ...
            // Проверка наличия поля ID куратора
            $isexist = $dbman->field_exists($table, 'curatorid');
            if ( ! $isexist )
            {// Поле не найдено
        ...
        return $result && $this->dof->storage('acl')->save_roles(
            $this->type(),
            $this->code(),
            $this->acldefault()
        );
    ...
    public function is_access($do, $objid = null, $userid = null, $depid = null)
    {
        ...
        switch ( $do )
        {
            // Право на просмотр договора
            case 'view':
                
                // Получение договора
                $contract = $this->get($objid);
                if ( ! empty($contract) )
                {// Договор указан
                    
                    // Дополнительные проверки на присутствие в договоре
                    if ( $this->is_seller($contract, $currentpersonid) )
                    {// Текущая персона является менеджером по договору
                        if ( $this->is_access('view/seller', $objid, $userid, $depid) )
                        {// Доступ на просмотр договора менеджером открыт
                            return true;
                        }
                    }
                    
                    if ( $this->is_client($contract, $currentpersonid) )
                    {// Текущая персона является законным представителем по договору
                        if ( $this->is_access('view/parent', $objid, $userid, $depid) )
                        {// Доступ на просмотр договора законным представителем открыт
                            return true;
                        }
                    }
                    
                    if ( $this->is_student($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('view/student', $objid, $userid, $depid) )
                        {// Доступ на просмотр договора студентом открыт
                            return true;
                        }
                    }
                    
                    if ( $this->is_curator($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('view/curator', $objid, $userid, $depid) )
                        {// Доступ на просмотр договора студентом открыт
                            return true;
                        }
                    }
            ...
            // Право на использование договора
            case 'use':
                ...
                {// Договор указан
                    ...
                    if ( $this->is_seller($contract, $currentpersonid) )
                    {// Текущая персона является менеджером по договору
                        if ( $this->is_access('use/seller', $objid, $userid, $depid) )
                        {// Доступ на использование договора менеджером открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_client($contract, $currentpersonid) )
                    {// Текущая персона является законным представителем по договору
                        if ( $this->is_access('use/parent', $objid, $userid, $depid) )
                        {// Доступ на использование договора законным представителем открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_student($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('use/student', $objid, $userid, $depid) )
                        {// Доступ на использование договора студентом открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_curator($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('use/curator', $objid, $userid, $depid) )
                        {// Доступ на использование договора студентом открыт
                            return true;
                        }
                    }
            ...
            case 'edit':
                ...
                {// Договор указан
                    ...
                    if ( $this->is_seller($contract, $currentpersonid) )
                    {// Текущая персона является менеджером по договору
                        if ( $this->is_access('edit/seller', $objid, $userid, $depid) )
                        {// Доступ на изменение договора менеджером открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_client($contract, $currentpersonid) )
                    {// Текущая персона является законным представителем по договору
                        if ( $this->is_access('edit/parent', $objid, $userid, $depid) )
                        {// Доступ на изменение договора законным представителем открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_student($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('edit/student', $objid, $userid, $depid) )
                        {// Доступ на изменение договора студентом открыт
                            return true;
                        }
                    }
                    ...
                    if ( $this->is_curator($contract, $currentpersonid) )
                    {// Текущая персона является студентом по договору
                        if ( $this->is_access('edit/curator', $objid, $userid, $depid) )
                        {// Доступ на изменение договора студентом открыт
                            return true;
                        }
                    }
            ...
            case 'view:billing/my':
                ...
                {// Договор указан
                    if ( $this->is_seller($contract, $currentpersonid) ||
                         $this->is_client($contract, $currentpersonid) ||
                         $this->is_student($contract, $currentpersonid) )
                    {
                        break;
                    }
                }
    ...
    public function acldefault()
    {
        ...
        $a['view/seller'] = [
            'roles' => [
                // По умолчанию любой работник, являющийся создателем договора, может проматривать его информацию
                'manager',
                'methodist',
                'teacher'
            ]
        ...
        $a['view/parent'] = [
            'roles' => [
                // По умолчанию любой, кто является законным представителем по договору - может просматривать его данные
                'manager',
                'methodist',
                'teacher',
                'parent',
        ...
        $a['view/student'] = [
            'roles' => [
                // По умолчанию недоступно ни одной из ролей
            ]
        ];
        ...
        // Право использования договора менеджером
        $a['use/seller'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право использования договора законным представителем
        $a['use/parent'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право использования договора студентом
        $a['use/student'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право использования договора куратором
        $a['use/curator'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право редактирования договора менеджером
        $a['edit/seller'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право редактирования договора законным представителем
        $a['edit/parent'] = [
            'roles' => [
                // По умолчанию недоступно ни одной из ролей
            ]
        ...
        // Право редактирования договора студентом
        $a['edit/student'] = [
            'roles' => [
                // По умолчанию недоступно ни одной из ролей
            ]
        ...
        // Право редактирования договора куратором
        $a['edit/curator'] = [
            'roles' => [
                // По умолчанию недоступно ни одной из ролей
            ]
        ...
        // @deprecated Право удаления договора
        $a['delete'] = [
            'roles' => [
                'manager'
            ]
        ...
        // Просматривать платежные данные по договору
        $a['view:billing'] = [
            'roles' => [
                'manager',
                'methodist'
        ...
        // Просматривать платежные данные по договору участникам
        $a['view:billing/my'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Пополнять счет договора
        $a['create:billinrefill'] = [
            'roles' => [
                'manager',
            ]
        ];
        
        // Списывать средства со счета договора
        $a['create:billinwriteof'] = [
            'roles' => [
                'manager',
            ]
        ];
    ...
    /**
     * Получить краткое название договора
     *
     * @param int|stdClass $contract - Договор или ID договора
     *
     * @return null|string - Краткое имя договора или null в случае ошибки
     */
    public function get_shortname($contract)
    {
        // Получение договора
        if ( ! is_object($contract) )
        {// Получение договора
            $contract = $this->get((int)$contract);
            if ( empty($contract) )
            {// Договор не найден
                return null;
            }
        ...
        // Получение краткого имени договора
        $stringvars = new stdClass();
        $stringvars->num = $contract->num;
        return $this->dof->get_string('contract_shortname', 'contracts', $stringvars, 'storage');
    }
    
    /**
     * Получить полное название договора
     *
     * @param int|stdClass $contract - Договор или ID договора
     *
     * @return null|string - Полное имя договора или null в случае ошибки
     */
    public function get_fullname($contract)
    {
        // Получение договора
        if ( ! is_object($contract) )
        {// Получение договора
        ...
        // Получение ременной зоны для текущего пользователя
        ...
        return $this->dof->get_string('contract_fullname', 'contracts', $stringvars, 'storage');
    }
    ...
    public function save($contractdata = null, $options = [])
    {
        ...
        {// Создание записи
            $contractid = $this->insert($normalized_data);
            if ( ! $contractid )
            {// Добавление не удалось
                throw new dof_exception_dml('error_save_contract');
            } else
            {// Добавление удалось
                $this->dof->send_event('storage', 'contracts', 'item_saved', (int)$contractid);
                if( ! empty($options['activate']) )
                {// Требуется активация объекта
                    $this->dof->send_event('storage', 'contracts', 'activate_request', (int)$contractid);
                }
                return $contractid;
            }
        }
    ...
    public function normalize($contractdata, $options = [])
    {
        ...
        {// Проверка на существование
            if ( ! $contract = $this->get($contractdata->id) )
            {// Договор не найден
        ...
        if ( ! empty($saveobj->id) && ! empty($saveobj->studentid) && !($contract->status == 'tmp') )
        {
            // проверим, что не просходит смены студенты по договору
            // запрещенная операция
            if ( $contract->studentid != $saveobj->studentid )
            {
                throw new dof_exception_dml('contract_cannot_change_studentid');
            }
        }
    ...
    public function import($data, &$report = null, $options = [])
    {
        ...
        if ( ! isset($options['activate']) || empty($options['activate']) )
        {// Запрос на активацию
        ...
        {// Данные определены
            ...
            {
                $statuses = ['new', 'clientsign', 'wesign', 'work', 'frozen', 'archives'];
            }
    ...
    /**
     * Получить список договоров, в которых закреплены указанные пользователи
     *
     * @param array|stdclass|int $ownerids - Массив персон или ID персоны
     * @param array $conditions - Дополнительные условия
     * @param array $options - Дополнительные опции работы
     *
     * @return
     */
    public function get_contracts_by_personids($ownerids, $conditions = [], $fields = '*', $options = [])
    {
        // Нормв=ализация списка персон
        if ( ! is_array($ownerids) )
        {
            if ( isset($ownerids->id) )
            {// Передан объект персоны
                $ownerids = (int)$ownerids->id;
            }
            // Массив персон
            $ownerids = [(int)$ownerids];
        }
        ...
        $select = ' sellerid IN ('.$ownerids.') OR
                    clientid IN ('.$ownerids.') OR
                    studentid IN ('.$ownerids.') OR
                    curatorid IN ('.$ownerids.') ';
        
        // Генерация дополнительных условий SQL запроса
        list($selectconditions, $params) = $this->where_clause((array)$conditions);
        
        if ( ! empty($selectconditions) )
        {
            $select = '( '.$select.' ) AND '.$selectconditions;
        }
        
        return $this->get_records_select($select, $params, '', $fields);
    }
    ...
    /**
     * Определение наличия указанной персоны в договоре с ролью менеджера
     *
     * @param int|stdClass|array $contract - ID договора или данные договора
     * @param int $personid - ID проверяемой персоны
     *
     * @return bool
     */
    public function is_seller($contract = null, $personid = null)
    {
        if ( is_null($personid) )
        {// Получение текущего пользователя
            $personid = (int)$this->dof->storage('persons')->get_by_moodleid_id();
        }
    
        if ( (int)$personid < 1 )
        {// Ошибка определения персоны
            return false;
        }
    
        // Определение в зависимости от переданных данных договора
        if ( is_null($contract) )
        {// Определение абстрактного признака менеджера
            return $this->is_exists(['sellerid' => $personid]);
        } elseif ( is_int($contract) || is_string($contract) )
        {// Указан идентификатор договора
            return $this->is_exists(['sellerid' => (int)$personid, 'id' => (int)$contract]);
        } elseif ( is_object($contract) && isset($contract->sellerid) )
        {// Указан объект договора
            return ( (int)$contract->sellerid === (int)$personid );
        } elseif ( is_array($contract) && isset($contract['sellerid']) )
        {// Указан массив данных договора
            return ( (int)$contract['sellerid'] === (int)$personid );
        }
        return false;
    }
    ...
    /**
     * Определение наличия указанной персоны в договоре с ролью студента
     *
     * @param int|stdClass|array $contract - ID договора или данные договора
     * @param int $personid - ID проверяемой персоны
     *
     * @return bool
     */
    public function is_client($contract = null, $personid = null)
    {
        if ( is_null($personid) )
        {// Получение текущего пользователя
            $personid = (int)$this->dof->storage('persons')->get_by_moodleid_id();
        ...
        // Определение в зависимости от переданных данных договора
        if ( is_null($contract) )
        {// Определение абстрактного признака студента
            return $this->is_exists(['clientid' => $personid]);
        } elseif ( is_int($contract) || is_string($contract) )
        {// Указан идентификатор договора
            return $this->is_exists(['clientid' => (int)$personid, 'id' => (int)$contract]);
        } elseif ( is_object($contract) && isset($contract->clientid) )
        {// Указан объект договора
            return ( (int)$contract->clientid === (int)$personid );
        } elseif ( is_array($contract) && isset($contract['clientid']) )
        {// Указан массив данных договора
            return ( (int)$contract['clientid'] === (int)$personid );
        }
        return false;
     ...
     * @return bool
     */
    public function is_student($contract = null, $personid = null)
    {
        if ( is_null($personid) )
        {// Получение текущего пользователя
            $personid = (int)$this->dof->storage('persons')->get_by_moodleid_id();
        ...
        if ( is_null($contract) )
        {// Определение абстрактного признака студента
            return $this->is_exists(['studentid' => $personid]);
        } elseif ( is_int($contract) || is_string($contract) )
        {// Указан идентификатор договора
            return $this->is_exists(['studentid' => (int)$personid, 'id' => (int)$contract]);
        } elseif ( is_object($contract) && isset($contract->studentid) )
        {// Указан объект договора
            return ( (int)$contract->studentid === (int)$personid );
        } elseif ( is_array($contract) && isset($contract['studentid']) )
        {// Указан массив данных договора
            return ( (int)$contract['studentid'] === (int)$personid );
        }
        return false;
    ...
    /**
     * Определение наличия указанной персоны в договоре с ролью куратора
     *
     * @param int|stdClass|array $contract - ID договора или данные договора
     * @param int $personid - ID проверяемой персоны
     *
     * @return bool
     */
    public function is_curator($contract = null, $personid = null)
    {
        if ( is_null($personid) )
        {// Получение текущего пользователя
            $personid = (int)$this->dof->storage('persons')->get_by_moodleid_id();
        ...
        // Определение в зависимости от переданных данных договора
        if ( is_null($contract) )
        {// Определение абстрактного признака куратора
            return $this->is_exists(['curatorid' => $personid]);
        } elseif ( is_int($contract) || is_string($contract) )
        {// Указан идентификатор договора
            return $this->is_exists(['curatorid' => (int)$personid, 'id' => (int)$contract]);
        } elseif ( is_object($contract) && isset($contract->curatorid) )
        {// Указан объект договора
            return ( (int)$contract->curatorid === (int)$personid );
        } elseif ( is_array($contract) && isset($contract['curatorid']) )
        {// Указан массив данных договора
            return ( (int)$contract['curatorid'] === (int)$personid );
        }
        return false;
    ...
    public function get_listing($conds = null, $limitfrom = null, $limitnum = null, $sort='', $fields='*', $countonly=false)
    {
        ...
        $sql = "FROM {$tblcontracts} as c
           LEFT JOIN {$tblpersons} as pr ON c.studentid=pr.id
               WHERE {$select} {$addwhere}";
        if ( $countonly )
    ...
    {
        $issortname = strripos($sort, 'sortname');
        if ( empty($sort) OR is_int($issortname) )
        {
            return " ORDER BY pr.{$sort}";
        }
    ...
    {
        // Владельцем договора является менеджер
        return $this->is_seller($objectid, $personid);
    }
}
