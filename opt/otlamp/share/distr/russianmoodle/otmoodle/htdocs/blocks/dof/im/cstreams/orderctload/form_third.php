...
/**
 * Интерфейс для приказа "Передача нагрузки преподавателя" (третья страница)
 * 
 * Формируется в три экрана: на этом можно отредактировать выбранные на прошлом
 * экране фильтры для выбора преподавателей
 */
...
if ( !empty($orderid) )
{
    ...
    $orderform = new dof_im_cstreams_order_change_teacher_page_three($DOF->url_im('cstreams', 
                    '/orderctload/form_third.php?edit='.$orderid.'&departmentid='.$departmentid,$addvars), $customdata);
} else
{
    $DOF->modlib('nvg')->add_level($DOF->get_string('title', 'cstreams'), 
                         $DOF->url_im('cstreams','/index.php'),$addvars);
...
$DOF->modlib('nvg')->add_level($DOF->get_string('order_form', 'cstreams'), 
                     $DOF->url_im('cstreams','/orderctload/form_third.php?edit='.$orderid),$addvars);
// Печать шапки страницы
$DOF->modlib('nvg')->print_header(NVG_MODE_PORTAL);
