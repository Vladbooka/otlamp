...
class dof_im_schedule_display
{
    ...
    private $rowclasses; // css-класс для отрисовки одной строки таблицы
    ...
    private function get_string_full($obj)
    {
        ...
        $daynum  = $this->get_template_week_days();
   ...
   /** Возвращает html-код таблицы
    ...
    private function get_header($type)
    {
        switch ( $type )
		{
            ...
            // под персону
            case 'person':
            return array($this->dof->get_string('lastname', $this->get_im()),//$this->get_im()),
            ...
            case 'place':
                return [
                    $this->dof->modlib('ig')->igs('actions'),
                    $this->dof->get_string('daynum', $this->get_im(),'<br>'),
                    $this->dof->get_string('time_lesson', $this->get_im(),'<br>'),
                    $this->dof->get_string('dayvar', $this->get_im(),'<br>'),
                    $this->dof->get_string('item', $this->get_im()),
                    $this->dof->get_string('teacher', $this->get_im()),
    ...
    protected function get_week_tabs($urloptions)
    {
        ...
        $days = $this->get_template_week_days();
    ...
    public function print_full_schedule($ageid, $displaytype, $daynum, $dayvar, $time=null, $form = 'all')
    {
        ...
        switch ( $displaytype )
        {
		    ...
		    case 'teachers': // по учителю';
                ...
                $string .= $this->dof->modlib('widgets')->print_table($table1,true).$table; 
		    break;
		    case 'place': // по номеру кабинета
		        $this->data = array();
		        $placestemplates = $this->compose_schedule_by_place($ageid);
		        
                if ( ! empty($placestemplates) ) 
                {// нашли данные
                    foreach ( $placestemplates as $place=>$templates)
                    {// формируем из них таблицы
                        ksort($templates);
                        ...
                        if ( $place == '' )
                        {
                            $place = $this->dof->get_string('empty_place', $this->get_im());
                        }
                        $a->label = $this->dof->get_string('place', $this->get_im())." ".$place;
                        $a->records = $templates;
                        $string .= $this->print_schedule_part($displaytype, $a);
                    }
                }else
                {// ничего не нашли - выведем сообщение
                    $this->dof->messages->add(
                        $this->dof->get_string('places_templates_not_found', $this->get_im()),
                        'message'
                    );
    ...
    protected function print_schedule_part($displaytype, $part, $dayvar=0)
    {
        ...
        switch ( $displaytype )
        {
		    ...
		    case 'place':// по номеру кабинета
		        ...
		        foreach ( $part->records as $template )
                {//для каждого шаблона формируем строку
                    $this->data[] = $this->get_string_place($template);
                }
                return $header.$this->print_table('place');
		    break;
    ...
    {
        return $this->dof->storage('ages')->get_list_daynums($this->ageid);
    }
    
    

    /** 
     * Сформировать массив шаблонов, сгруппированных по кабинетам.
     *
     * @param int $ageid - id учебного периода для которого отображается расписание
     * @return array массив объектов
     */
    protected function compose_schedule_by_place($ageid)
    {
        // условия выборки
        $conds = new stdClass();
        $conds->departmentid = $this->departmentid;
        ...
        $conds->cstreamsstatus = array('plan','active','suspend');
        $conds->status = array('active','suspend');
        // получение шаблонов по условиям
        $templates = $this->dof->storage('schtemplates')->get_objects_list(
            $conds,
            'place ASC'
        );
        $templateweekdays = $this->get_template_week_days();

        // формирование массива сгруппированных шаблонов
        $places = [];
        if ( ! empty($templates) )
        {
            foreach ( $templates as $template )
            {
                $complexkey = $templateweekdays[$template->daynum].'_'.$template->begin.'_'.$template->dayvar.'_'.$template->id;
                $places[$template->place][$complexkey] = $template;
            }
        }
        return $places;
    }
    ...
    /** 
     * Получает строку для отображения по номеру кабинета
     * 
     * @param object $obj - объект шаблона из таблицы schtemplates
     * @return array
     */
    private function get_string_place($obj)
    {
        $data = [];
        ...
        $params = $this->addvars + $outparams;
        
        // действия
        $actions = [];
        // действие - редактирование шаблона
        if ( $this->dof->storage('schtemplates')->is_access('edit', $obj->id) )
        {// пользователь может редактировать шаблон

            $imgtext = $this->dof->get_string('edit_template', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/edit.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $edittemplateparams = $params;
            $edittemplateparams['id'] = $obj->id;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im($this->get_im(), '/edit.php', $edittemplateparams),
                $img,
                ['id' => 'edit_schtemplate_'.$obj->id]
            );
        }
        // действие - просмотр шаблона
        if ( $this->dof->storage('schtemplates')->is_access('view', $obj->id) )
        {// пользователь может просматривать шаблон
            $imgtext = $this->dof->get_string('view_template', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/view.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $viewtemplateparams = $params;
            $viewtemplateparams['id'] = $obj->id;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im($this->get_im(), '/view.php', $viewtemplateparams),
                $img,
                ['id' => 'view_schtemplate_'.$obj->id]
            );
        }
        // действие - активировать шаблон
        if ( $this->dof->workflow('schtemplates')->is_access('changestatus')
             AND $obj->status == 'suspend' )
        {
            $imgtext = $this->dof->get_string('active_template', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/state.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $activatetemplateparams = $params;
            $activatetemplateparams['id'] = $obj->id;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im($this->get_im(), '/active.php', $activatetemplateparams),
                $img,
                ['id' => 'activate_schtemplate_'.$obj->id]
            );
        }
        // действие - приостанавливать шаблон
        if ( $this->dof->workflow('schtemplates')->is_access('changestatus')
            AND $obj->status == 'active' )
        {
            $imgtext = $this->dof->get_string('suspend_template', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/suspend.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $suspendtemplateparams = $params;
            $suspendtemplateparams['id'] = $obj->id;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im($this->get_im(), '/suspend.php', $suspendtemplateparams),
                $img,
                ['id' => 'suspend_schtemplate_'.$obj->id]
            );
        }
        // действие - удалять шаблон
        if ( $this->dof->workflow('schtemplates')->is_access('changestatus') )
        {
            $imgtext = $this->dof->get_string('delete_template', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/delete.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $deletetemplateparams = $params;
            $deletetemplateparams['id'] = $obj->id;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im($this->get_im(), '/delete.php', $deletetemplateparams),
                $img,
                ['id' => 'delete_schtemplate_'.$obj->id]
            );
        }
        // действие - ссылка на предмето-класс
        if ( $this->dof->storage('cstreams')->is_access('view', $obj->cstreamid) )
        {// пользователь может просматривать предмето-класс
            $imgtext = $this->dof->get_string('view_cstream', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/cstreams.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $cstreamlinkparams = $outparams;
            $cstreamlinkparams['cstreamid'] = $obj->cstreamid;
            
            $actions[] = dof_html_writer::link(
                $this->dof->url_im('cstreams','/view.php', $cstreamlinkparams),
                $img,
                ['id' => 'view_cstream_'.$obj->cstreamid.'_for_schtemplate_'.$obj->id]
            );
        }
        $data[] = implode(' ',$actions);
        
        // день недели
        $daynums  = $this->get_template_week_days();
        $data[] = $daynums[$obj->daynum];
        
        // время
        $hours = floor($obj->begin / 3600);
        $minutes = floor(($obj->begin - $hours * 3600) / 60);
        $timestart = $this->dof->storage('persons')->get_userdate(
            mktime($hours, $minutes),
            "%H:%M"
        );
        $hours = floor(($obj->begin + $obj->duration) / 3600);
        $minutes = floor(($obj->begin + $obj->duration - $hours * 3600) / 60);
        $timestop = $this->dof->storage('persons')->get_userdate(
            mktime($hours, $minutes),
            "%H:%M"
        );
        $data[] = $timestart.' - '.$timestop;
        
        // вариант недели
        $dayvars  = $this->dof->modlib('refbook')->get_day_vars();
        $data[] = $dayvars[$obj->dayvar];
        ...
        {// ссылка на просмотр предмета
            ...
            $pitemlinkparams = $outparams;
            $pitemlinkparams['pitemid'] = $pitemid;
            $pitemlink = html_writer::link(
                $this->dof->url_im('programmitems','/view.php', $pitemlinkparams),
                $pitem->name.' <br>['.$pitem->code.']'
            );
        }
        $data[] = $pitemlink;
        ...
        // преподаватель
        $teacherpersonid = 0;
        $teacherpersonfullname = $this->dof->get_string('cstreams_no_teacher', 'schedule');
        $appointmentid = $this->dof->storage('cstreams')->get_field($obj->cstreamid, 'appointmentid');
        $teachereventslink = '';
        $teachertemplateslink = '';
        if ( ! empty($appointmentid) )
        {// преподаватель для учебного процесса указан
            $teacherperson = $this->dof->storage('appointments')->get_person_by_appointment($appointmentid);
            $teacherpersonid = $teacherperson->id;
            $teacherpersonfullname = $this->dof->storage('persons')->get_fullname($teacherpersonid);
            
            if ( $this->dof->im('journal')->is_access('view_schevents'))
            {// пользователь может просматривать события
                $imgtext = $this->dof->get_string('view_events_teacher', $this->get_im());
                $img = dof_html_writer::img(
                    $this->dof->url_im($this->get_im(), '/icons/view_events.png'), 
                    $imgtext,
                    ['title' => $imgtext]
                );
                $teachereventslinkparams = $params;
                $teachereventslinkparams['personid'] = $teacherpersonid;
                $teachereventslinkparams['date_from'] = $teachereventslinkparams['date_to'] = time();
                
                $teachereventslink = dof_html_writer::link(
                    $this->dof->url_im(
                        'journal', 
        ...
        if ( $this->dof->storage('schtemplates')->is_access('view'))
        {// пользователь может просматривать шаблоны
            $imgtext = $this->dof->get_string('view_week_template_on_teacher', $this->get_im());
            $img = dof_html_writer::img(
                $this->dof->url_im($this->get_im(), '/icons/show_schedule_week.png'),
                $imgtext,
                ['title' => $imgtext]
            );
            $teachertemplateslinkparams = $params;
            unset($teachertemplateslinkparams['cstreamid']);
            unset($teachertemplateslinkparams['teacherid']);
            unset($teachertemplateslinkparams['studentid']);
            unset($teachertemplateslinkparams['agroupid']);
            $teachertemplateslinkparams['teacherid'] = $teacherpersonid;
            
            $teachertemplateslink = dof_html_writer::link(
                $this->dof->url_im(
                    $this->get_im(),
                    '/view_week.php', 
                    $teachertemplateslinkparams
                ),
                $img,
                ['id' => 'view_schtemplate_vacancy_week_'.$obj->id]
            );
        }
        $data[] =   dof_html_writer::div($teacherpersonfullname) .
                    dof_html_writer::div($teachertemplateslink . $teachereventslink);
        
        
        // учащиеся
        $data[] = $this->get_list_students($obj->cstreamid);
        ...
        // тип недели
        $type    = $this->dof->modlib('refbook')->get_day_vars();
        $data[] = $type[$obj->dayvar];
    
        // подразделение
        $department = $this->dof->storage('departments')->get($obj->departmentid);
        $data[] = $department->name.' <br>['.$department->code.']';
        
        // статус
        $data[] = $this->dof->workflow('schtemplates')->get_name($obj->status);
...
class dof_im_schedule_master_make
{
   ...
   /** Возвращает html-код таблицы
    ...
    private function get_string_intersection($obj)
    {
        ...
        // день недели
        $week_days = $this->dof->storage('ages')->get_list_daynums($this->ageid);
    ...
    /** Получить информацию о пересечении с другими шаблонами(кабинеты)
     * @param int $templateid - id шаблона из таблицы schtemplates
     * @return string
     */
    public function cross_with_others_templates_place($templateid)
    {
        // проверка на существование шаблона
        if ( ! $template = $this->dof->storage('schtemplates')->get($templateid) )
        {
        ...
        return $this->dof->storage('schtemplates')->get_select_templater_places($template);
    }
    ...
    public function show_cross_templates($id=0)
    {
        ...
        foreach ( $templates as $template )
        {
    		...
    		// Таблица пересечений по кабинетам
    		if ( $place = $this->cross_with_others_templates_place($template->id) )
    		{
    		    $rez .= $this->header_name_table('intersection_place');
    		    $this->data = [];
    		    foreach ( $place as $tc )
    		    {
    		        $this->data[] = $this->get_string_intersection($tc);
    ...
    {            
        ...
        {// выделяем просматривамый шаблон
            return $this->dof->modlib('widgets')->
                info_message($this->dof->get_string($name, $this->get_im()));
        }else 
        {
            return $this->dof->modlib('widgets')->
