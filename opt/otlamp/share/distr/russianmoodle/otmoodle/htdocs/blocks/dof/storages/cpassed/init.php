...
/**
 * Справочик подписок на дисциплины
 *
 * @package    storage
 * @subpackage cpassed
 *
...
class dof_storage_cpassed extends dof_storage implements dof_storage_config_interface
{
    ...
    public function list_catch_events()
    {
        return [
            ...
            ['plugintype'=>'storage', 'plugincode'=>'programmsbcs', 'eventcode'=>'changegroup'],
            ['plugintype'=>'storage', 'plugincode'=>'cstreamlinks', 'eventcode'=>'insert'],
            ['plugintype'=>'storage', 'plugincode'=>'cstreamlinks', 'eventcode'=>'delete'],
            // Запросы на удаление информации о персоне
            ['plugintype'=>'im', 'plugincode'=>'employees', 'eventcode'=>'delete_person_info'],
            ['plugintype'=>'im', 'plugincode'=>'employees', 'eventcode'=>'delete_person'],
        ];
    }
    ...
    public function is_access($do, $objid = NULL, $userid = NULL, $depid = null)
    {
        ...
        $personid = (int)$this->dof->storage('persons')->get_by_moodleid_id($userid);
        ...
        {// определяем дополнительные параметры в зависимости от запрашиваемого права
            ...
            case 'edit:grade/auto':
                ...
                break;
            // Видеть краткую информацию о подписке на дисциплину
            case 'viewdesk' :
                
                // Получение данных по подписке на дисциплину
                $teacherid = (int)$this->get_field($objid, 'teacherid');
                if ( $teacherid > 0 && $personid === $teacherid )
                {// Текущая персона преподает для указанной подписки
                    if ( $this->is_access('viewdesk/teacher') )
                    {// Преподаватель имеет доступ к просмотру краткой информации
                        return true;
                    }
                }
                
                $studentid = (int)$this->get_field($objid, 'studentid');
                if ( $studentid > 0 && $personid === $studentid )
                {// Текущая персона учится под указанной подпиской
                    if ( $this->is_access('viewdesk/student') )
                    {// Ученик имеет доступ к просмотру краткой информации по своей подписке
                        return true;
                    }
                }
                break;
    ...
    public function catch_event($gentype,$gencode,$eventcode,$intvar,$mixedvar)
    {
        ...
        {// Обновить кэшируемые поля справочника
            $cpasseds = $this->get_records(['cstreamid' => $intvar]);
            if ( $cpasseds )
            {
                $this->update_cached_fields($cpasseds);
            }
        }
    ...
    public function todo($code,$intvar,$mixedvar)
    {
        switch ($code)
        {
            ...
            case 'comparecpassed':
                return $this->compare_cpassed_with_cstream();
                break;
            // приостановить, а затем запустить все записи об изучаемых
            // и пройденных курсах у всех учеников в указанном периоде
            case 'resync_age_cpassed':
                return $this->todo_resync_age_cpassed($intvar,$mixedvar->personid) ;
                break;
            case 'resync_cancaled_active_cpassed':
                $this->todo_cancaled_active_cpassed();
                break;
            // запуск всез приостановленных
            case 'suspend_to_active_cpassed':
                return $this->todo_suspend_to_active($intvar,$mixedvar->personid);
                break;
            // остановка всех активных периода
            case 'active_to_suspend_cpassed':
                return $this->todo_active_to_suspend($intvar,$mixedvar->personid);
                break;
            // Разрывает связку отмененной подписки с пересдачамии
            case 'cancaled_repeatid_to_null':
                return $this->todo_cancaled_active_cpassed();
                break;
            // Обновляет learninghistoryid, связывая их с записями в таблице learninghistory
            case 'fill_learninghistoryid':
                return $this->todo_fill_learninghistoryid();
                break;
            // был ряд кейсов (ERM #453), при которых дата начала у подписок не выставлялась, todo фиксит эти подписки
            case 'fix_begindate':
                return $this->todo_fix_begindate();
                break;
            // Синхронизирует даты подписок на учебные процессы с периодом
            case 'resync_cpassed_duration_with_age':
                return $this->todo_resync_cpassed_duration($intvar);
                break;
    ...
    public function acldefault()
    {
        ...
        // Видеть полные данные подписки на дисциплину
        $a['view'] = [
            'roles' => [
                'manager',
                'methodist'
        ...
        // Видеть полные данные своей подписки на дисциплину
        $a['view/student'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Видеть полные данные подписки на дисциплину своего ученика
        $a['view/teacher'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Видеть краткие данные подписки на дисциплину
        $a['viewdesk'] = [
            'roles' => [
                'manager',
                'methodist'
        ...
        // Видеть краткие данные своей подписки на дисциплину
        $a['viewdesk/student'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Видеть краткие данные подписки на дисциплину своего ученика
        $a['viewdesk/teacher'] = [
            'roles' => [
                'manager',
                'methodist',
        ...
        // Право редактирования даты начала подписки
        $a['edit:begindate'] = [
            'roles' => [
                'manager'
            ]
        ...
        // Право редактирования даты конца подписки
        $a['edit:enddate'] = [
            'roles' => [
                'manager'
            ]
    ...
    public function config_default($code=null)
    {
        ...
        // Предоставлять возможность смены режима импорта подписок на учебные процессы
        $obj = new stdClass();
        $obj->type = 'checkbox';
        $obj->code = 'cpassed_import_copy_mode_allowed';
        $obj->value = '1';
        $config[$obj->code] = $obj;
    ...
    /**
     * Сохранить подписку на дисциплину в системе
     *
     * @param stdClass|array $data - Данные подписки на дисциплину(комплексные данные)
     * @param array $options - Массив дополнительных параметров
     *
     * @return bool|int - false в случае ошибки или ID подписки в случае успеха
     *
    ...
    public function save($data = null, $options = [])
    {
        ...
        {// Обновление записи
            $cpassed = $this->update($normalized_data);
            if ( empty($cpassed) )
            {// Обновление не удалось
                throw new dof_exception_dml('error_save_cpassed');
            } else
            {// Обновление удалось
                $this->dof->send_event('storage', 'cpassed', 'item_saved', (int)$normalized_data->id);
                
                // Обновление кэшируемых полей
                $this->update_cached_fields((int)$normalized_data->id);
                
                return $normalized_data->id;
            }
        } else
        {// Создание записи
            $cpassedid = $this->insert($normalized_data);
            if ( ! $cpassedid )
            {// Добавление не удалось
                throw new dof_exception_dml('error_save_contract');
            } else
            {// Добавление удалось
                $this->dof->send_event('storage', 'cpassed', 'item_saved', (int)$cpassedid);
                
                // Обновление кэшируемых полей
                $this->update_cached_fields((int)$cpassedid);
...
//        if ( ! $cstreamlist = $this->dof->storage('cstreams')->get_list() )
     ...
     */
    public function sign_student_on_cstream($cstreamid, $programmsbcid, $sbcorderid = null, $status = null)
    {
        ...
        {// Если плавающие учебные планы отключены,
         ...
         //  и параллель подписки не совпадает с параллелью предмета
     ...
     */
    public function move_cpassed($cpassedid, $destinationcstreamid, $options=[])
    {
        // Массив ошибок
        $errors = [];
        ...
        {// Исходные данные переданы
            ...
            {// Ошибки не найдены
                ...
                $setting = (int)$this->dof->storage('config')->get_config_value(
                    ...
                    'storage',
                    'cpassed',
                    $destinationcstream->departmentid
                );
                if ( ! empty($options['mode']) && $options['mode'] == 'copy' && $setting )
                {// Режим копирования
                    $copymode = true;
                }
                ...
                if ( ! $copymode )
                {//не включен режим копирования - значит старую подписку на дисциплину надо перевести в конечный статус
    ...
    public function sign_agroup_on_cstream($agroupid, $cstreamid, $programmsbc = null)
    {
        ...
        // найдем группу, чтоб узнать в какой параллели она учится
    ...
    private function unsign_cpassed_from_cstream($cstreamid, $agroupid, $programmsbc = null )
     {
        ...
        foreach ($cpassedlist as $one)
        {
           // Отписываем тех, у кого группа не совпадает с переданной
    ...
    public function set_final_grade($cpassedid, $grade = null, $orderid = null)
    {
        ...
        $pitem = $this->dof->storage('programmitems')->get_record(['id' => $cpassed->programmitemid]);
        if ( ! $this->dof->modlib('journal')->get_manager('scale')->is_grade_valid($grade, $pitem->scale) )
        ...
        {// подписка пересдана или завершена
            ...
            if ( ! $cpassedid = $this->save($new_cpass) )
        ...
        $obj->grade = $grade;
        $obj->orderid = $orderid;
        $orderobj = $this->dof->im('journal')->order('set_itog_grade');
        if( ! empty($orderobj) )
        {
            if( $order = $orderobj->load($orderid) )
            {
                $obj->teacherid = $order->data->teacherid;
            }
        }
     ...
     * или false
     */
    public function get_cpassed_on_programmsbcid_new($id, $status = 'active')
    {
        if ( ! is_int_string($id) )
        {//входные данные неверного формата
            return false;
     ...
     * @return bool
     */
    public function todo_fix_begindate()
    {
        dof_hugeprocess();
        
        // статусы, которые гарантировано должны иметь дату начала подписки
        $statuses = ['active', 'suspend', 'completed', 'reoffset', 'failed', 'academicdebt'];
        
        // массив учебных процессов
        $cstreams = [];
        
        // объект для обновления подписки
        $updateobj = new stdClass();
        
        $cpasseds = $this->get_records(['status' => $statuses, 'begindate' => NULL]);
        if (!empty($cpasseds))
        {
            // чиним подписки
            foreach ( $cpasseds as $cpassed )
            {
                if ( $cpassed->status == 'failed' && empty($cpassed->enddate) )
                {
                    // неуспешно-завершенная подписка с отсутствующей датой окончания
                    continue;
                }
                if ( !array_key_exists($cpassed->cstreamid, $cstreams) )
                {
                    $cstream = $this->dof->storage('cstreams')->get_record(['id' => $cpassed->cstreamid]);
                    if ( empty($cstream) )
                    {
                        continue;
                    }
                    $cstreams[$cpassed->cstreamid] = $cstream;
                }
                if ( empty($cstream->begindate) )
                {
                    continue;
                }
                
                $updateobj->id = $cpassed->id;
                $updateobj->begindate = $cstream->begindate + 1;
                
                // тихо обновляем дату начала
                $this->update($updateobj, null, true);
            }
        }
     ...
     *             string|int|array ['cstreamids'] - Массив идентификаторов учебных процессов или единичный идентификатор
     ...
     *             string ['timestart'] - Начальная граница временного интервала
     *             string ['timeend'] - Конечная граница временного интервала
     *
     *
    ...
    public function get_cpasseds_by_options($options)
    {
        ...
        // Фильтрация по учебному процессу
        if ( isset($options['cstreamids']) && ! empty($options['cstreamids']) )
        {// Указаны идентификаторы
            if ( is_string($options['cstreamids']) || is_int($options['cstreamids']) )
            {// Нормализация до массива
                $singleid = (int)$options['cstreamids'];
                $options['cstreamids'] = [$singleid];
            }
            if ( is_array($options['cstreamids']) )
            {// Получен верный тип данных
                if ( ! empty($where) )
                {// Разделитель
                    $where .= ' AND ';
                }
                $ids = "'".implode("','", $options['cstreamids'])."'";
                $where .= ' cstreamid IN ('.$ids.') ';
            }
        }
    ...
    /**
     * Получить список кэшируемых полей хранилища
     *
     * @return array
     */
    ...
    /**
     * Обновление кэшируемых полей объектов хранилища
     *
     * @param int, stdClass, array[], int[] $item - Данные о записях
     *      (ID записи, или Запись, или массив идентификаторов записей, или массив записей)
     */
    public function update_cached_fields($item)
    {
        ...
        {// Передан массив элементов
            foreach ( $item as $element )
            {
                ...
                {
                    $cpasseds[$element->id] = $element;
                } else
                {
                    $cpassed = $this->get((int)$element);
                    $cpasseds[$cpassed->id] = $cpassed;
                }
            }
            unset($item);
        } else
        {// Единичный элемент
            if ( ! is_object($item) )
            {// Нормализация до объекта
                $item = $this->get((int)$item);
            }
            if ( empty($item) )
            {
                return false;
            }
            $cpasseds[$item->id] = $item;
        }
        
        // Список кэшируемых полей
        $cachedfields = $this->get_cached_fields();
        
        // Обновление кэшируемых полей
        foreach ($cpasseds as $id => $cpassed )
        {
            // Проверка кэша данных учебного процесса
            if ( ! isset($cachedfields['ageid'][(int)$cpassed->cstreamid]) )
            {// Добавление в кэш данных об учебном процессе
                $age = $this->dof->storage('cstreams')->get_field(['id' => $cpassed->cstreamid], 'ageid');
                $cachedfields['ageid'][(int)$cpassed->cstreamid] = (int)$age;
            }
            if ( ! isset($cachedfields['teacherid'][(int)$cpassed->cstreamid]) )
            {// Добавление в кэш данных о преподавателе
                $teacherid = $this->dof->storage('cstreams')->get_field(['id' => $cpassed->cstreamid], 'teacherid');
                $cachedfields['teacherid'][(int)$cpassed->cstreamid] = (int)$teacherid;
            }
            
            // Обновление кэшируемых полей подписки на дисциплину
            $update = new stdClass();
            $update->id = $id;
            $needupdate = false;
            
            // Обновление периода
            if ( $cachedfields['ageid'][(int)$cpassed->cstreamid] )
            {
                $update->ageid = $cachedfields['ageid'][(int)$cpassed->cstreamid];
                $needupdate = true;
            }
            
            // Обновление преподавателя
            if ( $cachedfields['teacherid'][(int)$cpassed->cstreamid] )
            {
                $update->teacherid = $cachedfields['teacherid'][(int)$cpassed->cstreamid];
                $needupdate = true;
            }
    ...
    /**
     * Сужает рамки подписок на учебные процессы до дат, указанных в периоде
     * @param int $ageid идентификатор периода
     */
    protected function todo_resync_cpassed_duration($ageid)
    {
        $result = true;
        if( empty($ageid) )
        {
        ...
        if( ! empty($age) )
        {
            $sql = 'SELECT cp.*, cs.begindate as csbegindate, cs.enddate as csenddate
                      FROM {block_dof_s_cpassed} cp
                      JOIN {block_dof_s_cstreams} cs
                        ON cp.cstreamid=cs.id
                     WHERE cp.ageid = :ageid AND (cp.begindate < :begindate OR (cp.enddate IS NOT NULL AND cp.enddate > :enddate))';
            $params = [
                'ageid' => $age->id,
                'begindate' => $age->begindate,
                'enddate' => $age->enddate
            ];
            // Получаем все подписки на учебные процессы, выходящие за рамки периода
            $cpasseds = $this->dof->storage('cpassed')->get_records_sql($sql, $params);
            if( ! empty($cpasseds) )
            {
                // Получение очереди логов
                $logger = $this->dof->modlib('logs')->
                    create_queue('storage', 'cpassed', 'resync_duration', $ageid, 'csv');
                foreach($cpasseds as $cpassed)
                {
                    if( $cpassed->begindate < $age->begindate &&
                        $cpassed->csbegindate >= $age->begindate &&
                        $cpassed->csbegindate <= $age->enddate )
                    {// Если начало подписки за рамками периода и начало процесса внутри периода - ставим начало процесса
                        $cpassed->begindate = $cpassed->csbegindate;
                    } elseif( $cpassed->begindate < $age->begindate && $cpassed->csbegindate < $age->begindate )
                    {// Если и начало подписки и начало процесса за рамками периода - ставим начало периода
                        $cpassed->begindate = $age->begindate;
                    }
                    if( $cpassed->enddate > $age->enddate &&
                        $cpassed->csenddate <= $age->enddate &&
                        $cpassed->csenddate >= $age->begindate )
                    {//Аналогично с концом подписки
                        $cpassed->enddate = $cpassed->csenddate;
                    } elseif( $cpassed->enddate > $age->begindate && $cpassed->csenddate > $age->enddate )
                    {
                        $cpassed->enddate = $age->enddate;
                    }
                    // Обновляем даты
                    if( $update = $this->dof->storage('cpassed')->update($cpassed) )
                    {
                        $action = 'success';
                    } else
                    {
                    ...
                    $message = $this->dof->get_string($action . '_resync_cpassed_duration', 'cpassed', $cpassed, 'storage');
                    // Пишем лог
                    $logger->addlog(
                        null,
                        'update',
    ...
    /**
     * Поиск активных подписок, связанных с курсом
     * @param int $mdlcourseid идентификатор курса
     * @param int $personid идентификатор персоны
     * @return boolean true, если есть активные подписки, false - если нет
     */
    public function search_active_cpasseds($mdlcourseid, $personid)
    {
        // Поиск по привязке курса к учебному процессу
        $sql = 'SELECT cp.*
	              FROM {block_dof_s_cpassed} cp
	              JOIN {block_dof_s_cstreams} cs
	                ON cp.cstreamid=cs.id
	             WHERE cs.mdlcourse = :mdlcourse AND cp.status = :status AND cp.studentid = :studentid';
        $params = [
            'mdlcourse' => $mdlcourseid,
            'status' => 'active',
            'studentid' => $personid
        ];
        if( ! empty($this->get_records_sql($sql, $params)) )
        {// Есть активные подписки связанные с курсом
            return true;
        } else
        {// Поиск по привязке курса к дисциплине
            $sql = 'SELECT cp.*
	                  FROM {block_dof_s_cpassed} cp
	                  JOIN {block_dof_s_programmitems} pi
	                    ON cp.programmitemid=pi.id
	                 WHERE pi.mdlcourse = :mdlcourse AND cp.status = :status AND cp.studentid = :studentid';
            $params = [
                'mdlcourse' => $mdlcourseid,
                'status' => 'active',
                'studentid' => $personid
