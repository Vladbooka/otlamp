...
/**
 * Интерфейс управления подразделениями. Базовые функции плагина
 *
 * @package    im
...
/**
 * Класс, формы для смены принадлежности объектов к подразделению.
 *
 */
class dof_im_departments_change_department
{
    ...
    public function get_form($check='')
    {
        ...
        $html .= '<div class="felement fselect"><select name="'.$this->options['prefix'].'_'.$this->options['selectname'].'" class="custom-select" >'."\n";
        ...
        switch ($check)
        {
            ...
            case 'contract_employees':
                $html .= '<div class="fitemtitle"> </div>';
                $html .= '<div class="felement"><span><input type="checkbox" name="appointment" value="1" class="mr-2" checked>';
                $html .= $this->dof->get_string('and_appointment','employees').'</span></div>';
                $html .= '<br>';
                $html .= '<div class="fitemtitle"> </div>';
                $html .= '<div class="felement"><span><input type="checkbox" name="employees" value="1" class="mr-2" checked>';
        ...
        $html .= '<div class="felement fsubmit"><input type="submit" class="btn btn-primary" name="'.$this->options['prefix'].'_'.$this->options['submitname'].'" value="'.$this->options['submittitle'].'"/></div></fieldset></div>';
        // возвращаем добро
        return $html;
    }
