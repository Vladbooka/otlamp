...
class dof_storage_ages extends dof_storage implements dof_storage_config_interface
{
     ...
     * Сохранить период
     *
     * @param string|stdClass|array $data - Данные периода
    ...
    public function save($data = null, $options = [])
    {
        ...
        {// Обновление записи
            $age = $this->update($normalized_data);
            if ( empty($age) )
            {// Обновление не удалось
                throw new dof_exception_dml('error_save_age');
            } else
            {// Обновление удалось
                $this->dof->send_event('storage', 'ages', 'item_saved', (int)$normalized_data->id);
                return $normalized_data->id;
            }
        } else
        {// Создание записи
            $ageid = $this->insert($normalized_data);
            if ( ! $ageid )
            {// Добавление не удалось
                throw new dof_exception_dml('error_save_age');
            } else
            {// Добавление удалось
                $this->dof->send_event('storage', 'ages', 'item_saved', (int)$ageid);
                if( ! empty($options['activate']) )
                {// Требуется активация объекта
                    $this->dof->send_event('storage', 'ages', 'activate_request', (int)$ageid);
                }
                return $ageid;
            }
        }
    ...
    /**
     * Нормализация данных периода
     *
     * Формирует объект периода на основе переданных данных. В случае критической ошибки
     * или же если данных недостаточно, выбрасывает исключение.
     *
     * @param string|stdClass|array $data - Данные периода(id или комплексные данные)
     * @param array $options - Опции работы
     *
     * @return stdClass - Нормализовализованный Объект периода
    ...
    public function normalize($data, $options = [])
    {
        ...
        {// Новый период
            ...
            {// Установка названия по умолчанию
                $saveobj->firstname = date('Y');
            }
            if ( ! isset($saveobj->begindate) || empty($saveobj->begindate) )
            {
            ...
            if ( ! isset($saveobj->eduweeks) || empty($saveobj->eduweeks) )
            {
                $saveobj->eduweeks = 0;
            }
            ...
            if ( ! isset($saveobj->previousid) || empty($saveobj->previousid) )
            {
                $saveobj->previousid = null;
            }
            if ( ! isset($saveobj->previousid) || empty($saveobj->previousid) )
            {
                $saveobj->previousid = null;
            }
            if ( ! isset($saveobj->schdays) || empty($saveobj->schdays) )
            {
                $saveobj->schdays = 7;
            }
            if ( ! isset($saveobj->schedudays) || empty($saveobj->schedudays) )
            {
                $saveobj->schdays = '1,2,3,4,5';
            }
            if ( ! isset($saveobj->schedudays) || empty($saveobj->schedudays) )
            {
                $saveobj->schdays = 1;
        ...
        // Нормализация даты начала периода
        if ( isset($saveobj->begindate) && $saveobj->begindate != null )
        {
            if ( ! is_int_string($saveobj->begindate) )
            {// Время представлено в строковом формате
                $saveobj->begindate = strtotime($saveobj->begindate);
            }
        }
        // Нормализация даты окончания периода
        ...
        {
            if ( ! $this->dof->storage('departments')->is_exists($saveobj->departmentid) )
            {
