...
$addvars['showall'] = optional_param('showall', 1, PARAM_INT);
if ( $linktype === 'plan' )
...
$linkyes ='/activeall.php?linktype='.$linktype.'&linkid='.$linkid.'&active=1&showall='.$addvars['showall'];
$linkno ='/themeplan/editthemeplan.php?linktype='.$linktype.'&linkid='.$linkid.'&showall='.$addvars['showall'];
if ( $delete )
{// если сказали удалить - сменим статус
    if ( $plans = $DOF->storage('plans')->get_records(array('linktype'=>$linktype,'linkid'=>$linkid,
                  'status'=>array('draft','fixed','excluded'))) )
    {//если есть темпланы, которые можно подтвердить - подтверждаем их
        foreach ( $plans as $plan )
        {
            $DOF->workflow('plans')->change($plan->id,'active');
        }
    }
...
}else
{
    ...
    $DOF->modlib('nvg')->add_level($DOF->get_string('active_plan', 'plans'),
                                   $DOF->url_im('plans','/activeall.php?linktype='.$linktype.'&linkid='.$linkid,$addvars));
    $DOF->modlib('nvg')->print_header(NVG_MODE_PAGE);

    // спросим об удалении
    $DOF->modlib('widgets')->notice_yesno(
        $DOF->get_string('confirmation_active_plan_all','plans'), 
        $DOF->url_im('plans',$linkyes,$addvars),
