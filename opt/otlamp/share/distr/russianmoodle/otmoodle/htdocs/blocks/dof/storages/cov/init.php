...
class dof_storage_cov extends dof_storage
{
    ...
    /**
     * Дополнение объекта всеми возможными его дополнительными полями
     *
     * @param unknown $plugintype - типа плагина
     * @param unknown $plugincode - код плагина
     * @param stdClass $object - объект, подлежащий наполнению
     * @param unknown $objectid - идентификатор (может содержаться в объекте)
     * @throws dof_exception
     * @return stdClass
     */
    public function fill_object_with_all_customs($plugintype, $plugincode, stdClass $object = null, $objectid = null)
    {
        if (is_null($object))
        {
            $object = new stdClass();
        ...
        {
            $conds['objectid'] = $object->id;
        } else
        {
            throw new dof_exception('Object id is required and was not found.');
        }
        
        $fields = $this->dof->storage($this->code())->get_records($conds,'','id,code,value,substorage');
        
        foreach ($fields as $field)
        {
            $fieldname = $field->code;
            if (!is_null($field->substorage))
            {
                $fieldname = $field->substorage.'__'.$fieldname;
            }
    ...
    /**
     * Поиск единственного объекта по значению кастомного поля
     * 
     * @param string $plugintype - тип плагина
     * @param string $plugincode - код плагина
     * @param string $code - код поля
     * @param unknown $value - искомое значение
     * @param string $substorage - код подхранилища (любое, если null)
     * 
     * @return int|boolean - идентификатор объекта, если найден единственный или false во всех других случаях
     */
    public function find_object_by_value($plugintype, $plugincode, $code, $value, $substorage=null)
    {        
        // Получим значение
        $records = $this->find_objects_by_value($plugintype, $plugincode, $code, $value, $substorage);
    ...
    /**
     * Поиск всех объектов, соответствующих значению кастомного поля
     * 
     * @param string $plugintype - тип плагина
     * @param string $plugincode - код плагина
     * @param string $code - код поля
     ...
     * @param string $substorage - код подхранилища (любое, если null)
     * 
     * @return array|boolean - массив с записями, содержащими идентификатор объекта или false в случае, если выборка завершилась ошибкой
     */
    public function find_objects_by_value($plugintype, $plugincode, $code, $value, $substorage=null)
    {
        $conds = array(
            'plugintype' => $plugintype,
            'plugincode' => $plugincode,
        ...
        if (!is_null($substorage))
        {
            $conds['substorage'] = $substorage;
        }
        
        // Получим значение
        $records = $this->get_records_sql(
            'SELECT id, substorage, objectid
             FROM {block_dof_s_cov}
             WHERE
                plugintype = :plugintype AND
                plugincode = :plugincode AND
                '. (is_null($substorage) ? '' : 'substorage = :substorage AND') . '
                code = :code AND
                '.$this->dof->storage('cov')->sql_compare_text('value'). ' = ' . $this->dof->storage('cov')->sql_compare_text(':value'),
            $conds
        );
        
//         $this->dof->storage($this->code())->get_records($conds, '', 'id, substorage, objectid');
