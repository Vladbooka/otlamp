...
class dof_storage_config extends dof_storage
{
    ...
    public function list_catch_events()
    {
        return [
            ...
            ['plugintype' => 'storage', 'plugincode' => 'config', 'eventcode' => 'insert'],
            ['plugintype' => 'storage', 'plugincode' => 'config', 'eventcode' => 'update'],
            ['plugintype' => 'storage', 'plugincode' => 'config', 'eventcode' => 'delete']
        ];
    }
    ...
    public function catch_event($gentype,$gencode,$eventcode,$intvar,$mixedvar)
    {
        ...
        if ( $gentype === 'core' AND $gencode === 'core' )
        {
            ...
            switch($eventcode)
            {
                ...
                case 'plugin_upgrade':
                    ...
                    $cache = $this->dof->get_cache('storage', 'config', 'configs');
                    if( $cache !== false )
                    {
                        $cache->purge();
                    }
        ...
        if( $gentype === 'storage' && $gencode === 'config' )
        {
            switch($eventcode)
            {
                case 'insert':
    ...
    public function get_config($configcode, $plugintype, $plugincode, $departmentid=0, $personid=0)
    {
        ...
        $result = false;
        $cache = $this->dof->get_cache('storage', 'config', 'configs');
        $key = md5($configcode . $plugintype . $plugincode . (string)$departmentid . (string)$personid);
        if( $cache !== false )
        {
    ...
    public function get_select_listing($inputconds,$prefix='')
    {
        ...
        if( ! empty($conds->departmentid) )
        {// Если требуется найти настройки для конкретного подразделения
            $depstring = $this->dof->storage('departments')->change_path_department($conds->departmentid);
            
            $selects[] = " departmentid IN (".$depstring.",0) ";
    ...
    {
        // по умолчанию имя
        $sqlsort = $prefix.'plugintype,plugincode,code';
        if ( ! is_array($sort) )
        ...
        $selects[] = $prefix.'plugintype,plugincode '.$dir;
