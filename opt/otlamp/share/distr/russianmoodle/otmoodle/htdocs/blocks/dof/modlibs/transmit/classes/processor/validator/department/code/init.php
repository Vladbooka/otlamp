...
/**
 * Обмен данных с внешними источниками. Валидация кода подразделения
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_validator_department_code extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_required = ['department_code' => null];
     ...
     * @var array
     */
    public static $slots_output = ['department_code' => null];
    ...
    {
        $code = trim(((string)$input['department_code']));
        if ($code) {
            return ['department_code' => $code];
        }
        return [];
    }
}