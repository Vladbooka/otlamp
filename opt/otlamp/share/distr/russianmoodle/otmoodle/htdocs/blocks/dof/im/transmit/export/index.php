...
/**
 * Интерфейс управления обменом данными. Экспорт.
 *
 * @package    im
 * @subpackage transmit
...
$DOF->modlib('nvg')->add_level(
    $DOF->get_string('page_export_name', 'transmit'),
    $DOF->url_im('transmit', '/export/index.php', $addvars)
);

// Добавление вкладок
$html .= $DOF->im('transmit')->render_tabs('export', $addvars);
...
// Текущий url
$currenturl = $DOF->url_im('transmit', '/export/index.php', $addvars);

// Получение конфигуратора
$configurator = $DOF->modlib('transmit')->get_export_configurator();
...
{// Отображение формы настройки импорта
    ...
    // Добавление уровня навигации
    $DOF->modlib('nvg')->add_level(
        $DOF->get_string('page_export_config_name', 'transmit'),
        $currenturl
    );
