...
/**
 * Роутер статусов для отчетов Деканата
 * 
 * @package    workflow
 * @subpackage reports
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_reports implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'reports' AND $eventcode === 'insert' )
    ...
    {
        return [
                        'requested' => $this->dof->get_string('status:new',        'reports', NULL, 'workflow'),
                        'active'    => $this->dof->get_string('status:active',     'reports', NULL, 'workflow'),
                        'failed'    => $this->dof->get_string('status:failed',     'reports', NULL, 'workflow'),
                        'completed' => $this->dof->get_string('status:completed',  'reports', NULL, 'workflow'),
                        'suspend'   => $this->dof->get_string('status:suspend',    'reports', NULL, 'workflow'),
                        'deleted'   => $this->dof->get_string('status:deleted',    'reports', NULL, 'workflow')
        ];
    }
    ...
    public function get_available($id)
    {
        ...
        if ( ! $obj = $this->dof->storage('reports')->get($id) )
        {
            // Объект не найден
            return false;
        }
        ...
        switch ( $obj->status )
        {
            ...
            case 'active':
                ...
                return $statuses;
            case 'failed':
                $statuses['requested'] = $this->get_name('requested');
                $statuses['active']    = $this->get_name('active');
                $statuses['deleted']   = $this->get_name('deleted');
                return $statuses;
            case 'completed':
    ...
    public function change($id, $newstatus, $options = null)
    {
        ...
        if ( $result )
        {
            ...
            {   
                // Выбросим событие об удалении объекта
                $this->dof->send_event('workflow', 'reports', 'junk', $id);
            }
        }
    ...
    {
        // Получаем объект
        if ( ! $skill = $this->dof->storage('reports')->get($id) )
        {
            // Объект не найден
            return false;
        }
        ...
        return $this->dof->storage('reports')->update($obj);
    }
