...
/**
 * Роутер статусов для назначений на должности
 *
 * @package    workflow
 * @subpackage appointments
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_appointments implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'appointments' AND $eventcode === 'insert' )
        {
    ...
    public function get_list()
    {
        return array('plan'=>$this->dof->get_string('status:plan',$this->get_storage(),NULL,'workflow'),
                     ...
                     'patient'=>$this->dof->get_string('status:patient',$this->get_storage(),NULL,'workflow'),
                     'vacation'=>$this->dof->get_string('status:vacation',$this->get_storage(),NULL,'workflow')
        );
    }
    ...
    public function get_available($id)
    {
        ...
        switch ( $obj->status )
        {
            case 'plan':       // переход из статуса "запланирован"
                $statuses = array();
                if ( isset($obj->eagreementid) AND isset($obj->schpositionid) AND
                   ($eagreement = $this->dof->storage('eagreements')->get($obj->eagreementid)) AND
                   ($schposition = $this->dof->storage('schpositions')->get($obj->schpositionid)) )
                {// если у вакансии есть должность
                    if ( isset($eagreement->status) AND ($eagreement->status == 'active') AND
                         isset($schposition->status) AND ($schposition->status == 'active') )
                    {// и эта должность активена, то вакансию можно перевести в статус идет
            ...
            case 'active':   // переход из статуса "идет"
                $statuses = array('canceled'=>$this->get_name('canceled'),
                                  'vacation'=>$this->get_name('vacation'),
                                  'patient' =>$this->get_name('patient'));
            break;
            case 'canceled':  // переход из статуса "отменен"
                $statuses = array();
            break;
            case 'patient':  // переход из статуса "больничный"
                $statuses = array('active'  =>$this->get_name('active'),
                                  'vacation'=>$this->get_name('vacation'),
                                  'canceled'=>$this->get_name('canceled'));
            break;
            case 'vacation':  // переход из статуса "отпуск"
                $statuses = array('active'  =>$this->get_name('active'),
                                  'patient' =>$this->get_name('patient'),
                                  'canceled'=>$this->get_name('canceled'));
            break;
    ...
    public function change($id, $newstatus, $options=null)
    {
        ...
        if ( $result )
        {
            ...
            $select->basetype  = 'record';
            $select->baseid    = $object->departmentid;
            $select->personid  = $this->dof->storage('eagreements')->get_field($object->eagreementid,'personid');
            ...
            switch($newstatus)
            {
                case 'active':
                    ...
                    {//Обновляем все записи, касающиеся данного назначения
                        if ( $this->dof->storage('aclwarrants')->get_field($record->aclwarrantid,'status') == 'active' )
                        {// если сама мандата в том же статусе, в которое обновляем применение - можем сменить статус
                            $result = ( $result AND $this->dof->workflow('aclwarrantagents')->change($record->id,'active') );
                        }
                    }
                    break;
                case 'canceled':
                    ...
                    $counds->departmentid = $object->departmentid;
                    $counds->status = 'active';
                    if ( $this->dof->storage($this->get_storage())->get_listing($counds) )
                    ...
                    foreach($oldlist as $key => $record)
                    {//Обновляем все записи, касающиеся данного назначения
                        $result = ( $result AND $this->dof->workflow('aclwarrantagents')->change($record->id,'archive') );
                    }
                    break;
                case 'patient':
                case 'vacation':
                ...
                default: 
                    ...
                    foreach($oldlist as $key => $record)
                    {
                        $result = ( $result AND $this->dof->workflow('aclwarrantagents')->change($record->id,'draft') );
                    }
            }
        }
        
        if ( ! $result )
        {// какому-то применению полномочий не удалось изменить статус - вернем подписке исходное состояние
            $storage->update($object);
            // сообщим о неудачной операции
            return false;
        }
        
        // если меняем на активный статус, то бросаем событие об этом
        // чтобы плагин sync/personstom его отловил и в зависимости от конфига
        // либо синхронизировал персону с пользователем moodle, либо этого не делал
        if (array_key_exists($newstatus, $this->get_meta_list('active')))
        {
            $this->dof->send_event('workflow', 'appointments', 'appointment_activated', $id);
        }
