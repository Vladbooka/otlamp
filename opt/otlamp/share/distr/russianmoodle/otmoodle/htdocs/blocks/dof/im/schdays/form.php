...
class dof_im_schdays_edit_schday_form extends dof_modlib_widgets_form
{
    ...
    public function definition()
    {
        ...
        $mform->setDefault('department', 
                           $this->dof->storage('departments')->get_field($this->_customdata->age->departmentid,'name').' ['.
                           $this->dof->storage('departments')->get_field($this->_customdata->age->departmentid,'code').']');
        // выбор даты
        ...
        $mform->addElement('hidden', 'beforeactionbuttons', 0);
        $mform->setType('beforeactionbuttons', PARAM_INT);
    ...
    public function definition_after_data()
    {
        ...
        {// создать расписание
		    $createevents =& $mform->createElement('checkbox', 'create', '', $this->dof->get_string('create_events',$this->im_code()));
		    $mform->setType('create', PARAM_BOOL);
		    $mform->insertElementBefore($createevents, 'beforeactionbuttons');
        }
        if ( $id )
    ...
    public function process()
    {
        ...
        if ( $this->is_submitted() AND confirm_sesskey() AND $formdata = $this->get_data() )
        {
            ...
            //получение периода
            $age = $this->dof->storage('ages')->get($dbobj->ageid);
            //вновь создающиеся дни должны принадлежать тому же подразделению, что и период, к которому день относится
...
class dof_im_schdays_calendar_form extends dof_modlib_widgets_form
{
    ...
    public function definition()
    {
        ...
        // Массовая обработка
        $action = $mform->addElement('dof_hierselect', 'action', $this->dof->get_string('form_days_action', 'schdays'),
    ...
    private function get_data_full_day($day, $schday, $i)
    {
        ...
        {// Подразделение дня должно соответствовать подразделению периода, к которому день относится
            if ( ! empty($this->addvars['ageid']) )
            {// Передан идентификатор учебного периода
                if ( $age = $this->dof->storage('ages')->get($this->addvars['ageid']) )
                {// Период получен
                    if ( $this->departmentid != $age->departmentid )
                    {// Подразделение периода не совпадает с текущим - не показываем ссылку
                        $res->link = $res->numday.'/'.$res->nameweekday;
                    }
                } else
                {// Период должен существовать - не показываем ссылку
                    $res->link = $res->numday.'/'.$res->nameweekday;
                }
            } else
            {// Период должен быть указан - не показываем ссылку
                $res->link = $res->numday.'/'.$res->nameweekday;
            }
        }
        return $res;
