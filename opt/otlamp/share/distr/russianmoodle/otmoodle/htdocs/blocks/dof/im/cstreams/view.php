...
{// выводим ссылку на экспорт файла в формат odf
    $html .= ('<p><a href="'.$DOF->url_im('cstreams', '/view.php?cstreamid='.$cstreamid.'&export=odf').'">'.
...
if ( $message )
{
    $html .= ('<p align="left">'.$message.'</p>');
}
// выводим поток
$html .= '<br>'.$cstream;

// Создание ссылки на историю статусов
$html .= $DOF->modlib('widgets')->status_link(
    ['plugintype' => 'storage', 'plugincode' => 'cstreams', 'objectid' => $cstreamid]
    );
...
$html .= $DOF->im('cstreams')->get_table_statushistory($cstreamid);
