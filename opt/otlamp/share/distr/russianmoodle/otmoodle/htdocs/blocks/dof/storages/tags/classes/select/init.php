...
/**
 * select - подкласс тега для выбора всех объектов из одного хранилища
 * 
 * Опции класса
 *   ->storage - Хранилище, из которого необходимо получить записи
 *                          
 */
class dof_storage_tags_tags_select extends dof_storage_tags_tags 
{
    /**
    ...
    public function get_tagoptions()
    {
        ...
        $options = array( 
                'storage' => array(
                    ...
                    'name' => 'select',
                    'label' => $this->dof->
                        get_string('class_select_storage','tags', NULL, 'storage'),
                    'setDefault' => 'persons',
                    'setType' => PARAM_TEXT,
                    'addRule' => 'required',
                    'addRuleValidation' => 'client'
                    )
                );
    ...
    public function check_tag($code, $department, $alias='', $about='', $parentid = 0, $ownerid = 0, $cron = -1, $cronrepeate = 0, $options = NULL)
    {
        ...
        {
            // Сериализуем опции тега
            $result->options = serialize($options);
        } else 
        {
    ...
    */
    ...
    {   
        ...
        /* Производим проверку по критериям */
        
        if (! $plugintype == 'storage')
        {// Плагин не из хранилища
            return NULL;
        }
        
        if ( ! $plugincode == $tagoptions->storage)
        {// Хранилище не удовлетворяет условиям
            return NULL;
        }
    ...
    */
    ...
    {      
        ...
        // Формируем массив статусов для возвращаемых объектов
        if ( $this->dof->plugin_exists('workflow', $tagoptions->storage) )
        { // Если плагин статусов  есть - возвращаем массив реальных статусов
            $statuses = $this->dof->workflow($tagoptions->storage)->get_meta_list('real');
        } else
        { // Если плагина нет - нельзя определить статусы, которые нам нужны
            $statuses = NULL;
        }
        
        if ( ! empty($statuses) )
        {// Если статусы определены - фильтруем по ним
            $sql .= ' ( ';
            $start = false;
        ...
        // Переопределяем подразделение, если оно не указано в методе
        if ( empty($department) )
        {
            $department = $tagobject->departmentid;
        }
        
        // Получаем массив дочерних подразделений
        $departments = $this->dof->storage('departments')->
                            departments_list_subordinated($department);
        // Добавляем к массиву родительский ID
        ...
        if ( ! empty($sql) )
        {
            $sql .= ' AND ( ';
            $start = false;
            foreach ( $departments as $depid => $item )
            {
                if ( $start )
                {
                    $sql .= ' OR ';
        ...
        // Получаем объекты
        $objects = $this->dof->storage($tagoptions->storage)->
                   get_records_select($sql, $params);
        // Добавляем каждый объект в массив
        foreach ($objects as $object)
        {
            $result->list['storage'][$tagoptions->storage]
                [$object->id] = $object;
        }
    ...
    public function show_taglinks_list($tagobject, $list, $addvars = NULL )
    {
        ...
        foreach ($list as $item)
        {
            ...
            if ( $this->dof->storage('taglinks')->is_access('view', $item->id, null, $depid) )
            {
                ...
                // Ссылка
                $row[] = '<a 
                    href="'.$this->dof->url_im('crm','/tags/taglink.php',$addvars).'" 
                    title="'.$this->dof->get_string('go_to_taglink', 'crm').'">'.
                            $item->id.
                    '</a>';
            } else 
            {
