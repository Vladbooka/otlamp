...
// **********************************************
     ...
     *      'return_html'  - Вернуть HTML-код вместо отображения 
     *          false - Непосредственно отобразить блок комментариев
     *          true -  Вернуть HTML-код блока комментариев
     *          ['form' => true, 'comments_list' => true ] - Тонкая настройка элементов блока
     */
    public function commentsform($ptype, $pcode, $objectid, $addvars, $code = NULL, $options = [] )
    {
        ...
        if ( isset($options['return_html']['form']) && isset($options['return_html']['form']) === false )
        {// Запрет отображения формы
            
        } else 
        {// Требуется отобразить форму
            // Инициализация формы
        ...
        if ( isset($options['return_html']['comments_list']) && isset($options['return_html']['comments_list']) === false )
        {// Запрет отображения списка комментариев
        
        } else
        {// Требуется отобразить список комментариев
            $coptions = $options;
            $coptions['return_html'] = true;
            $html .= $this->show_comments_list($ptype, $pcode, $objectid, $code, $coptions);
        }
        
        if ( (bool)$options['return_html'] )
        {// Требуется вернуть HTML-код блока
            return $html;
        } else 
        {// Требуется отобразить блок
            echo $html;
        }
    }
