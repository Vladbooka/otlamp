...
/**
 * Роутер статусов для причин отсуствия на занятии
 *
 * @package    workflow
 * @subpackage schabsenteeism
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_schabsenteeism implements dof_workflow
{
    /**
    ...
    public function is_access($do, $objid = NULL, $userid = NULL, $depid = null)
    {
        ...
        // Проверка наличия права на перевод в конкретный статус
        $statuses = array_keys($this->get_list());
        foreach ( $statuses as $status )
        {
            if ( $do === 'changestatus:to:'.$status )
            {// Проверяется право перехода в указанный статус
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'schabsenteeism' AND $eventcode === 'insert' )
        {
    ...
    {
        return [
            'active'  => $this->dof->get_string('status:active', 'schabsenteeism', NULL, 'workflow'),
            'deleted' => $this->dof->get_string('status:deleted', 'schabsenteeism', NULL, 'workflow')
        ];
    }
    ...
    public function get_available($id)
    {
        ...
        if ( ! $obj = $this->dof->storage('schabsenteeism')->get($id) )
        {
            // Объект не найден
            return false;
        }
    ...
    public function init($id)
    {
        ...
        return $this->dof->storage('schabsenteeism')->update($obj);
    }
