...
class dof_im_recordbook_learning_program
{
    ...
    public function print_info_table($programsbcid)
    {
        ...
        // получаем данные подписке на программу
        $programmsbc = $this->dof->storage('programmsbcs')->get($programsbcid);
        // заносим в строку таблицы текущий семестр учащегося по подписке
        $currentagedata = array('<b>'.$this->dof->get_string('currentage','recordbook').'</b>', $programmsbc->agenum);
        // заносим в строку таблицы количество семестров в программе
        $agenumdata = array('<b>'.$this->dof->get_string('agenums','recordbook').'</b>', $programm->agenums);
        // формируем структуру и свойства таблицы
        $table->tablealign = 'left';
        $table->data[] = $programmdata;
        $table->data[] = $departmentdata;
        $table->data[] = $studentdata;
        $table->data[] = $currentagedata;
        $table->data[] = $agenumdata;
    ...
    private function print_oneagenum_progitemstable($programmid, $studentid, $ageid, $clientid, $agenum,$programmsbcid=null)
    {
        ...
        // основным заголовком таблицы будет являтся название семестра и номер параллели (в скобках)
        $title = $this->dof->storage('ages')->get_field($ageid, 'name') . ' (' . $agenum . ' ' . $this->dof->get_string('agenum', 'recordbook') . ')';
        $title = dof_html_writer::tag('strong', $title);
        $title = dof_html_writer::tag('h4', $title);
        $titletable->head   = [$title];
        
        $link = dof_html_writer::link(
            $this->dof->url_im('recordbook', '/finalgrades.php?programmsbcid='.$programmsbcid.'&ageid='.$ageid, $addvars), 
            $this->dof->get_string('finalgrades', 'recordbook')
        );
        $titletable->data[] = [$link];
        
        // выводим основное содержимое
        $table = new stdClass();
        $table->class = 'generaltable dof_im_recordbook_program_customview';
        ...
        if ( ! empty($strings) AND is_array($strings) )
        {// выводим таблицу с предеметами
            // Статусы успешных cpassed-ов
            $success = array_keys($this->dof->workflow('cpassed')->get_meta_list('actual'));
            // Статусы завершенных cpassed-ов
            $complete = array_keys(array_diff($this->dof->workflow('cpassed')->get_meta_list('real'), $this->dof->workflow('cpassed')->get_meta_list('actual')));
            
            // Успешные cpassed-ы
            $stringssuc = [];
            
            // Завершенные cpassed-ы
            $stringscomplete = [];

            foreach ($strings as $string)
            {
                if ( in_array($string[4], $success) )
                {// Если у cpassed-а успешный статус, то его в массив успешных
                    unset($string[4]);
                    $stringssuc[] = $string;
                }
                elseif ( in_array($string[4], $complete) )
                {// Если у cpassed-а завершенный статус, то его в массив завершенных
                    unset($string[4]);
                    $stringscomplete[] = $string;
                }    
            }
            ...
            $result .= "<div class='dof_im_recordbook_program_customdiv'>";
            
            if ( ! empty($stringssuc) )
            {// Отображаем таблицу с активными дисциплинами
                // Добавляем заголовки для таблицы с активными дисциплинами
                //$this->dof->get_string('credits', 'recordbook'));
                $table->head = [
                    $this->dof->get_string('active_discipline', 'recordbook'),
                    $this->dof->get_string('controltype', 'recordbook'),
                    $this->dof->get_string('total_grade', 'recordbook'),
                    $this->dof->get_string('hours_on_plan', 'recordbook')
                ];
                $table->data = $stringssuc;
            ...
            {// Нет активных дисциплин
                $result .= '<div class="dof_im_recordbook_program_empty"><i>' . $this->dof->get_string('active_discipline_empty','recordbook') . ' </i></div>';
            }
            
            if ( ! empty($stringscomplete) )
            {// Отображаем таблицу с завершенными дисциплинами
                // Добавляем заголовки для таблицы с завершенными дисциплинами
                $table->head = [
                    $this->dof->get_string('failed_discipline', 'recordbook'),
                    $this->dof->get_string('controltype', 'recordbook'),
                    $this->dof->get_string('total_grade', 'recordbook'),
                    $this->dof->get_string('hours_on_plan', 'recordbook')
                ];
                $table->data = $stringscomplete;
                $result .= $this->dof->modlib('widgets')->print_table($table, true);
            }
            else
            {// Нет завершенных дисциплин
                $result .= '<div class="dof_im_recordbook_program_empty"><i> ' . 
                $this->dof->get_string('failed_discipline_empty','recordbook') . 
                ' </i></div>';
            }
            $result .= "</div>";
            return $result;
    ...
    private function print_oneagenum_plannedtable($programmsbcid, $agenum)
    {
        ...
        $hours->header = true;
        $controltype = new html_table_cell($this->dof->get_string('controltype', 'recordbook'));
        $controltype->header = true;
        $head = new html_table_cell($agenum . ' ' . strtolower($this->dof->get_string('agenum', 'recordbook')));
        $head->colspan = 2;
        $head->style = 'text-align: center';
        $table->head = array($head);
        $table->data = array();
        $table->data[] = array($discipline, $controltype, $hours);
                ...
                if ( $cpassed = $this->dof->storage('cpassed')->get_listing($data) )
                {
                    ...
                    foreach ( $cpassed as $cpass )
                    {
                        ...
                        $fullstring = $cphistorylink . ' <a href="'.$this->dof->url_im('recordbook', '/discipline.php', 
                                      ...
                                      '">'.$pitem->name.'</a>';
                        $controltype = $this->dof->modlib('refbook')->get_st_total_control_name($pitem->controltypeid);
                        $grade = '-';
                        ...
                        $result[] = array($fullstring, $controltype, $cpass->grade, $this->get_progitem_hours($pitem->id), $cpass->status);
                    }
                }else
        ...
        {// нет элементов учебной программы в этом семестре
            ...
            {// перебираем учебные программы и ищем оценки';
                $fullstring = '<a href="'.$this->dof->url_im('programmitems', '/view.php', 
                              ...
                              '">'.$pitem->name.'</a>';
                $controltype = $this->dof->modlib('refbook')->get_st_total_control_name($pitem->controltypeid);
                $result[] = array($fullstring, $controltype, $this->get_progitem_hours($pitem->id));
            }
        }
        return $result;
    }
...
class dof_im_recordbook_recordbook
{
    ...
    private function get_title_recordbook()
    {
        ...
        $result->titledate     = '<div align="center">'.
            ...
            $result->titleplace    = $this->dof->get_string('place', 'recordbook');
    ...
    private function get_lesson_string($event, $studentid, $style = '#FFFFFF')
    {
        ...
        // Добавление данных о кабинете
        $eventdata->place = ! empty($event->place) ? $event->place : '';
