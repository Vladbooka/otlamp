...
class dof_workflow_achievements implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'achievements' AND $eventcode === 'insert' )
    ...
    {
        return array(
                'draft'        => $this->dof->get_string('status:draft',        'achievements', NULL, 'workflow'),
                'available'    => $this->dof->get_string('status:available',    'achievements', NULL, 'workflow'),
                'notavailable' => $this->dof->get_string('status:notavailable', 'achievements', NULL, 'workflow'),
                'deleted'      => $this->dof->get_string('status:deleted',      'achievements', NULL, 'workflow')
        );
    }
    ...
    public function get_available($id)
    {
        ...
        if ( ! $obj = $this->dof->storage('achievements')->get($id) )
        {
            // Объект не найден
            return false;
        }
    ...
    {
        // Получаем объект из achievements
        if ( ! $object = $this->dof->storage('achievements')->get($id) )
        {// Объект не найден
            return false;
        }
        // Меняем статуc
        ...
        return $this->dof->storage('achievements')->update($obj);
    }
