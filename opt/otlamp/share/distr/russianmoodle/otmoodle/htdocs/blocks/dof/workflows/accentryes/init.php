 ...
 * 
 */
class dof_workflow_accentryes implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('accentryes'=>2014110000,
                                      'acl'=>2011041800));
    }
    ...
    {
        return array(array('plugintype' => 'storage', 'plugincode' => 'accentryes', 'eventcode' => 'insert'));
    }
    /** 
    ...
    {
        if ( $gentype === 'storage' AND $gencode === 'accentryes' )
    ...
    {
        return array('active'      => $this->dof->get_string('status:active','accentryes',NULL,'workflow'),
                     'canceled'    => $this->dof->get_string('status:canceled','accentryes',NULL,'workflow'),
                     'rejected'    => $this->dof->get_string('status:rejected','accentryes',NULL,'workflow'),
                     'deleted'   => $this->dof->get_string('status:deleted','accentryes',NULL,'workflow'));
    }
    ...
    public function get_available($id)
    {
        ...
        {
            case 'active':
                $statuses = array('canceled' => $this->get_name('suspend'),
                                  'rejected' => $this->get_name('completed'),
    ...
    public function change($id, $newstatus, $options=null)
    {
        ...
        // Изменение статуса
        $result = $storage->change_status($id, $newstatus);
    ...
    {
        // Получаем объект 
        if ( ! $obj = $this->dof->storage('accentryes')->get($id) )
        {
            // Объект не найден
            return false;
        }
        ...
        $obj = new stdClass();
        $obj->status = 'active';
        return $this->dof->storage('accentryes')->update($obj, intval($id));
    }
