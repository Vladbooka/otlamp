...
/**
 * Обмен данных с внешними источниками. Конвертер электронной почты в идентификатор персоны
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_person_email extends dof_modlib_transmit_processor_base
{
    /**
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        $persons = $dof->storage('persons')->get_records(
                [
                    'email' => $input['email'],
                    'status' => array_keys($dof->workflow('persons')->get_meta_list('active'))
                ]);
        if ( ! empty($persons) )
        {// Персона найдена
            return ['personid' => key($persons)];
        } 
        else if( ! empty($input['trycreatefrommoodle']) )
        {// требуется попытка создать персону из пользователя moodle
            $moodleusers = $dof->modlib('ama')->user(false)->get_user_by_email($input['email']);
            if( ! empty($moodleusers) && count($moodleusers) == 1 )
            {// есть пользователь moodle с указанным email
