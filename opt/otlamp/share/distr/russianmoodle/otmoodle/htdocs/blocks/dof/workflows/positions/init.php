...
/**
 * Роутер статусов для должностей
 *
 * @package    workflow
 * @subpackage positions
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_positions implements dof_workflow
{
    /**
    ...
    {
        return array('storage'  => array('positions'   => 2010040200,
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'positions','eventcode'=>'insert'));
    }
    /** 
    ...
    {
		if ( $gentype==='storage' AND $gencode === 'positions' AND $eventcode === 'insert' )
		{
    ...
    public function change($id, $newstatus, $options=null)
    {
        ...
        if ( $result )
        {
            // синхронизируем доверенности с должностью
            if ( ! $this->acl_syncronize_warrants_with_position($id, $newstatus) )
            {//синхронизация не удалась, откатываем все назад
    ...
    /** Снхронизировать доверенности с должностью. При выполнении этой функции 
     * для активированных должностей активируются доверенности, а для удаленных
     * должностей доверенности отправляются в архив  
     * 
     * @return bool 
     * @param int $positionid - id должности в таблице positions
     * @param string $newstatus - новый статус должности
     */
    protected function acl_syncronize_warrants_with_position($positionid, $newstatus)
    {
        switch ( $newstatus )
        {
            // при активации должности активируем все ее доверенности
            case 'active': 
                $newstatus = 'active';
                $oldstatus = 'draft';
            break;
            // при удалении должности деактивируем все ее доверенности
            case 'canceled' :
                $newstatus = 'archive';
                $oldstatus = array('draft', 'active');
            break;
        ...
        $conditions = array();
        $conditions['linkptype'] = 'storage';
        $conditions['linkpcode'] = $this->get_storage();
        $conditions['linktype']  = 'record';
        $conditions['linkid']    = $positionid;
        $conditions['status']    = $oldstatus;
        
        // составляем sql для извлечения всех черновиков доверенностей
        $sql = $this->dof->storage('aclwarrants')->get_select_listing($conditions);
        
        // получаем список доверенностей, которые нужно синхронизировать
        if ( ! $warrants = $this->dof->storage('aclwarrants')->get_records_select($sql) )
        {// ничего синхронизировать не надо
            return true;
        }
        
        return $this->dof->workflow('aclwarrants')->set_status_to_list($warrants, $newstatus);
    }
}
?>