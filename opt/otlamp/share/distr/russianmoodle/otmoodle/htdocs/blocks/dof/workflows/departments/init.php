...
/**
 * Роутер статусов для подразделений
 *
 * @package    workflow
    ...
    public function install()
    {
        ...
        foreach ( $records as $record )
        {
            if ( $record->status != 'deleted' )
            {
                ...
                if ( $this->dof->storage($this->get_storage())->update($record, $record->id, true, true) )
                {
                    $this->dof->storage('aclwarrants')->create_proxy_warrants($record->id);
                }
            }
        }
                ...
                case 'deleted':
                    ...
                    {// если есть дочерние перекинем их на родителя
                        foreach ( $deps as $dep )
                        {
                            $obj = new stdClass();
                            $obj->id = $dep->id;
                            $obj->leaddepid = $object->leaddepid;
                            $result = ($result AND $storage->update($obj));
                        }
                    }
                    $obj = new stdClass();
                    $obj->id = $id;
