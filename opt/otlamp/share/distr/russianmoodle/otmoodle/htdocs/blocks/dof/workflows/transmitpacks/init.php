...
/**
 * Рабочие процессы пакетов настроек для обмена данными
 * 
 * @package    workflow
...
class dof_workflow_transmitpacks implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'transmitpacks' AND $eventcode === 'insert' )
    ...
    {
        return [
            'new'          => $this->dof->get_string('status:new',       'transmitpacks', NULL, 'workflow'),
            'active'       => $this->dof->get_string('status:active',    'transmitpacks', NULL, 'workflow'),
            'suspended'    => $this->dof->get_string('status:suspended', 'transmitpacks', NULL, 'workflow'),
            'deleted'      => $this->dof->get_string('status:deleted',   'transmitpacks', NULL, 'workflow')
        ];
    }
    ...
    public function get_available($id)
    {
        ...
        {
            case '':
                $statuses['new'] = $this->get_name('new');
                return $statuses;
            case 'new':
                ...
                return $statuses;
            case 'active':
                $statuses['suspended'] = $this->get_name('suspended');
                $statuses['deleted'] = $this->get_name('deleted');
                return $statuses;
            case 'suspended':
                $statuses['active'] = $this->get_name('active');
    ...
    public function change($id, $status, $opt = null)
    {
        ...
        if ( ! $object = $this->dof->storage($this->get_storage())->get($id) )
        {// Запись не найдена
            return false;
        }
        ...
        $this->dof->storage('statushistory')->
            change_status($this->get_storage(),intval($id), $status, $object->status, $opt);
        ...
        $new->id = $id;
        $new->status = $status;
        return $this->dof->storage($this->get_storage())->update($new);
    }
