...
/** Класс хранилища "должности"
 * 
 */
class dof_storage_positions extends dof_storage implements dof_storage_config_interface
{
    /**
    ...
    {
		return array('storage'=>array('departments'=>2010022700,
    ...
    public function get_listing($conds=null, $limitfrom = null, $limitnum = null, $sort='', $fields='*', $countonly=false)
    {
        ...
        return $this->get_records_select($select, null,'name ASC, departmentid ASC', '*', $limitfrom, $limitnum);
    }
    ...
    private function get_select_listing($inputconds,$prefix='')
    {
        ...
        {// если мы производим поиск по имени - то нужен особый запрос
            // составим для поиска по названию собственный запрос            
            $selects[] = " {$prefix}name LIKE '%".trim($conds->name)."%' ";
            // исключим поле "название" из списка стандартных запросов 
            unset($conds->name);
        }
        if ( isset($conds->code) AND trim($conds->code) )
        {// если мы производим поиск по имени - то нужен особый запрос
            // составим для поиска по названию собственный запрос            
            $selects[] = " {$prefix}code LIKE '%".trim($conds->code)."%' ";
            // исключим поле "название" из списка стандартных запросов 
            unset($conds->code);
        }
