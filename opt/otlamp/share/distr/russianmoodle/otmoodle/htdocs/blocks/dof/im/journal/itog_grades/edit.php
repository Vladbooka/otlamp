...
$addvars['showall'] = optional_param('showall', 0, PARAM_INT);
$error = '';
...
{// передан id приказа - надо что-то с ним делать
	...
	{// надо сохранить оценки
	    ...
	    {// все прошло успешно - запомним это
	    	...
	    	//получим приказ
	    	$order = $DOF->im('journal')->order('set_itog_grade',$saveorderid);
	    	//получим данные приказа
	    	$orderdata = $order->load($saveorderid);
	    	//сгенерируем событие - оценка выставлена
	    	$DOF->send_event('im', 'journal', 'itog_grade_was_set', $saveorderid, $orderdata);
