        ...
        switch ($name)
        {
            case 'link':
                $result = dof_html_writer::link(
                    $this->dof->url_im($this->code(),'/index.php', $addvars),
                    $this->dof->get_string('page_main_name', $this->code())
                );
                break;
            default:  
                break;
        ...
        switch ($name)
        {
            case 'my_partner_codes';
                ...
                {// Персона есть в деканате
                    ...
                    $departments = $this->dof->storage('departments')->get_records(['managerid' => $person->id]);
                    
                    if ( ! empty($departments) )
                    {// Подразделения найдены
                        foreach ( $departments as $department )
                        {
                            $result .= $this->get_codestable($department->id);
                        }
                    }
                }
                break;
