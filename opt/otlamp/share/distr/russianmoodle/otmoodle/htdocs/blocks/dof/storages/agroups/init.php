...
class dof_storage_agroups extends dof_storage implements dof_storage_config_interface
{
    ...
    public function get_agroups_by_filter($filter = [], $options = [])
    {
        ...
        {
            // Формируем кусок запроса для выборки и параметры для подстановки
            list($insqldepartmentid, $paramsdepartmentid) = $DB->get_in_or_equal($filter['departmentid'], SQL_PARAMS_NAMED);
            // Добавляем кусок запроса в общий список селектов
            $select[] = 'departmentid ' . $insqldepartmentid;
            // Добавляем параметры к общему списку параметров
            $params = array_merge($params, $paramsdepartmentid);
        }
     ...
     * Сохранить группу
     *
     * @param string|stdClass|array $groupdata - Данные группы(название или комплексные данные)
     * @param array $options - Массив дополнительных параметров
     *
     * @return bool|int - false в случае ошибки или ID группы в случае успеха
     *
     * @throws dof_exception_dml - В случае ошибки
     */
    public function save($groupdata = null, $options = [])
    {
        // Нормализация данных
        try {
            $normalized_data = $this->normalize($groupdata, $options);
        ...
        {// Обновление записи
            $groupitem = $this->update($normalized_data);
            if ( empty($groupitem) )
            {// Обновление не удалось
                throw new dof_exception_dml('error_save_agroup');
            } else
            {// Обновление удалось
                $this->dof->send_event('storage', 'agroups', 'item_saved', (int)$normalized_data->id);
                return $normalized_data->id;
            }
        } else
        {// Создание записи
            $agroupid = $this->insert($normalized_data);
            if ( ! $agroupid )
            {// Добавление не удалось
                throw new dof_exception_dml('error_save_agroup');
            } else
            {// Добавление удалось
                $this->dof->send_event('storage', 'agroups', 'item_saved', (int)$agroupid);
                if( ! empty($options['activate']) )
                {// Требуется активация объекта
                    $this->dof->send_event('storage', 'agroups', 'activate_request', (int)$agroupid);
                }
                return $agroupid;
            }
        }
     ...
     *
     * Формирует объект группы на основе переданных данных. В случае критической ошибки
     * или же если данных недостаточно, выбрасывает исключение.
     *
     * @param string|stdClass|array $groupitemdata - Данные группы(название или комплексные данные)
     * @param array $options - Опции работы
     *
     * @return stdClass - Нормализовализованный Объект группы
     * @throws dof_exception_dml - Исключение в случае критической ошибки или же недостаточности данных
     */
    public function normalize($groupitemdata, $options = [])
    {
        // Нормализация входных данных
        if ( is_object($groupitemdata) || is_array($groupitemdata) )
        {// Комплексные данные
            $groupitemdata = (object)$groupitemdata;
        } else
        ...
        if ( isset($groupitemdata->id) && (int)$groupitemdata->id < 1)
        {
            unset($groupitemdata->id);
        }
        
        if ( isset($groupitemdata->id) )
        {// Проверка на существование
            if ( ! $this->get($groupitemdata->id) )
            {// Группа не найдена
                throw new dof_exception_dml('agroup_not_found');
            }
        }
        
        if ( ! isset($groupitemdata->id) && empty($groupitemdata->code) )
        {// Невозможно определить код группы
            throw new dof_exception_dml('create_without_code');
        }
        
        // Валдация программы
        if ( isset($groupitemdata->programmid) )
        {
            if ( ! $this->dof->storage('programms')->get($groupitemdata->programmid) )
            {// Программа не найдена
                throw new dof_exception_dml('programm_not_found');
            }
        }
        
        // Валдация подразделения
        if ( isset($groupitemdata->departmentid) )
        {
            if ( ! $this->dof->storage('departments')->get($groupitemdata->departmentid) )
