...
/**
 * Роутер статусов для задач пользователей
 *
 * @package    workflow
 * @subpackage tasks
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_tasks implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('tasks' => 2014120000,
                                      'acl'   => 2011041800 )
        );
    }
    ...
    {
        return array(array('plugintype' => 'storage', 'plugincode' => 'tasks', 'eventcode' => 'insert'),
                     array('plugintype' => 'storage', 'plugincode' => 'tasks', 'eventcode' => 'soft_delete')
        );
    }
    /** 
    ...
    public function is_access($do, $objid = NULL, $userid = NULL, $depid = null)
    {
        ...
        switch ($do)
        {
            case 'changestatus/owner' : // изменять статус у своей задачи
                {
                    if ( ! empty($objid) )
                    {
                        // Получаем объект из справочника
                        if ( ! $obj = $this->dof->storage($this->get_storage())->get($objid) )
                        {// Объект не найден
                            return false;
                        }
                        if ( empty($userid) )
                        ...
                        if ( $obj->assignedpersonid == $userid ||
                             $obj->assignedpersonid == 0 ||
                             $obj->purchaserpersonid == $userid
                           )
                        {
                            return true;
                        }
                ...
                // Регресс к общему праву смены статуса
                $do = 'changestatus';
                break;
            default:
                break;
    ...
    {
        if ( $gentype === 'storage' AND $gencode === 'tasks' )
        {// Обрабатываем события от своего собственного справочника
            switch ( $eventcode )
            {
                ...
                case 'soft_delete':
                    return $this->change($intvar, 'deleted');
            }
        }
        return true;
    }
    ...
    public function get_available($id)
    {
        ...
        switch ( $obj->status )
        {
            ...
            case 'suspend':
                ...
                break;
            case 'completed':
                $statuses = array('completed' => $this->get_name('completed'),
                                  'failed' => $this->get_name('failed')
                                 );
                break;
            case 'failed':
    ...
    public function acldefault()
    {
        ...
        // Право на смену статуса у своих задач
