...
class block_dof_storage_cstreams_namecstream
{
    ...
    /** 
     * Сохраняет имя учебного процесса в БД
     * 
     * @param int $cstreamid - id предмето-поток
     * 
    ...
    public function save_cstream_name($cstreamid)
    {
        global $DOF;
        $obj = new stdClass();
        $obj->code = $this->get_cstream_name($cstreamid);
        try {
            $obj->name = $DOF->storage('cstreams')->get_cstream_name_by_config($cstreamid);
        } catch ( dof_storage_cstreams_exception $e )
        {// Имя не сгенерировано
        }
        if ( $DOF->storage('cstreams')->update($obj, (int)$cstreamid, true) )
        {
            // инициирование события смены названия учебного процесса
            $DOF->send_event('storage', 'cstreams', 'сhanged_name', $cstreamid);
            return true;
        }
