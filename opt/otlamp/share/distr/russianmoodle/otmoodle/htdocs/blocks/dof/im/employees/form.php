...
class dof_im_employees_appointment_edit_form extends dof_modlib_widgets_form
{
    ...
    function definition()
    {
        ...
        $this->department   = $this->dof->storage('departments')->get($this->_customdata->departmentid);
        ...
        {// Табельный номер создается "с чистого листа"
            ...
            $options['departmentid'] = $this->department->id;
            ...
            $mform->addElement('dof_autocomplete', 'eagreementid',
                    ...
                    ['style' => 'width: 100%; height: auto;'], $options);
        }
...
class dof_im_employees_eagreement_edit_form extends dof_modlib_widgets_form
{
    ...
    function definition()
    {
        ...
        $userselect = $mform->addElement('dof_hierselect', 'userid', 
                $this->dof->get_string('user_type', $this->im_code()).':<br/>'.
                $this->dof->get_string('name_employee', $this->im_code()).':', 'null', '<div class="col-12 px-0"></div>');
    ...
    public function validation($data, $files)
    {
        ...
        {// Назначение сохраняется
            ...
            {// редактирование - переносить нельзя в переполненые
                ...
                if ( ! $this->dof->storage('config')->get_limitobject('eagreements',$data['personid__departmentid'] ) AND $depid != $data['personid__departmentid'] )
                {
                    $errors['departmentid'] = $this->dof->get_string('limit_message','employees');
                }           
            }
