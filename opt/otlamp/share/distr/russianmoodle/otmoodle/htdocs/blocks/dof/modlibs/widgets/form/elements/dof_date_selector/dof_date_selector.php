...
class MoodleQuickForm_dof_date_selector extends MoodleQuickForm_group {
    ...
    /**
     * @var bool - Используется для проверки того, 
     *             нужно ли в селект добавлять год (если изначально его там не было)
     */
    protected $_bool = false;
    ...
    function onQuickFormEvent($event, $arg, &$caller) {
        ...
        switch ($event) {
            case 'updateValue':
                ...
                if (null !== $value) 
                {
                    // Обрабатываем ситуацию, когда defaultyear < startyear || defaultyear > stopyear
                    // Добавляем в селект этот год
                    $elements = $this->getElements();
                    if ( ! empty($elements) && ! $this->_bool )
                    {
                        $select_year = $this->getElements()[2]->_options;
                        if ( isset($value['year']) )
                        {
                            $year = $value['year'];
                            if ( ! empty($select_year) && ! empty($year) )
                            {
                                $bool = false;
                                foreach ($select_year as $select)
                                {
                                    if ( $select['attr']['value'] == $year )
                                    {
                                        $bool = true;
                                        break;
                                    }
                                ...
                                if ( ! $bool )
                                {
                                    $push_year = ['text' => $year, 'attr' => ['value' => $year]];
                                    array_push($this->_elements[2]->_options, $push_year);
                                }
                            }
                        }
                    }
    ...
    /**
     * Обрабатываем пришедшие при создании формы данные
     *
     * @param array $options - пришедшие данные
     *
     * @return void
     */
    private function check_options($options)
    {
        ...
        if ( ! empty($options['stopyear']) )
        {
            $this->_options['stopyear'] = make_timestamp($options['stopyear']);
            if ( $options['stopyear'] === 'currentyear' )
            {// Обрабатываем дату
                $this->_options['stopyear'] = time();
            }
            if ( ! empty($options['offset_stopyear']) )
            {
                $this->_options['stopyear'] = $this->_options['stopyear'] + $options['offset_stopyear'];
            }
            $this->_options['stopyear'] = dof_userdate($this->_options['stopyear'], '%Y');  
        }

        if ( ! empty($options['startyear']) )
        {
            $this->_options['startyear'] = make_timestamp($options['startyear']);
            if ( $options['startyear'] === 'currentyear' )
            {// Обрабатываем дату
                $this->_options['startyear'] = time();
            }
            if ( ! empty($options['offset_startyear']) )
            {
                $this->_options['startyear'] = $this->_options['startyear'] + $options['offset_startyear'];
            }
            $this->_options['startyear'] = dof_userdate($this->_options['startyear'], '%Y'); 
        }
    }
}
