...
class dof_broadcast_result implements Iterator
{
    ...
    public function add($plugintype, $plugincode, $answer)
    {
        ...
        {// Плагин ответил на запрос
            ...
            if ( is_object($answer) && is_a($answer, 'dof_exception') )
            {// Передано исключение
                $result->exception = $answer;
            } else 
            {// Передан ответ от плагина
                $result->result = $answer;
            }
            
            // Дополняем структурированный массив
            $this->result[$plugintype][$plugincode] = $result->result;
