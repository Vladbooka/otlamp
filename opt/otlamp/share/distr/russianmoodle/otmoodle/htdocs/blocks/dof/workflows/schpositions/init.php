...
/**
 * Роутер статусов для вакансий
 *
 * @package    workflow
 * @subpackage schpositions
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_schpositions implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('schpositions' => 2010040200,
                                      'acl'          => 2011082200));
    }
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'schpositions','eventcode'=>'insert'));
    }
    /** 
    ...
    {
		if ( $gentype==='storage' AND $gencode === 'schpositions' AND $eventcode === 'insert' )
		{
    ...
    public function get_available($id)
    {
		...
		switch ( $obj->status )
		{
			case 'plan':       // переход из статуса "запланирован"
				$statuses = array();
				if ( isset($obj->positionid) AND ($position = $this->dof->storage('positions')->get($obj->positionid)) )
				{// если у вакансии есть должность
					if ( isset($position->status) AND ($position->status == 'active')  )
