...
/**
 * Удалить пользовательское достижение
 *
 * @package    im
...
if ( $achievementins->status == 'deleted' )
{// Статус не поддерживается
    $errors[] = $DOF->get_string('error_achievementins_deleted', 'achievements');
}

// Права доступа
$DOF->im('achievements')->require_access('my');
$DOF->storage('achievementins')->require_access('delete', $id);

if ( $confirm && empty($errors) )
{// Удалить достижение
    $errors = $DOF->storage('achievementins')->delete_achievementin($id);
    if ( empty($errors) )
    {// Успех
        // Редирект по ссылке
        redirect($backlink);
...
$addvars['id'] = $id;
$DOF->modlib('nvg')->add_level(
        $DOF->get_string('delete_achievementins', 'achievements'),
        $DOF->url_im('achievements', '/delete_achievementinst.php'), $addvars
);
...
{// Отобразим форму подтверждения
    ...
    $addvars['confirm'] = 1;
    $linkyes = $DOF->url_im('achievements', '/delete_achievementinst.php', $addvars);
    // Языковая строка подтверждения
    $confirmation = $DOF->get_string('confirmation_delete_achievementins', 'achievements');
