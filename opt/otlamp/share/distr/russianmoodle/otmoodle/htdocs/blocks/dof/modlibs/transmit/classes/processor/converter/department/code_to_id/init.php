...
class dof_modlib_transmit_processor_converter_department_code_to_id extends dof_modlib_transmit_processor_base
{
    /**
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        $items = (array)$dof->storage('departments')->get_records(['code' => $input['code']]);
        if ( ! empty($items) )
        {
            return ['id' => key($items)];
        }
