...
class dof_modlib_transmit_processor_converter_programm_code_to_id extends dof_modlib_transmit_processor_base
{
    /**
    ...
    {
        $items = $dof->storage('programms')->get_records(['code' => $input['code']]);
        if ( ! empty($items) )
        {
            return ['id' => key($items)];
        }
