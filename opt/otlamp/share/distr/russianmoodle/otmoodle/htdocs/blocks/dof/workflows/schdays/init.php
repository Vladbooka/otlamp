...
/**
 * Роутер статусов для дней учебных периодов
 *
 * @package    workflow
 * @subpackage schdays
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_schdays implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('schdays' => 2009060800,
                                      'acl'     => 2011082200));
    }
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'schdays','eventcode'=>'insert'));
    }
    /** 
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'schdays' AND $eventcode === 'insert' )
        {
        ...
        switch ( $type )
        {
            ...
            case 'real':
                return array('plan'=>$this->dof->get_string('status:plan',$this->get_storage(),NULL,'workflow'),
                             ...
                             'fixed'=>$this->dof->get_string('status:fixed',$this->get_storage(),NULL,'workflow'));
            case 'junk':                
                return array('deleted'=>$this->dof->get_string('status:deleted',$this->get_storage(),NULL,'workflow'));
            default:
    ...
    public function get_available($id)
    {
        ...
        switch ( $obj->status )
        {
            ...
            break;
            case 'active':       // переход из статуса "создано расписание"
                ...
                $statuses['deleted'] = $this->get_name('deleted');
            break;
            case 'draft':       // переход из статуса "неполное расписание"
                $statuses['plan'] = $this->get_name('plan');
                $statuses['fixed'] = $this->get_name('fixed');
                $statuses['active'] = $this->get_name('active');
                $statuses['draft'] = $this->get_name('draft');
                $statuses['deleted'] = $this->get_name('deleted');
                break;
            case 'completed':  // переход из статуса "отмечены уроки"
                $statuses['plan'] = $this->get_name('plan');
                $statuses['active'] = $this->get_name('active');
                $statuses['fixed'] = $this->get_name('fixed');
                $statuses['draft'] = $this->get_name('draft');
                ...
                $statuses['deleted'] = $this->get_name('deleted');
            break;
            case 'fixed':  // переход из статуса "зафиксирован"
                $statuses = array();
            break;
    ...
    /**
     * Перевод дня в ранее установленное состояние
     *
