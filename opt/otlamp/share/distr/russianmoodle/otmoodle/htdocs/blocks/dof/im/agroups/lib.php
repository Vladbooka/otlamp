 ...
 */
class dof_im_agroups_order_status
{
	/**
     * @var dof_control
     */
    ...
    /** Сформировать приказ об изменении статуса группы
     * 
    ...
    public function save_order_change_status($orderobj)
    {
        ...
        $order = $this->dof->im('agroups')->order('change_status');
    ...
    {
    	//подключаем методы работы с приказом
    	if ( ! $order = $this->dof->im('agroups')->order('change_status',$orderid) )
    ...
    private function get_change_status_fororder()
    {
		...
		$order->agroupid   = $this->gradedata->id;//id периода
