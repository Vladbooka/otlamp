...
$linkyes = $DOF->url_im('persons', '/restore.php?personid='.$personid.'&restore=1',$addvars);
$linkno = $DOF->url_im('persons', '/list.php',$addvars);
if ( $restore )
{// если сказали архивировать
    // Меняем статус персоны
    if ( $DOF->workflow('persons')->change($person->id, 'normal') === false )
...
}else
{
    ...
    $DOF->modlib('nvg')->add_level($DOF->get_string('restore_person', 'persons'),
                                   $DOF->url_im('persons','/restore.php',$addvars));
    ...
    // спросим об удалении
    $DOF->modlib('widgets')->notice_yesno($DOF->get_string('confirmation_restore_person','persons'), $linkyes, $linkno);
    //печать подвала
