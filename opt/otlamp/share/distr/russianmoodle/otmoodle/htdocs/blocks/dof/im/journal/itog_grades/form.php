...
class dof_im_journal_edit_form extends dof_modlib_widgets_form
{
    ...
    {// делаем глобальные переменные видимыми
        ...
        list($statusesselect, $statusesparams) = $this->dof->storage('cpassed')->get_in_or_equal(array_keys($this->dof->workflow('cpassed')->get_register_statuses()));
        $statusesselect = 'cp.status ' . $statusesselect;
        $sql = 'SELECT cp.* FROM {block_dof_s_cpassed} cp
                       JOIN {block_dof_s_persons} p
                         ON p.id = cp.studentid
                      WHERE ' . $statusesselect . ' AND cstreamid = ? AND repeatid IS NULL
                   ORDER BY p.lastname ASC, p.firstname ASC, p.middlename ASC';
        $params = $statusesparams;
        $params[] = $this->cstreamid;
        $cpassed = $this->dof->storage('cpassed')->get_records_sql($sql, $params);
        if ( ! $cpassed )
        ...
        {// подписки есть
            ...
            {// для каждой создадим поле
                ...
                {// ссылка на скачивание ведомости в odt
                    $orderdate = dof_userdate($this->dof->storage('orders')->get_field($successor->orderid, 'exdate'),'%d.%m.%Y');
                    $downloadlink = '&nbsp;&nbsp;&nbsp;<a href="'.$this->dof->url_im('journal', 
                        '/itog_grades/export_order_itoggrades.php', [
                            'type' => 'odf', 
                            'departmentid' => optional_param('departmentid', 0, PARAM_INT),
                            'cstreamid' => $this->cstreamid,
                            'id' => $successor->orderid
                        ]).'">'.
