...
/**
 * Роутер статусов для событий Деканата
 *
 * @package    workflow
 * @subpackage schevents
...
class dof_workflow_schevents implements dof_workflow
{
    ...
    public function get_available($id)
    {
        ...
        switch ( $obj->status )
        {
            ...
            case 'postponed': // переход из статуса "отложено"
                ...
                $statuses['deleted'] = $this->get_name('deleted');
            break;
            case 'implied': // переход из статуса "Подразумевалось"
                $statuses['replaced'] = $this->get_name('replaced');
                $statuses['canceled'] = $this->get_name('canceled');
                $statuses['implied'] = $this->get_name('implied');
                $statuses['deleted'] = $this->get_name('deleted');
    ...
    public function change($id, $newstatus, $options=null)
    {
        ...
        $astatus = $this->dof->storage('appointments')->get_field($object->appointmentid,'status');
        $personid = $this->dof->storage('persons')->get_by_moodleid()->id;
        $aperson = $this->dof->storage('appointments')->get_person_by_appointment($object->appointmentid);
        ...
        switch ($newstatus)
        {
            ...
            case 'completed':
                if ( ! $this->limit_time($object->date) OR ! isset($object->teacherid) OR ! $object->teacherid )
        ...
        $updateobject->salfactor      = $this->calculation_salfactor($id);
        $updateobject->salfactorparts = serialize($this->calculation_salfactor($id, true, true));
        $updateobject->rhours         = $this->calculation_salfactor($id,true);
