 ...
 * 
 */
class dof_im_learningplan implements dof_plugin_im
{
    /**
     * @var dof_control
     */
    ...
    public function require_access($do, $objid = NULL, $userid = NULL)
    {
        ...
        if ( ! $this->is_access($do, $objid, $userid) )
        {
            ...
            $notice = "learningplan/{$do} (block/dof/im/learningplan: {$do})";
    ...
    /** Отобразить основную информацию на странице создания подписок учебного плана
     *  и обработать пришедшие с формы данные
     * 
     * @param string $type
     * @param int $typeid
     * @return void
     */
    public function print_process_unsigned_info($type, $typeid)
    {
        global $addvars;
        // Подключаем формы
        require_once($this->dof->plugin_path('im', 'learningplan', '/form.php'));
        ...
        // Объявляем форму
        $learningplan = new dof_im_learningplan_subscribe_form($this->dof->url_im('learningplan',
                        '/create.php', $addvars + $params), $customdata, 'post', '');
        //            array('onsubmit' => 'setScroll()'));
