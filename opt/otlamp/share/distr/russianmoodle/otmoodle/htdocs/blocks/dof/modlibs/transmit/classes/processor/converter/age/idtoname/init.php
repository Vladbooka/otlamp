...
/**
 * Обмен данных с внешними источниками. Конвертер идентификатора учебного периода в название
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_age_idtoname extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_required = ['age_id' => null];
     ...
     * @var array
     */
    public static $slots_output = ['age_name' => null];
    ...
    {
        $name = $dof->storage('ages')->get_field($input['age_id'], 'name');
        if ( ! empty($name) )
        {
            return ['age_name' => $name];
        }
        return [];
    }
}
