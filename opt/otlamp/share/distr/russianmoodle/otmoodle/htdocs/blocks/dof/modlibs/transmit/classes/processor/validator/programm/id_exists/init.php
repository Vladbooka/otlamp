...
/**
 * Обмен данных с внешними источниками. Валидация идентификатора программы
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_validator_programm_id_exists extends dof_modlib_transmit_processor_base
{
    /**
    ...
    {
        // Поиск периода
        $exists = $dof->storage('programms')->is_exists((int)$input['id']);
        if ( $exists )
        {
            return ['id' => $input['id']];
        }
