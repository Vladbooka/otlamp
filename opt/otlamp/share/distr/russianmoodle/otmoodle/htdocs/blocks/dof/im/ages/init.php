    ...
    function get_block($name, $id = 1)
    {
        ...
        switch ($name)
        {
            ...
            case 'main':
                ...
                if ( $this->dof->storage('ages')->is_access('view',null, null, $addvars['departmentid']) )
                {//может видеть все периоды
                    $path = $this->dof->url_im($this->code(),'/list.php',$addvars);
                }
                //ссылка на список подразделений
                $result .= "<a href=\"{$path}\">".$this->dof->get_string('list', $this->code()).'</a>';
                if ( $this->dof->storage('ages')->is_access('create',null, null, $addvars['departmentid']) )
                {//может создавать период - покажем ссылку
                    $result .= "<br />";
                    $path = $this->dof->url_im($this->code(),'/edit.php',$addvars);
                    $result .= "<a href=\"{$path}\">".$this->dof->get_string('new', $this->code()).'</a>';
                }
            break;
        }
        return $result;
