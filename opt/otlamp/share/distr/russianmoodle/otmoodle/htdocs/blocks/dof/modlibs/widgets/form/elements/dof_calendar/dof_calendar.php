...
class MoodleQuickForm_dof_calendar extends MoodleQuickForm_group
{
    ...
    */
    function __construct($elementName = null, $elementLabel = null, $options=null)
    {
        ...
        GLOBAL $DOF;
        HTML_QuickForm_element::__construct($elementName, $elementLabel, null);
        ...
        $this->dates = ['00.00.00'];// добавляем 0-й день, чтобы не было ошибок
    ...
    function _createElements()
    {
        ...
        $this->_elements[] = $this->createFormElement('html', $this->get_html());
        $this->_elements[] = $this->createFormElement('hidden','date_from',0,array('id'=>'id_'.$this->_elementName.'_from'));
        $this->_elements[] = $this->createFormElement('hidden','date_to',0,array('id'=>'id_'.$this->_elementName.'_to'));
	...
	*	Отрисовка элементов(и черт там голову сломает)
    ...
    /**
     * Формирует html скрипт и подключает необходимые JS файлы
     */
    function get_html()
    {
        ...
        <div id='info_".$this->_elementName."' class='fitem' align=center style='display:none'>
        	<table align='".$this->alignment."' border=0>
       			<tr align='center'>
        ...
        {
            $html .= "<td> <div id='".$this->_elementName."_to'></div></td>";
        }
        $html .= "</tr>
        		<tr align=center>
        ...
        if ( $this->calendartype == 'two_calendar' )
        {
            $html .= "<td> по <span id='".$this->_elementName."_data_to'>   $this->text_to  </span>  </td>";
        }
        $html .= "</tr>
			</table>
		</div></br>
		...
		if ( $this->calendartype == 'two_calendar' )
		{
       	...
       	$js .= '// устанавливаем русский язык
				...
				show_calendar("'.$this->_elementName.'","'.$this->text_from.'","'.$this->text_to.'","'.$this->text_today.'", '.json_encode($this->dates).')

		})});';

        global $DOF;
        $DOF->modlib('nvg')->add_js('modlib', 'widgets', '/form/elements/dof_calendar/dof_calendar.js', false);
        $DOF->modlib('nvg')->add_js_code($js);
    ...
    /**
     * Переопределяем updateValue (сабмит)
     *
    ...
    function onQuickFormEvent($event, $arg, &$caller)
    {
        ...
        switch ($event) {
            case 'updateValue':
                ...
                if ( ! empty($value) )
                {
                    $this->time_unix_from = $value['date_from'];
                    $this->time_unix_to   = $value['date_to'];
                    $this->text_from = date('d.m.Y', $value['date_from']);
                    $this->text_to = date('d.m.Y', $value['date_to']);
                }
                break;
            default:
