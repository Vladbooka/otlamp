...
class dof_im_plans_themeplan_view
{
    ...
    { 
        ...
        {//покажем ссылку на страницу редактирования';
            ...
            $link .= '<a id="view_plan_'.$plan->id.'" href='.$this->dof->url_im('plans','/view.php?pointid='.$plan->id,$conds).'>
                <img src="'.$this->dof->url_im('plans',
                    '/icons/view.png').'" alt="'.$this->dof->modlib('ig')->igs('view').'
                " title="'.$this->dof->modlib('ig')->igs('view').'"></a>&nbsp;';
    ...
    {// находим записи
        ...
        ($this->linktype, $this->linkid,array('draft','active','fixed','excluded','checked','completed'), false, null, false, true);
        if ( ! $plans )
