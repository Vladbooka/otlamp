...
/**
 * Рабочие процессы дополнительных полей
 * 
 * @package    workflow
...
class dof_workflow_customfields implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'customfields' AND $eventcode === 'insert' )
    ...
    {
        return [
                'available'    => $this->dof->get_string('status:available',    'customfields', NULL, 'workflow'),
                'deleted'      => $this->dof->get_string('status:deleted',      'customfields', NULL, 'workflow')
        ];
    }
