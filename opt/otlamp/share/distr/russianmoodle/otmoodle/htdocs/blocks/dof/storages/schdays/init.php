...
class dof_storage_schdays extends dof_storage
{
    ...
    public function update($dataobject,$id = NULL,$quiet=false, $bulk=false)
    {
        ...
        $currentday = $this->get((int)$id);
        if ( empty($currentday) )
        {// Обновляемый день не найден
            return false;
        }
        
        if ( isset($dataobject->departmentid) )
        {// Требуется нормализация подразделения
            // Получение учебного периода для нормализации подразделения
            $ageid = $currentday->ageid;
            if ( ! empty($dataobject->ageid) )
            {
                $ageid = $dataobject->ageid;
            }
            ...
            // Нормализация
            // Изменение запрещено до июля 2017
