...
/**
 * Обмен данных с внешними источниками. Конвертер идентификатора персоны в электронную почту
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_person_id extends dof_modlib_transmit_processor_base
{
    /**
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        $personemail = $dof->storage('persons')->get_field($input['personid'], 'email');
        if ( $personemail )
        {// Персона найдена
