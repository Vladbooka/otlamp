...
$linkyes ='/active.php?id='.$templateid.'&active=1';
$linkno ='/view_week.php';
if ( $active )
{// если сказали удалить - сменим статус
    $DOF->workflow('schtemplates')->change($templateid,'active');
...
}else
{
    ...
    $DOF->modlib('nvg')->add_level($DOF->get_string('active_template', 'schedule'),
                                   $DOF->url_im('schedule','/active.php',$addvars));
    $DOF->modlib('nvg')->print_header(NVG_MODE_PAGE);
    // спросим об удалении
    $DOF->modlib('widgets')->notice_yesno($DOF->get_string('confirmation_active_template','schedule'), 
