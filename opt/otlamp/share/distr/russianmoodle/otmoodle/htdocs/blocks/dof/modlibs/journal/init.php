...
/**
 * Менеджер учебного процесса. Основной класс менеджера.
 * 
 * Менеджер, объединяющий в себе хранилища и роутеры статусов 
 * для реализации бизнес-логики учебного процесса
 * 
 * @package    modlib
 * @subpackage journal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_modlib_journal implements dof_plugin_modlib
{
    /**
    ...
    public function list_catch_events()
    {
        ...
        {
            $list = array_merge($list, $manager->list_catch_events());
        }
    ...
    public function catch_event($gentype,$gencode,$eventcode,$intvar,$mixedvar)
    {
        ...
        foreach ( $this->get_managers() as $manager )
        {
            $result = $manager->catch_event($gentype, $gencode, $eventcode, $intvar, $mixedvar) && $result;
        }
    ...
    public function __construct($dof)
    {
        ...
        require_once $dof->plugin_path('modlib', 'journal', '/classes/exception.php');
        // Базовый класс подсистем
        require_once $dof->plugin_path('modlib', 'journal', '/classes/basemanager.php');
    }
     ...
     *
     * @return dof_modlib_journal_basemanager[]
     */
    public function get_managers()
    {
        if ( empty($this->managers) )
        {
            foreach ( ['lessonprocess', 'schabsenteeism', 'scale'] as $managername )
            {
                ...
                {// Подсистема не найдена
                    ...
                    $path = $this->dof->plugin_path('modlib', 'journal', '/classes/managers/'.$managername.'/init.php');
                    if ( file_exists($path) )
                    {// Подсистема найдена
                        require_once $path;
                        
                        $managername = 'dof_modlib_journal_'.$managername;
