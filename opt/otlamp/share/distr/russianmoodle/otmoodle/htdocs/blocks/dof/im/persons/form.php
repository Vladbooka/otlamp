...
class dof_im_persons_edit_form extends dof_modlib_widgets_form
{
    ...
    protected function show_person_form($code)
    {
        ...
        $genderisoptional = $this->dof->storage('config')->get_config_value(
            'make_person_gender_field_optional',
            'im',
            'persons',
            $this->addvars['departmentid']
        );
        if (empty($genderisoptional)) {
    ...
    function validation($data, $files)
    {
        ...
        {// Валидация полей пользователя
            ...
            // Проверка пола
            if (!empty($this->_customdata->person_gender_field_optional)) {
    ...
    protected function process_person($personcode)
    {
        ...
        {// Передано поле даты рождения персоны
            ...
            {// Нормализация даты рождения
                $savedperson->dateofbirth = (int)$savedperson->dateofbirth;
            }
        } else
        {// Дата рождения не передана
...
class dof_im_persons_email_edit_form extends dof_modlib_widgets_form
{
    ...
    function definition()
    {
        ...
        // Опция сброса пароля и отправки нового пароля пользователю
        $mform->addElement('advcheckbox', 'reset_password', $this->dof->get_string('reset_password','persons'));
        $mform->setDefault('reset_password', 0);
        $mform->setType('reset_password', PARAM_INT);
        if( empty($this->dof->storage('config')->get_config_value(
                    'allow_change_not_auth_dof_users',
                    'storage',
                    'persons',
                    $departmentid)) )
        {// Если настройка в подразделении не включена, блокируем возможность сброса пароля
            $mform->freeze('reset_password');
        }
        // Кнопка "сохранить"
...
class dof_im_person_search_form extends dof_modlib_widgets_form
{
    function definition()
    {
        ...
        //Показывать только персоны в активных статусах
        $mform->addElement('advcheckbox', 'showactive', '', $this->dof->get_string('showactive', 'persons'));
