...
class dof_im_cstreams_edit_form extends dof_modlib_widgets_form
{
    ...
    {// делаем глобальные переменные видимыми
        ...
        $headergroup[] = $mform->createElement(
            'header',
            'header_main',
            $this->dof->get_string('formsave_header_main_label', 'cstreams')
        );
        ...
        $mform->addElement(
            'text',
            'name',
            $this->dof->get_string('formsave_name_label', 'cstreams'),
            ['size' => '60']
        );
        $mform->setType('name', PARAM_RAW_TRIMMED);
        $this->add_help('name', 'formsave_name_label', 'cstreams');
        if ( isset($this->cstream->name) )
        {
            $mform->setDefault('name', $this->cstream->name);
        }
        ...
        $mform->addElement('select', 'programmid', $this->dof->get_string('programm','cstreams').':', $this->get_list_programms(),$options);

        $ajaxoptions = array('plugintype' => 'im','plugincode' => 'cstreams');
        $ajaxoptions['querytype'] = 'list_programmitems';
        $ajaxoptions['parentid'] = 'id_programmid';
        $ajaxoptions['customdata'] = array('cstreamid'=>$this->cstream->id);
        $mform->addElement('dof_ajaxselect', 'programmitemid', $this->dof->get_string('programmitem','cstreams').':', $options, $ajaxoptions);
        if ( $this->cstream->id )
        {
            $itemid = $this->dof->storage('cstreams')->get_field($this->cstream->id,'programmitemid');
            $ajaxoptions['customdata']['programmitemid'] = $itemid;
        }

        $ajaxoptions['querytype'] = 'list_teachers';
        $ajaxoptions['parentid'] = 'id_programmitemid';
        $mform->addElement('dof_ajaxselect', 'appointmentid', $this->dof->get_string('teacher','cstreams').':', null, $ajaxoptions);
        ...
        $mform->addElement('select', 'depid', $this->dof->get_string('department','cstreams').':', $departments);
        $mform->setType('depid', PARAM_INT);
        if ( isset($this->cstream->departmentid) )
        {
            $mform->setDefault('depid', $this->cstream->departmentid);
        }
        if ( ! $this->cstream->id )
        {// создаем поток - делаем зависимым поле подразделения и галочку
            $mform->disabledIf('depid', 'depcheck', 'checked');
        }else
        {// редактируем поток - поле подразделения обязательно
            $mform->addRule('depid',$this->dof->get_string('err_required', 'cstreams'), 'required',null,'client');
        }
        ...
        if ( ! $this->cstream->id )
        {
            $mform->addElement(
                    ...
                    'courselinktypecheck',
                    null,
                    $this->dof->get_string('take_courselinktype_from_pitem','cstreams')
                    );
            $mform->setType('courselinktypecheck', PARAM_BOOL);
            $mform->setDefault('courselinktypecheck', true);

            // Тип связи курса мудл учебного процеса с курсом мудл дисциплины
            $mform->addElement(
                    'select',
                    'courselinktype',
                    $this->dof->get_string('courselinktype', 'cstreams') . ':',
                    $this->courselinktypes,
                    'disabled'
                    );
            $mform->disabledIf('courselinktype', 'courselinktypecheck', 'checked');
        }
        ...
        $headergroup[] = $mform->createElement(
            'header',
            'header_additional',
            $this->dof->get_string('formsave_header_additional_label', 'cstreams')
        );
        $mform->addGroup($headergroup, 'groupheader_studying', '', '', true);

            // Описание учебного процесса
            $group = [];
            $select = [
                0 => $this->dof->get_string('formsave_description_value_programmitem', 'cstreams'),
                1 => $this->dof->get_string('formsave_description_value_custom', 'cstreams')
            ];
            $group[] = $mform->createElement(
                'select',
                'description_value',
                $this->dof->get_string('formsave_description_value_label', 'cstreams'),
                $select
            );
            // Текст описания
            $group[] = $mform->createElement(
                'editor',
                'description_custom',
                $this->dof->get_string('formsave_description_custom_label','cstreams')
            );
            $mform->setType('groupdescription[description_custom]', PARAM_RAW_TRIMMED);
            $mform->disabledIf(
                'groupdescription[description_custom]',
                'groupdescription[description_value]',
                'eq',
                '0'
            );
            $mform->addGroup(
                $group,
                'groupdescription',
                $this->dof->get_string('formsave_description_label', 'cstreams'),
                '<div class="col-12 px-0"></div>',
                true
            );
            $this->add_help('groupdescription', 'formsave_description_label', 'cstreams');
            $mform->setDefault('groupstudentslimit[description_value]', 0);
            $mform->setDefault(
                'groupdescription[description_custom]',
                ['format' => FORMAT_HTML, 'text' => '']
            );
            if ( isset($this->cstream->description) )
            {// Указано собственное описание учебного процесса
                $mform->setDefault('groupdescription[description_value]', 1);
                if ( ! empty($this->cstream->description) )
                {// Установлено описание
                    $mform->setDefault(
                        'groupdescription[description_custom]',
                        ['format' => FORMAT_HTML, 'text' => $this->cstream->description]
                    );
                }
            }
            ...
            $beginopt = [];
            $beginopt['startyear'] = dof_userdate(time(),'%Y')-12;
            $beginopt['stopyear']  = dof_userdate(time(),'%Y')+12;
            $beginopt['optional']  = false;
            $endopt = [];
            $endopt['startyear'] = dof_userdate(time(),'%Y')-12;
            $endopt['stopyear']  = dof_userdate(time(),'%Y')+12;
            $endopt['optional']  = false;
            // Получаем часовой пояс подразделения, к которому относится учебный процесс
            $departmentid = $this->addvars['departmentid'];
            if ( ! empty($cstream->departmentid) )
            {
                $departmentid = $cstream->departmentid;
            }
            $timezone = $this->dof->storage('departments')->get_field($departmentid, 'zone');
            // Добавляем зону к опциям для корректного отображения даты
            $beginopt['timezone'] = $timezone;
            $endopt['timezone'] = $timezone;
            // Добавим установку корректного времени для конца учебного процесса
            $endopt['hours'] = 23;
            $endopt['minutes'] = 55;

            // добавляем сам элемент
            $mform->addElement('date_selector', 'begindate', $this->dof->get_string('begindate', 'cstreams').':', $beginopt);
            ...
            // добавляем сам элемент
            $mform->addElement('date_selector', 'enddate', $this->dof->get_string('enddate', 'cstreams').':', $endopt);

        // Заголовок - Обучение в учебном процессе
        $headergroup = [];
        $headergroup[] = $mform->createElement(
            'header',
            'header_studying',
            $this->dof->get_string('formsave_header_studying_label', 'cstreams')
        );
        $mform->addGroup($headergroup, 'groupheader_studying', '', '', true);

            // Самостоятельная запись
            $params = [
                '' => $this->dof->get_string('formsave_selfenrol_default', 'cstreams'),
                0 => $this->dof->get_string('formsave_selfenrol_disabled', 'cstreams'),
                1 => $this->dof->get_string('formsave_selfenrol_enabled','cstreams'),
                2 => $this->dof->get_string('formsave_selfenrol_enabled_request','cstreams')
            ];
            $mform->addElement(
                'select',
                'selfenrol',
                $this->dof->get_string('formsave_selfenrol_label', 'cstreams'),
                $params
            );
            $this->add_help('selfenrol', 'formsave_selfenrol_label', 'cstreams');
            $mform->setDefault('selfenrol', '');
            if ( isset($this->cstream->selfenrol) )
            {// Указан статус самостоятельной записи в учебный процесс
                $mform->setDefault('selfenrol', (int)$this->cstream->selfenrol);
            }

            // Максимальное количество студентов
            $group = [];
            $select = [
                0 => $this->dof->get_string('formsave_studentslimit_value_programmitem', 'cstreams'),
                1 => $this->dof->get_string('formsave_studentslimit_value_custom', 'cstreams')
            ];
            $group[] = $mform->createElement(
                'select',
                'studentslimit_value',
                $this->dof->get_string('formsave_studentslimit_value_label', 'cstreams'),
                $select
            );
            // Максимальное количество студентов
            $group[] = $mform->createElement(
                'text',
                'studentslimit_custom',
                $this->dof->get_string('formsave_studentslimit_custom_label','cstreams')
            );
            $mform->setType('groupstudentslimit[studentslimit_custom]', PARAM_INT);
            $mform->disabledIf('groupstudentslimit[studentslimit_custom]', 'groupstudentslimit[studentslimit_value]', 'eq', '0');
            $mform->addGroup($group, 'groupstudentslimit', $this->dof->get_string('formsave_studentslimit_label', 'cstreams'), '', true);
            $this->add_help('groupstudentslimit', 'formsave_studentslimit_label', 'cstreams');
            $mform->setDefault('groupstudentslimit[studentslimit_value]', 0);
            $mform->setDefault('groupstudentslimit[studentslimit_custom]', '');
            if ( isset($this->cstream->studentslimit) )
            {// Указан лимит обучающихся студентов
                $mform->setDefault('groupstudentslimit[studentslimit_value]', 1);
                if ( (int)$this->cstream->studentslimit > 0 )
                {// Установлено ограничение
                    $mform->setDefault('groupstudentslimit[studentslimit_custom]', (int)$this->cstream->studentslimit);
                }
            }
    ...
    public function validation($data,$files)
    {
        ...
        // Тип связи курса мудл учебного процеса с курсом мудл дисциплины
        if ( empty($data['courselinktypecheck']) &&
                ! empty($data['courselinktype']) &&
                ! array_key_exists($data['courselinktype'], $this->courselinktypes) )
        {
            $errors['courselinktype'] = $this->dof->get_string('err_courselinktype','cstreams');
        }
        ...
        if ( ! ($this->cstream->id AND $this->cstream->status != 'plan' AND ! $this->dof->is_access('datamanage')) )
        {
            ...
            if ( isset($data['programmitemid']) AND ! $this->dof->storage('programmitems')->is_exists($data['programmitemid']) )
            {// учебное подразделение не существует
                $errors['programmitemid'] = $this->dof->get_string('err_required','cstreams');
            }
        }
        $depid = null;
        ...
        {// если подразделение указано
            ...
            if ( ! isset($data['depid']) OR ! $this->dof->storage('departments')->is_exists($data['depid']) )
            {// учебное подразделение не существует
                $errors['depid'] = $this->dof->get_string('err_required','cstreams');
            }
            $depid = $data['depid'];
        } else
        {// Достанем departmentid из дисциплины
            if ( ! $depid = $this->dof->storage('programmitems')->get_field($data['programmitemid'], 'departmentid') )
            {
                $errors['depid'] = $this->dof->get_string('department_not_found', 'cstreams');
            }
        }
        // Проверим, можно ли создавать в указанном подразделении
        $cstreamid = null;
        if ( $this->cstream->id != 0)
        {
            $cstreamid = $this->cstream->id;
        }
        if ( ! $person = $this->dof->storage('persons')->get_bu() )
        {
            $errors['ageid'] = $this->dof->get_string('err_noperson', 'cstreams');
        }
        // Если нет прав редактировать процессы в указанном подразделении
        if ( ! $this->dof->storage('cstreams')->is_access('edit', null, null, $depid) )
        {
            $errors['depid'] = $this->dof->get_string('err_noaccess_depid', 'cstreams');
        }
        ...
        if ( ! $this->cstream->id )
        {
            if (    isset( $data['depid'] ) AND ! $this->dof->storage('config')->get_limitobject('cstreams',$data['depid']) )
            {
                $errors['depid'] = $this->dof->get_string('limit_message','cstreams');
            }
        }else
        {
            $depid = $this->dof->storage('cstreams')->get_field($this->cstream->id, 'departmentid');
            if ( ! $this->dof->storage('config')->get_limitobject('cstreams',$data['depid']) AND $depid != $data['depid'] )
            {
                $errors['depid'] = $this->dof->get_string('limit_message','cstreams');
            }
        }
    ...
    protected function get_list_teachers($pitemid=null)
    {
        ...
        {// если поток редактируется - и старого учителя нет в списке учителей
            ...
            {//  - добавим его принудительно
                ...
                $enumber = $this->dof->storage('appointments')->
                                 get_field($this->cstream->appointmentid,'enumber');
                $rez[$this->cstream->appointmentid] = "$fullname [$enumber]";
            }else
...
class dof_im_cstreams_search_form extends dof_im_cstreams_edit_form
{
    ...
    function definition()
    {
        ...
        */
        ...
        // поле "замещающий коэффициент"
        $subsalfactor = $this->get_list_subsalfactor();
        $mform->addElement('select', 'subsalfactor', $this->dof->get_string('substsalfactor','cstreams').':', $subsalfactor);
        $mform->setType('substsalfactor', PARAM_TEXT);
        // поле "ученик"
    ...
    */
     ...
     * @return array
     */
    private function get_list_subsalfactor()
    {
        $list = array();
        // добавляем значение, на случай, если по статусу искать не нужно
        $list['none'] = '--- '.$this->dof->modlib('ig')->igs('any_mr').' ---';
        $list[1] = $this->dof->modlib('ig')->igs('present');
        $list[0] = $this->dof->modlib('ig')->igs('absent');
        // возвращаем список всех статусов
        return $list;
    }
}
...
class dof_im_cstreams_create_forprogramm extends dof_modlib_widgets_form
{
    ...
    */
    function definition()
    {
        ...
        if ( $this->programmid )
        {// если значение по умолчанию установлено - запрещаем редактировать поле
            $hselect = &$mform->addElement('dof_hierselect', 'prog_agenum',
                        $this->dof->get_string('program','cstreams').':<br/>'.
                        $this->dof->get_string('parallel','cstreams').':',
                        $disabled, '<div class="col-12 px-0"></div>');
...
class dof_im_cstreams_search_form_by_groups extends dof_modlib_widgets_form
{
    ...
    public function definition()
    {
        ...
        $myselect =& $mform->addElement('dof_hierselect', 'progdata',
