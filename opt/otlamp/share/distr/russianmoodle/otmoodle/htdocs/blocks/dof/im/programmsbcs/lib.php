...
$DOF->modlib('nvg')->add_level(
    $DOF->get_string('title', 'programmsbcs'), 
    $DOF->url_im('programmsbcs','/list.php'), 
    $addvars
);
 ...
 */
class dof_im_programmsbcs_order_status
{
	/**
     * @var dof_control
     */
    ...
    /** Сформировать приказ об изменении статуса подписки на программу
     * 
    ...
    public function save_order_change_status($orderobj)
    {
        ...
        $order = $this->dof->im('programmsbcs')->order('change_status');
    ...
    {
    	//подключаем методы работы с приказом
    	if ( ! $order = $this->dof->im('programmsbcs')->order('change_status',$orderid) )
    ...
    private function get_change_status_fororder()
    {
		...
		$order->programmsbcid = $this->gradedata->id;//id периода
