...
if ( $person = $DOF->storage('persons')->get($clientid) )
{
    $fullname = $DOF->storage('persons')->get_fullname($clientid);
    $html .= $DOF->modlib('widgets')->print_heading(
        $DOF->get_string('recordbook_common_data', 'recordbook', $fullname), '', 2, 'main', 'true');
    $html .= $DOF->im('recordbook')->get_section('list_learning_data', $clientid);
}
