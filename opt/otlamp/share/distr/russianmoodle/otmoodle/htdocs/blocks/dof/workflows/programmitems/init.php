...
/**
 * Роутер статусов для дисциплин
 *
 * @package    workflow
 * @subpackage programmitems
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_programmitems implements dof_workflow
{
    /**
    ...
    {
        return array('storage' => array('programmitems' => 2009060800,
                                        'acl'           => 2011041800));
    }
    ...
    public function get_available($id)
    {
        ...
        switch ( $obj->status )
        {
            case 'active':
                ...
                if ( $this->dof->is_access('datamanage') OR ( !$this->dof->storage('cstreams')->
                        get_records(array('programmitemid' => $obj->id, 'status' => array('plan', 'active', 'suspend')))
                        AND ! $this->dof->storage('programmitems')->
                        get_records(array('metaprogrammitemid' => $obj->id, 'status' => array('active', 'suspend')))) )
                {// если нет активных потоков - можно удалять
    ...
    public function acldefault()
    {
        ...
        $a['changestatus/meta'] = array('roles' => array('manager'));
        return $a;
    }
