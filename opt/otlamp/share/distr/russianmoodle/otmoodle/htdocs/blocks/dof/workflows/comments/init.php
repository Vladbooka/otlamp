...
/**
 * Роутер статусов для комментариев
 *
 * @package    workflow
 * @subpackage comments
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_comments implements dof_workflow
{
    /**
    ...
    {
        if ( $gentype==='storage' AND $gencode === 'comments' AND $eventcode === 'insert' )
    ...
    {
        return array(
                'active'  => $this->dof->get_string('status:active', 'comments', NULL, 'workflow'),
                'deleted' => $this->dof->get_string('status:deleted', 'comments', NULL, 'workflow')
        );
    }
    ...
    public function get_available($id)
    {
        ...
        if ( ! $obj = $this->dof->storage('comments')->get($id) )
        {
            // Объект не найден
            return false;
        }
    ...
    {
        // Получаем объект из comments
        if ( ! $comment = $this->dof->storage('comments')->get($id) )
        {
            // Объект не найден
            return false;
        }
        ...
        return $this->dof->storage('comments')->update($obj);
    }
