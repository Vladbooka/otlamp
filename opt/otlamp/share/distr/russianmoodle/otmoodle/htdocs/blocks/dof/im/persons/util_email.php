...
$form = new dof_im_persons_email_edit_form();

$DOF->modlib('nvg')->print_header(NVG_MODE_PORTAL);
$form->display();
if ($formdata = $form->get_data() AND ! empty($formdata->emails) )
{
    if( ! empty($formdata->reset_password) )
    {// Выставлен флаг на смену пароля
        $options = ['reset_password' => true];
    } else 
    {
        $options = [];
    }
    $emails = preg_split("/[\s,]+/", $formdata->emails);
    // Рисуем таблицу
	$table = new stdClass();
	$table->data = array();
        ...
        if (!$person = $DOF->storage('persons')->get_record(array('email' => $email)) )
        {
        	...
        	if ($objmdluser = $DOF->modlib('ama')->user(false)->get_list(array('email'=>$email)))
        	{
        	    if ( count($objmdluser) == 1 )
        	    {
            		...
            		if (!$personid = $DOF->storage('persons')->reg_moodleuser($objmdluser, false, $options))
            		{
