...
/**
 * Роутер статусов для наборов инвентаря
 *
 * @package    workflow
 * @subpackage invsets
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_invsets implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('invsets' => 2011112400,
                                      'acl'   => 2011062100));
    }
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'invsets','eventcode'=>'insert'));
    }
    /** 
    ...
    {
		if ( $gentype==='storage' AND $gencode === 'invsets' AND $eventcode === 'insert' )
		{
    ...
    public function get_list()
    {
		return array('active'=>$this->dof->get_string('status:active',$this->get_storage(),NULL,'workflow'),
		             ...
		             'granted'=>$this->dof->get_string('status:granted',$this->get_storage(),NULL,'workflow'),
    ...
    public function get_meta_list($type)
    {
        ...
        {
            case 'active':   
                return array('granted'=>$this->dof->get_string('status:granted',$this->code(),NULL,'workflow'));
            case 'actual':
                return array('active'=>$this->dof->get_string('status:active',$this->code(),NULL,'workflow'),
                             'granted'=>$this->dof->get_string('status:granted',$this->code(),NULL,'workflow'),
    ...
    public function get_available($id)
    {
		...
		switch ( $obj->status )
		{
			case 'active':       // переход из статуса "доступен"
				$statuses['granted']  = $this->get_name('granted');
            ...
            break;
            case 'granted':       // переход из статуса "выдан"
				$statuses['active'] = $this->get_name('active');
			break;
            case 'canceled':       // переход из статуса "расформирован"
				$statuses = array();
			break;
