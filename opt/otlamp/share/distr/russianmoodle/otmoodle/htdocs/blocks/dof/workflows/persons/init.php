...
/**
 * Роутер статусов для персон
 *
 * @package    workflow
 * @subpackage persons
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_persons implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('persons' => 2014041000,
                                      'acl'     => 2011041800 )
        );
    }
    /** 
    ...
    public function is_access($do, $objid = NULL, $userid = NULL, $depid = NULL)
    {
        ...
        // Получаем ID персоны по moodle userid
    ...
    public function require_access($do, $objid = NULL, $userid = NULL, $depid = NULL)
    {
        ...
        {// Доступ запрещен, выведем сообщение об этом
    ...
    {
        // Статус normal оставлен вместо active в целях поддержки ранее созданных методов
        return array('normal'  => $this->dof->get_string('status:normal',  $this->code(), NULL, 'workflow'),
                     'archived' => $this->dof->get_string('status:archived', $this->code(), NULL, 'workflow'),
    ...
    public function get_available($id)
    {
        ...
        {
            case 'normal':
                $statuses = array('archived' => $this->get_name('archived'),
                                  'deleted' => $this->get_name('deleted')
                                 );
                break;
            case 'archived':
                $statuses = array('normal' => $this->get_name('normal'),
                                  'deleted' => $this->get_name('deleted')
                                 );
                break;
            case 'deleted':
                $statuses = array();
                break;
            case '': // чтобы была возможность изменить с пустого статуса на активный
                $statuses = array('normal' => $this->get_name('normal'));
                break;
    ...
    public function change($id, $newstatus, $options=null)
    {
        ...
        $mixedvar = new stdClass();
        if( array_key_exists($newstatus, $this->get_meta_list('active')) )
        {//персона переведена в активный статус
            //синхронизация с moodle
            $eventcode = "person_active";
        } else
        {//персона удалена
            //если персона в джанк-статусе (удалена совсем) - не храним пользователя мудл, удаляем
            $mixedvar->keepmdluser = !array_key_exists($newstatus, $this->get_meta_list('junk'));
        ...
        if ( $result )
        {
            ...
            $mixedvar->oldstatus = $object->status;
            $mixedvar->newstatus = $newstatus;
            
            // Кидаем событие
            $this->dof->send_event('workflow', 'persons', $eventcode, $id, $mixedvar);
        }
        return $result;
    }
    ...
    {
        return $this->change((int)$id, 'normal');
    }
