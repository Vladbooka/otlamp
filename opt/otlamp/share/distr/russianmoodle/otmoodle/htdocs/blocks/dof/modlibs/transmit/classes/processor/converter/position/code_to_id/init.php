...
/**
 * Обмен данных с внешними источниками. Конвертер кода должности в идентификатор
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_position_code_to_id extends dof_modlib_transmit_processor_base
{
    /**
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        // получение должности
        $positions = $dof->storage('positions')->get_records(['code' => $input['code'], 'status' => $statuses]);
        if ( ! empty($positions) )
        {
