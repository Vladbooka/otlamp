...
/**
 * Обмен данных с внешними источниками. Конвертер пароля в чистом виде в хеш md5, пригодный для сохранения
 *
 * @package    sync
 * @subpackage transmit
...
class dof_modlib_transmit_processor_converter_person_password extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_required = ['password' => null, 'passwordformat' => null];
    ...
    {
        if ($input['passwordformat'] == 'md5') {
            return ['passwordmd5' => $input['password']];
        } else if ($input['passwordformat'] == 'clear') {
            return ['passwordmd5' => md5($input['password'])];
        }
        return [];
    }
}
