...
/**
 * Справочик итемов классификаторов
 *
 * @package    storage
 * @subpackage taxitems
...
class dof_storage_taxitems extends dof_storage implements dof_storage_config_interface
{
    /**
    ...
    public function install()
    {
        ...
        $result = true;
        $table_classificators = new xmldb_table('block_dof_s_taxitems');
        $table = new xmldb_table($this->tablename());
        if ( $dbman->table_exists($table) && $dbman->table_exists($table_classificators) )
        {
            $classificator = $this->dof->storage('taxonomies')->get_record(['code' => 'programmtypes', 'type' => 'edulevel']);
            if ( ! empty($classificator) )
            {
                $types = [];
                $types['1'] = 'дошкольное образование';
                $types['2'] = 'начальное общее образование';
                $types['3'] = 'основное общее';
                $types['4'] = 'среднее (полное) общее';
                $types['5'] = 'начальное профессиональное';
                $types['6'] = 'среднее профессиональное';
                $types['7'] = 'высшее профессиональное';
                $types['8'] = 'послевузовское профессиональное';
                $types['9'] = 'профессиональная подготовка';
                $types['10'] = 'дополнительное образование';
                
                foreach ( $types as $index => $name )
                {
                    $record = new stdClass();
                    $record->classificatorid = $classificator->id;
                    $record->code = $index;
                    $record->name = $name;
                    $record->parentitemid = 0;
    ...
    {
        return $this->dof->storage('acl')->has_right($acldata->plugintype, $acldata->plugincode, $acldata->code,
    ...
    public function save($data = null, $options = [])
    {
        ...
        {// Обновление записи
            ...
            {// Обновление не удалось
                throw new dof_exception_dml('error_save_classificator_i');
            } else
            {// Обновление удалось
    ...
    public function normalize($data, $options = [])
    {
        ...
        {// Проверка на существование
            ...
            if ( ! empty($data->code) && $this->get_record(['code' => $data->code, 'classificator' => $data->classificatorid]) )
            {
                unset($data->code);
            }
        }
        ...
        if ( empty($data->id) )
        {
            if ( empty($data->classificatorid) || empty($data->code) || empty($data->name) )
            {
                throw new dof_exception_dml('classificator_empty');
            }
            if ( $this->get_record(['classificatorid' => $data->classificatorid, 'code' => $data->code]) )
            {
                throw new dof_exception_dml('classificator_duplicate');
            }
        }
        ...
        if ( ! empty($saveobj->availability) )
        {
            $saveobj->availability = 1;
        } else 
        {
            $saveobj->availability = 0;
        }
        if ( ! empty($saveobj->parentitemid) )
        {
            $saveobj->parentitemid = intval($saveobj->parentitemid);
        } else
        {
            $saveobj->parentitemid = 0;
        }
