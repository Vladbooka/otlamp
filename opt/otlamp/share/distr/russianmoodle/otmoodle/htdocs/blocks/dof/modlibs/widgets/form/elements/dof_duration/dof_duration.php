 ...
 * @package formslib
 */
class MoodleQuickForm_dof_duration extends MoodleQuickForm_group {
   /**
    ...
    *       'availableunits' => array(1 => 'seconds', 60 => 'minutes', 3600 => 'hours') Array of available time units.
    *                           If not specified, days, hours, minutes and seconds is used; 
    ...
    */
    function __construct($elementName = null, $elementLabel = null, $options = array(), $attributes = null)
    {
        ...
        if ( isset($options['availableunits']) )
        {
            if ( ! is_array($options['availableunits']) OR empty($options['availableunits']) )
            {
                throw new dof_exception_coding('Incorrect time units list set in MoodleQuickForm_dof_duration.');
            }
            $this->_options['availableunits'] = $options['availableunits'];
            $this->_units = $options['availableunits'];
        }
        if (isset($options['defaultunit'])) {
            if (!array_key_exists($options['defaultunit'], $this->get_units())) {
                throw new dof_exception_coding($options['defaultunit'] .
                        ' is not a recognised unit in MoodleQuickForm_dof_duration.');
            }
    ...
    //function __construct($elementName = null, $elementLabel = null, $options = array(), $attributes = null)
    //{
    //    $this->MoodleQuickForm_dof_duration($elementName, $elementLabel, $options, $attributes);
    //}
    ...
    function onQuickFormEvent($event, $arg, &$caller) {
        ...
        switch ($event) {
            ...
            case 'createElement':
                if (isset($arg[2]['optional']) and $arg[2]['optional']) {
