...
} else 
{
    ...
    {// Редактирование договора
        ...
        $accessedit = $DOF->storage('contracts')->is_access('edit', $contractid);
        if ( ! $accessedit )
        {
            $DOF->messages->add(
                $DOF->get_string('error_page_contract_edit_access_edit_denied', 'sel'),
                'error'
            );
        }
    } else
    {// Создание договора
        ...
        if ( ! $DOF->storage('contracts')->is_access('create', null, null, $addvars['departmentid']) )
        {// Доступ к созданию договора закрыт
            $DOF->messages->add(
                $DOF->get_string('error_page_contract_edit_access_create_denied', 'sel'),
                'error'
            );
        }
    }
