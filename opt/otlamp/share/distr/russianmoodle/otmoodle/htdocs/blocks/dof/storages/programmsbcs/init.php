...
class dof_storage_programmsbcs extends dof_storage implements dof_storage_config_interface
{
    ...
    /**
     * Получить краткое название подписки на программу
     *
     * @param int|stdClass $contract - Подписка на программу или ID подписки
     *
     * @return null|string - Краткое имя подписки или null в случае ошибки
     */
    public function get_shortname($programmsbc)
    {
        // Получение подписки
        if ( ! is_object($programmsbc) )
        {
            $programmsbc = $this->get((int)$programmsbc);
            if ( empty($programmsbc) )
            {// Подписка не найдена
                return null;
            }
        }
        ...
        $stringvars->programmname = $this->dof->storage('programms')->get_field($programmsbc->programmid, 'name');
        return $this->dof->get_string('programmsbc_shortname', 'programmsbcs', $stringvars, 'storage');
    }
    ...
    public function save($programmsbcdata = null, $options = [])
    {
        ...
        {// Обновление записи
            ...
            if ( ! empty($normalized_data->contractid) && ! empty($normalized_data->programmid) && ! empty($normalized_data->agroupid) )
            {
        ...
        {// Создание записи
            ...
            {// Создание удалось
                ...
                {// Требуется активация объекта
                    $this->dof->send_event('storage', 'programmsbcs', 'activate_request', (int)$id);
                }
                return $id;
            }
        }
                ...
                switch ( $options['notexist_action'] )
                {
                    ...
                    case 'create' :
                        ...
                        {// Обработка академической группы
                            ...
                            if ( isset($options['agroup']) )
                            {// Переданы опции импорта
                                $importoptions = array_merge($importoptions , $options['agroup']);
                            }
