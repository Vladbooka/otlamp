...
class dof_modlib_transmit_processor_converter_contract_num extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_required = ['num' => null];
    ...
    {
        $items = $dof->storage('contracts')->get_records(['num' => $input['num']]);
        if ( ! empty($items) )
        {
            return ['contractid' => key($items)];
        }
        return [];
    }
}
