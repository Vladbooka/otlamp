...
{//кнопки нажаты - обрабатываем их нажатие
	if( isset($_POST['yes']) )
	{//нажата кнопка да
		$rez = $DOF->plugin_uninstall($type, $code);
		if($rez)
		{//плагин удален - сообщаем об этом
			print $DOF->get_string('plugin_delete_true', 'admin', $code);
		}else
		{//плагин не удален - сообщаем об этом
			print $DOF->get_string('plugin_delete_false', 'admin', $code);
		}
...
{//рисуем форму с запросом на выполнение действий
	...
	//спросили что делать
	print $DOF->get_string('delete', 'admin').'&nbsp;'.$DOF->get_string($type, 'admin').'&nbsp;'
