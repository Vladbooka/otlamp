    ...
    public function process()
    {
        if ( $this->is_submitted() AND confirm_sesskey() AND $formdata = $this->get_data() )
        {
            ...
            {// Подразделение не указано - создаем новое
                ...
                // Часовая зона
                $department->zone = $this->dof->storage('departments')->zerotimezone;
            ...
            {// Генерация партнера завершена без ошибок
                ...
                {// Генерация партнера по коду
                    ...
                    if ( ! empty($person->mdluser) )
                    {
                        ...
                        // От личного кабинета ЭД давно ушли, поэтому редиректим на страницу личного кабинета,
                        // где предположительно уже настроен транслируемый блок для партнера
        ...
        {// Код указан
            ...
            */
            ...
            switch ($info['type'])
            {
                ...
                case 'teachercode' :
                    // Тип пользователя
                    $mform->addElement('hidden', 'form_registrtion_person_type', 'teacher');
