...
class dof_im_departments implements dof_plugin_im
{
    ...
    public function catch_event($gentype,$gencode,$eventcode,$intvar,$mixedvar)
    {
        ...
        if( $gentype == 'storage' && $gencode == 'departments' && ($eventcode == 'insert' || $eventcode == 'update') )
        {
            // Очищаем кеш списка подразделений в верхней плашке переключения подразделений
            $cache = $this->dof->get_cache('im', 'departments', 'availabledeps');
            if( $cache !== false )
            {
                $cache->purge();
            }
            
            // Очищаем кеш списка подразделений в левой колонке деканата
                ...
                {// есть права
                    foreach ( $dep as $id=>$objdep )
                    {
                        ...
                        {// есть право смотреть все объекты - покажем это
                            if ( $depid == 0)
                            {
                                $result = "<a href='{$path}' style='color:green; font-size:17px;'>".$this->dof->get_string('see_allobj', 'departments') ."</a><br>".$result;
                            }else 
                            {
                                $result = "<a href='{$path}'>". $this->dof->get_string('see_allobj', 'departments') ."</a><br>".$result;
                            }     
                        }elseif ( $depid == $id)
                        {// ссылка, на которой мы сейчас находимся
                            $result .= "<a href='{$path}' title='{$deptname}' style='color:green; font-size:17px;'>{$objdep}</a><br>";
                        }else 
                        {// все остальные ссылки
                            $result .= "<a href='{$path}' title='{$deptname}'>{$objdep}</a><br>";
                        }  
                    }
                }  
        }
    ...
    public function get_information_section($departmentid = null, $options = [])
    {
        ...
        $hasrightsinmanydeps = false;
        // Получим текущую персону
        $person = $this->dof->storage('persons')->get_bu();
        if( ! empty($person) )
        {
            if( count($this->dof->storage('departments')->get_right_dep($person->id, false)) > 1 )
            {// Если у текущей персоны права есть больше чем в одном подразделении, выведем панель переключения подразделений
                $hasrightsinmanydeps = true;
            }
        }
        
        if( $hasrightsinmanydeps || 
            $this->dof->is_access('datamanage') ||
    ...
    private function block_department_time($departmentid = 0, $options = [])
    {
        ...
        {
            if( ! is_numeric($deptimezone) )
            {
                $deptimezone = dof_get_offset($deptimezone);
            }
        }
    ...
    private function block_department_information_footer($departmentid = 0, $options = [])
    {
        ...
        //Обрезаются ли слишком длинные названия подразделений
        $cutdepname = false;
        foreach ( $subdepartments as $department )
        {
            ...
            {// Есть право на просмотр подразделения
                ...
                {
                    //Если название подразделения > 70 символов, обрезать и добавить многоточие,
                    //показывающее полное название по клику
                    if (mb_strlen($department->name) > 70)
                    {
                        $depnamehead = mb_substr($department->name, 0, 70);
                        $depnametail = mb_substr($department->name, 70);
                        $innerdepname = $depnamehead .
                        dof_html_writer::span($depnametail, 'depnametail hidden');
                        $dots = dof_html_writer::span('...', 'depnamedots');
                        
                        $name = dof_html_writer::span($innerdepname, 'dof_departmens_inf_f_p_name');
                        //Фиксируем, что название подразделений было обрезано
                        $cutdepname = true;
                    } else {
                        $dots = '';
                        $name = dof_html_writer::span($department->name, 'dof_departmens_inf_f_p_name');
                    }
                }
                ...
                $html .= $dots . dof_html_writer::end_div();
            }
        }
        
        //Если длинные названия подразделений были обрезаны, подключить JS для отображения полного названия
        if ($cutdepname){
            $this->dof->modlib('nvg')->add_js('im', 'departments', '/js/departments_showmore.js', false);
        }
    ...
    public function get_departments_select_options($parentdep = 0, $options = [])
    {
        ...
        // Объявим статичный кеш, чтобы получать его один раз, т.к. метод у нас рекурсивный
        static $cache = null;
        $departments = false;
        if( is_null($cache) )
        {// Если первый прогон - получаем объект кеша
            $cache = $this->dof->get_cache('im', 'departments', 'availabledeps');
        }
        // Получаем дочерние подразделения из кеша
        if( $cache !== false )
        {
            $departments = $cache->get($parentdep);
        }
        if( $departments === false )
        {// Если в кеше пусто - лезем в базу
            $departments = $this->dof->storage('departments')->
                get_records(['leaddepid' => $parentdep, 'status' => $options['statuses']], 'code');
            // Выставляем кеш
            if( $cache !== false )
            {
                $cache->set($parentdep, $departments);
            }
        }
        $subdeps = [];
        ...
        if ( isset($options['access_callback']) )
        {// Указан колбэк
            $access = $access && $options['access_callback']($parentdep);
        }
        ...
        {// Право на просмотр подразделения есть, либо есть дочерние доступные подразделения
            ...
            {// Получение кода подразделения
                ...
                if ( ! empty($department) )
                {
                    if ( ! empty($options['name_with_code']) )
                    {
                        $keyval = $department->name . " [{$department->code}]";
                    } else
                    {
