...
/**
 * Роутер статусов для приказов
 *
 * @package    workflow
 * @subpackage orders
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_workflow_orders implements dof_workflow
{
    /**
    ...
    {
        return array('storage'=>array('orders'=>2009050600,
                                       'acl'=>2011040504) );
    }
    ...
    {
        return array(array('plugintype'=>'storage','plugincode'=>'orders','eventcode'=>'insert'));
    }
    /** 
    ...
    {
		if ( $gentype==='storage' AND $gencode === 'orders' AND $eventcode === 'insert' )
		{
    ...
    {
		return array('requested'=>$this->dof->get_string('status:requested',$this->code(),NULL,'workflow'),
					 'executed'=>$this->dof->get_string('status:executed',$this->code(),NULL,'workflow'),
		             'canceled'=>$this->dof->get_string('status:canceled',$this->code(),NULL,'workflow'),
		             'archive'=>$this->dof->get_string('status:archive',$this->code(),NULL,'workflow'));
    }
    ...
    {
		// Получение объекта из orders
		if ( ! $obj = $this->dof->storage('orders')->get($id) )
		{// Объект не найден
			return false;
		}
		// Определение возможного состояния в зависимости от текущего статуса
		switch ( $obj->status )
		{
		    // Переход из статуса "заказан"
			case 'requested':       
				$statuses = ['executed' => $this->get_name('executed'), 'canceled' => $this->get_name('canceled')];
                break;
            // Переход из статуса "завершен"
            case 'executed':  
                $statuses = ['archive' => $this->get_name('archive')];
                break;
            // Переход из статуса "отменен"
            case 'canceled':  
                $statuses = [];
                break;
            // Переход из статуса "архивирован"
            case 'archive':  
                $statuses = [];
                break;
            default: $statuses = ['requested' => $this->get_name('requested')];
		}
    ...
    /** 
     * Возвращает массив состояний, доступных для установки вручную пользователем,
     * в которые может переходить объект из текущего состояния 
     *  
     * @param int $id - ID объекта
     * 
     * @return mixed|array - Массив возможных состояний или false
     */
    public function get_available_to_user($id)
    {
		// Получаем объект из orders
		if ( ! $obj = $this->dof->storage('orders')->get($id) )
		{// Объект не найден
			return false;
		}
        ...
        // Убеждаемся, что нет ошибки и в статус возможен переход
		return array_intersect($statuses, $this->get_available($id));
