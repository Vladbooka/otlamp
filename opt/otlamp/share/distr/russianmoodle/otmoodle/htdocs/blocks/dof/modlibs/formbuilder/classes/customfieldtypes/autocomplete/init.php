...
include_once(dirname(realpath(__FILE__)).'/../select/init.php');
/**
 * Менеджер построения форм
 *
 * Класс дополнительного поля типа autocomplete
 *
 * @package    modlib
 * @subpackage formbuilder
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dof_customfields_autocomplete extends dof_customfields_select
{
    /**
    ...
    public function create_element(&$mform, $objectid, $beforeelement = null, $options = [])
    {
        ...
        // Получение доступных значений autocomplete
        $autocompleteoptions = $this->get_autocompleteoptions();
        ...
        if(!empty($additionaloptions['noselectionstring'])) {
            $selectattrs['noselectionstring'] = $additionaloptions['noselectionstring'];
        }
        // Инициализация поля
        if (empty($beforeelement)) {
            $element = $mform->addElement('autocomplete', $elementname, $customfield->name, $autocompleteoptions, $selectattrs);
        } else {
            $element = $mform->createElement('autocomplete', $elementname, $customfield->name, $autocompleteoptions, $selectattrs);
        ...
        if ((int)$objectid > 0) {
            try {
                ...
                // устанавливать значение по умолчанию если нет сохраненного значения при этом есть
                // активна настройка использовать значения по умолчанию
                if (!($this->customfield->defaultvalue == $saveddata
                    && empty($additionaloptions['use_default'])))
                {
                ...
                if (isset($customfield->defaultvalue) && !empty($additionaloptions['use_default'])) {
    ...
    public function render_data($objectid, $options = [])
    {
        ...
        $options = $this->get_autocompleteoptions();
     ...
     * @return array
     */
    private function get_autocompleteoptions()
    {
        $autocompleteoptions = [];
        if (!empty($this->get_options()['autocompleteoptions'])) {
            // Определены пункты выпадающего списка
            $autocompleteoptions = (array)$this->get_options()['autocompleteoptions'];
        }
     ...
     */
    public function get_autocompleteoption($optiontext)
    {
        // Получение опций выпадающего списка
        $autocompleteoptions = $this->get_autocompleteoptions();

        $opton = array_search((string)$optiontext, (array)$autocompleteoptions);
        if ( $opton === false )
        {// Значение не найдено
            return null;
        }
    ...
    {
        // Базовая валидация
        $errors = dof_customfields_base::validate_data($formvalue, $objectid);
        ...
        $selectoptions = $this->get_autocompleteoptions();
    ...
    public function saveform_definition(dof_modlib_widgets_form &$form, MoodleQuickForm &$mform)
    {
        ...
        dof_customfields_base::saveform_definition($form, $mform);
        ...
        $mform->addElement(
            'textarea',
            'autocompleteoptions',
            $this->dof->get_string('saveform_autocompleteoptions_title', 'formbuilder', null, 'modlib')
        );
        $mform->setType('autocompleteoptions', PARAM_RAW_TRIMMED);
        $form->add_help('autocompleteoptions', 'saveform_autocompleteoptions_help', 'formbuilder', 'modlib');
        ...
        $additionaloptions = $this->get_additional_options();
        $multiple = ! empty($additionaloptions['multiple']);
        $usedefault = ! empty($additionaloptions['use_default']);
        $selectattrs = [];
        ...
        $mform->addElement(
            'checkbox',
            'use_default',
            $this->dof->get_string('saveform_use_default', 'formbuilder', null, 'modlib'),
            ''
            );
        $mform->setType('use_default', PARAM_BOOL);
        $mform->disabledIf('use_default', 'multiple');
        ...
        if ($customfield) {
            ...
            // Установка значения по умолчанию
            if ($usedefault || !$multiple) {
                $mform->setDefault('use_default', true);
            }
            // Список доступных элементов
            $options = $this->get_options();
            if (!empty($options['autocompleteoptions'])) {
                // Указан список
                $textoptions = (string)implode("\n", $options['autocompleteoptions']);
                $mform->setDefault('autocompleteoptions', $textoptions);
            }
        }
    }
    ...
    {
        dof_customfields_base::saveform_subprocess($form, $mform, $formdata);
        // Значение по умолчанию
        ...
        if (!empty($formdata->use_default) || empty($formdata->multiple)) {
            $additionaloptions['use_default'] = true;
        }
        // Список доступных элементов
        $this->customfield->options['additional_options'] = $additionaloptions;
        // Список доступных элементов
        $this->customfield->options['autocompleteoptions'] =
            explode("\n", $formdata->autocompleteoptions);
    }
}