...
$sql = 'assignedpersonid = :assignedpersonid';
$params['assignedpersonid'] = 0;
...
// Печатаем вкладки
echo $DOF->im('crm')->print_tab($addvars, 'tasks', 'freetasks');

// Добавляем пагинацию к GET параметрам
$addvars['limitfrom'] = $limitfrom;
// Выводим пагинацию
echo $pages->get_navpages_list('/tasks/freetasks.php', $addvars).'<br />';

// Выводим таблицу
$DOF->im('crm')->print_table_task($items, 'freetasks', $addvars);

// Выводим пагинацию
echo '<br />'.$pages->get_navpages_list('/tasks/freetasks.php', $addvars);
