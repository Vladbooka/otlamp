...
class dof_modlib_refbook implements dof_plugin_modlib
{
    ...
    {// подключаем файл
        ...
        {
            $items = $this->dof->storage('taxitems')->get_records(['classificatorid' => $classificator->id]);
            $new_items = [];
            if ( ! empty($items) )
            {
                foreach ( $items as $item )
                {
                    $new_items[$item->code] = $item->name;
                }
            }
            
            if ( ! empty($edulevel) )
            {
                if ( array_key_exists($edulevel, $new_items) )
                {
                    return $new_items[$edulevel];
                }
            } else 
            {
                return $new_items;
    ...
    /** 
     * Типы связи с курсом мудл при создании учебного процесса
     * 
     * @return array
     */
    public function get_courselinktypes($type = '')
    {
        $path = $this->dof->plugin_path('modlib', 'refbook', '/standards/courselinktypes.php');
        if ( ! file_exists($path) )
        {//если файла нет - сообщим об этом
            $this->dof->print_error('file_not_found', $path);
        }
        //файл есть - подключаем файл
        include($path);
        if ( isset($string) AND is_array($string) )
        {//данные на месте
            if ( ! empty($type) && array_key_exists($type, $string) )
            {
                return $string[$type];
            } else 
            {
