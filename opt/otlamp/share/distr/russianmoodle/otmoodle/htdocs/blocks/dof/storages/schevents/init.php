...
class dof_storage_schevents extends dof_storage implements dof_storage_config_interface
{
    ...
    public function prepare_calculation_data($schevent, $options = [])
    {
        ...
        $studentscount = empty($historycpasseds) ? 0 : count($historycpasseds);
     ...
     * @return array
     */
    public function get_mass_date($cstreamid,$stevent,$stplan,$emevent=true,$showall=true)
    {
        ...
        {
            // выборка для events по условию
            $select = 'cstreamid=:cstreamid AND status IN (' . implode(',', array_map(function ($str)
                                    {
                                        return "'" . $str . "'";
                                    }, $stevent)) . ') AND date>=:date';
            $params = [
                'cstreamid' => $cstreamid,
                'date' => time() - 60 * 60 * 24 * 30
            ];
            $rezevent = $this->get_records_select($select, $params);
        }
        // выборка КТ
        $rezplan = $this->dof->storage('plans')->get_theme_plan('cstreams', $cstreamid, $stplan, true, '1', false, $showall);
   ...
   public function get_time_list($conds)
    ...
    /**
     * Получить массив событий по условиям
     *
     * @param stdClass $conditions - условия
     *                              status - массив допустимых статусов события
     *                              cstreamsstatus - массив допустимых статусов учебного процесса
     *                              begintime - начальная дата для выборки даты события
     *                              endtime - конечная дата для выборки даты события
     *
     * @param string $order - sql-код сортировки
     *
     * @return array - результат выборки, включающий programmitemid, полученный через учебный процесс
     */
    public function get_events($conditions, $order='sch.date ASC')
    {
        // таблицы участвующие в выборке
        $tbl = $this->prefix().$this->tablename();
        $tblcstreams = $this->prefix().$this->dof->storage('cstreams')->tablename();
        ...
        // условие для фильтрации событий по статусу
        if( ! empty($conditions->status) && is_array($conditions->status))
        {
            list($sqlsubcondition, $sqlsubparams) = $this->where_clause_list(
                'sch.status',
                $conditions->status
            );
            $sqlconditions[] = '('.$sqlsubcondition.')';
            foreach($sqlsubparams as $sqlsubparam)
            {
                $sqlparams[] = $sqlsubparam;
            }
        }
        // условие для фильтрации событие по статусу учебного процесса
        if( ! empty($conditions->cstreamsstatus) && is_array($conditions->cstreamsstatus) )
        {
            list($sqlcondition, $sqlparam) = $this->where_clause_list(
                'cs.status',
                $conditions->cstreamsstatus
            );
            $sqlconditions[] = '('.$sqlsubcondition.')';
            foreach($sqlsubparams as $sqlsubparam)
            {
        ...
        // условия для ограничения выборки по датам
        if ( ! empty($conditions->begintime) AND ! empty($conditions->endtime) )
        {// времеенной интервал
            // формируем дату начала дня
            $dateday = getdate($conditions->begintime);
            $time_from = $this->dof->storage('persons')->get_make_timestamp(
                0, 0, 0, $dateday['mon'], $dateday['mday'], $dateday['year']
            );
            $dateday = getdate($conditions->endtime);
            $time_to = $this->dof->storage('persons')->get_make_timestamp(
                0, 0, 0, $dateday['mon'], $dateday['mday'], $dateday['year']
            );
            $sqlconditions[] = 'sch.date >= '.$time_from;
            $sqlconditions[] = 'sch.date < '.($time_to + 86400);
        }

        // выборка событий по сформированным условиям
        return $this->get_records_sql("SELECT sch.*, cs.programmitemid as programmitemid
            FROM {$tbl} as sch
                INNER JOIN {$tblcstreams} as cs ON sch.cstreamid=cs.id
            WHERE
    ...
    public function get_events_intersection_place($eventid = 0, $datestart = 0, $datestop = 0, $place = '')
    {
        ...
        // Статусы событий для проверки занятости кабинетов
        $eventstatuses = array_keys($this->dof->workflow('schevents')->get_meta_list('real'));

        //Запишем названия таблиц, из которых будем доставать данные
        $schevents = $this->prefix() . $this->tablename();

        // Составляем запрос
        $sql = "SELECT e.*
                FROM $schevents as e WHERE e.place = '$place'";
        if ( ! empty($eventid) )
        {
            $sql .= " AND e.id <> $eventid";
        }
        // учет времени
        $sql .= " AND ((e.date > $datestart AND e.date < $datestop) OR ((e.date+e.duration) > $datestart AND (e.date+e.duration) < $datestop)
                    OR (e.date = $datestart AND (e.date+e.duration) = $datestop) OR (e.date < $datestop AND (e.date+e.duration) > $datestart))";
        $sql .= " AND e.status IN ('" . implode("','", $eventstatuses) . "')";
