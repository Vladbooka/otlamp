...
class dof_modlib_transmit_processor_converter_general_exploder extends dof_modlib_transmit_processor_base
{
    /**
     ...
     * @var array
     */
    public static $slots_output = ['explodeddata' => null];
    ...
    public static function execute($input, $dof, $logger, $filemanager)
    {
        ...
        $exploded = (array)@explode("$delimiter", $input['data']);
        
        if ( $exploded )
        {// Данные разбиты по частям
            return ['explodeddata' => $exploded];
        }
        return [];
    }
}