...
//        $mform->addElement('static', 'edutype', $this->dof->get_string('edutype',$this->im_code()).':', $edutype);
    ...
    private function show_reoffsets()
    {
        ...
        // Header вкладки перезачетов
        $mform->addElement('header', 'reoffsets', $header);
        $mform->setExpanded('reoffsets');

        // Добавляем кнопку "Перезачесть дисциплины", если тип плана - подписка на программу
        if ( $this->type == 'programmsbc' )
        {
            $link = dof_html_writer::link(
                    $this->dof->url_im('cpassed', '/register.php', $this->addvars + array('programmsbcid' => $this->typeid)),
                    $this->dof->get_string('reoffsetstodo',$this->im_code()),
                    ['class' => 'btn button dof_button']
                    );
            $buttonadd = dof_html_writer::tag('div', $link);
            $mform->addElement('html', $buttonadd);
        }
