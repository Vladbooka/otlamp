...
{// переводим доверенность в активный статус
    $DOF->print_error('not_found_warrant', '', $aclwarrantid, 'im', 'acl');
}
if ( $warrant->parenttype != 'sub' )
{// переводим доверенность в активный статус
    $DOF->print_error('error_active_warrant', '', $aclwarrantid, 'im', 'acl');
}

// ссылки на подтверждение и непотдверждение сохранения приказа
$linkyes ='/activewarrant.php?aclwarrantid='.$aclwarrantid.'&delete=1';
$linkno ='/warrantacl.php?id='.$aclwarrantid;

if ( $delete )
{// если сказали удалить - сменим статус
    $obj = new stdClass();
    $opt['changestatuswa'] = true;
    $DOF->workflow('aclwarrants')->change($aclwarrantid, 'active', $opt);
...
}else
{
    ...
    $DOF->modlib('nvg')->add_level($DOF->get_string('active_warrant', 'acl'),
                                   $DOF->url_im('acl','/activewarrant.php',$addvars));
    ...
    // спросим об удалении
    $DOF->modlib('widgets')->notice_yesno($DOF->get_string('confirmation_active_warrant','acl'), $DOF->url_im('acl',$linkyes,$addvars),
