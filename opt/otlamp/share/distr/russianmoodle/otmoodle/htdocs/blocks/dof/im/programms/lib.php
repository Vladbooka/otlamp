 ...
 */
class dof_im_programms_order_status
{
	/**
     * @var dof_control
     */
    ...
    public function save_order_change_status($orderobj)
    {
        ...
        $order = $this->dof->im('programms')->order('change_status');
    ...
    {
    	//подключаем методы работы с приказом
    	if ( ! $order = $this->dof->im('programms')->order('change_status',$orderid) )
    ...
    private function get_change_status_fororder()
    {
		...
		$order->programmid = $this->gradedata->id;//id периода
