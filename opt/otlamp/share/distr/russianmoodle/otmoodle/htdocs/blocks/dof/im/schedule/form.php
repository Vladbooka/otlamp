...
class dof_im_schedule_edit_schetemplate_form extends dof_modlib_widgets_form
{
    ...
    public function definition()
    {
        ...
        // Подразделение
        $mform->addElement('static', 'department', 
            $this->dof->get_string('department', $this->im_code()),
            $this->dof->storage('departments')->get_field($this->_customdata->departmentid, 'name'). '['.
            $this->dof->storage('departments')->get_field($this->_customdata->departmentid, 'code').']');
    ...
    function validation($data,$files)
    {
        ...
        if ( isset($data['cstreamid']) AND $data['cstreamid'] )
        {
            ...
            {
                $errors[$csname] = $this->dof->get_string('error:age_not_found', $this->im_code());
            }
    ...
    public function process()
    {
        ...
        if ( $this->is_submitted() AND confirm_sesskey() AND $formdata = $this->get_data() )
        {
            ...
            $cstream = $this->dof->storage('cstreams')->get($formdata->cstreamid);
            $age = $this->dof->storage('ages')->get($cstream->ageid);
            // id подразделения 
            $dbobj->departmentid = $age->departmentid;
            ...
            $template = $this->dof->storage('schtemplates')->get($id);
            
            // Проверка пересечений по кабинету
            if ( $this->dof->storage('schtemplates')->get_select_templater_places($template) )
            {
                $add['editinfo'] = 'notice:template_has_place_intersection';
            }
    ...
    {
        return $this->dof->storage('ages')->get_list_daynums($this->_customdata->ageid, true);
    }
...
class dof_im_schedule_display_mode_form extends dof_modlib_widgets_form
{
    ...
    {
        // @todo брать из workflow
        $ages = $this->dof->storage('ages')->get_records(
            [
                'status' => [
                    'plan', 
                    'createstreams', 
    ...
    protected function get_display_modes()
    {
        return array(
                ...
                'place' => $this->dof->get_string('display_mode:place', $this->im_code()));
    }
...
class dof_im_schedule_create_event_form extends dof_modlib_widgets_form
{
    ...
    public function definition()
    {
        ...
        // Массив доступных учебных периодов
        $ages = $this->get_ages();
        // Готовим массив номеров учебных дней
        $daynum = array();
        foreach ( $ages as $id => $agename )
        {
            $daynum[$id] = $this->get_week_days($id, true);
        }
        // Добавим зависимость выбора дня недели от периода
        $complex = $mform->addElement('dof_hierselect', 'complex',
                $this->dof->get_string('age', $this->im_code()).':<br/>'.
                $this->dof->get_string('daynum', $this->im_code()).':', null, '<div class="col-12 px-0"></div>');
        $complex->setOptions(array($ages, $daynum));
     ...
     *                 для которого получаем число дней в учебной неделе
     * 
     * @return array - массив алиасов для номеров дней учебной недели
     */
    protected function get_week_days($ageid)
    {
        // Вернем массив дней для учебного периода
        return $this->dof->storage('ages')->get_list_daynums($ageid, true);
    }    
    ...
    public function process()
    {
        ...
        $formdata->ageid = $formdata->complex[0];
        $formdata->daynum = $formdata->complex[1];
        ...
        {// создаем один день
            ...
            {// день не существует, или нам нужно обновить расписание
                ...
                if ( ! $dayid = $this->dof->storage('schdays')->save_day($formdata) )
                ...
                {// при сохранении шаблонов возникли ошибки
                    $this->dof->workflow('schdays')->change($dayid, 'draft');
                    $errordays[] = $this->get_templates_errors($templateids);
                } else 
                {
                    $this->dof->workflow('schdays')->change($dayid, 'active');
                }
            }
        }
        ...
        {// создаем расписание на неделю
            // получим дни недели
            $days = $this->get_week_days($formdata->ageid);
            ...
            foreach ( $days as $num => $day )
            {
                ...
                if ( ! $dayid = $this->dof->storage('schdays')->save_day($dayobj) )
                ...
                $templateids = $this->create_events_day(
                     $dayobj->ageid,$num,$dayobj->dayvar,$dayid,$dayobj->departmentid,$implied);
    ...
    protected function delete_entire_day($formdata, $num = null)
    {
        ...
        $schdays = $this->dof->storage('schdays')->get_records_select(
            "ageid={$formdata->ageid} AND date={$date} AND 
             departmentid={$formdata->departmentid} AND status IN ('active','holiday','fixed', 'plan')");
        ...
        {
            $deleteresult = $this->dof->storage('schdays')->delete_entire_day($schday->id);
...
class dof_im_schedule_change_begindate_form extends dof_modlib_widgets_form
{
    /**
     * @var dof_control
     */
    ...
    public function definition()
    {
        ...
        $mform =& $this->_form;
        $this->dof = $this->_customdata->dof;
        $this->ageid = $this->_customdata->ageid;
        
        $mform->addElement('static', 'caption', '', $this->dof->get_string('build_replaces_grid', 'schedule'));
        
        $fields[] = $mform->createElement(
            'dof_duration',
            'oldbegindate_hours',
            '',
            [
                'optional' => false,
                'defaultunit' => 3600,
                'availableunits' => [3600 => get_string('hours')]
            ]
        );
        
        $fields[] = $mform->createElement(
            'dof_duration',
            'oldbegindate_minutes',
            '',
            [
                'optional' => false,
                'defaultunit' => 60,
                'availableunits' => [60 => get_string('minutes')]
            ]
        );
        ...
        $fields[] = $mform->createElement(
            ...
            'delimiter',
            '',
            ' ' . $this->dof->get_string('change_begin_time', 'schedule') . ' '
        );
        ...
        $repeatarray[] = $mform->createElement(
            'group',
            'dur',
            $this->dof->get_string('begin_time', 'schedule'),
            $fields,
            null,
            false
        );
        ...
        // настройки полей
        $repeateloptions = [];
        $repeateloptions['oldbegindate_hours']['type'] = PARAM_INT;
        $repeateloptions['oldbegindate_minutes']['type'] = PARAM_INT;
        $repeateloptions['newbegindate_hours']['type'] = PARAM_INT;
        $repeateloptions['newbegindate_minutes']['type'] = PARAM_INT;
        $repeateloptions['delimiter']['type'] = PARAM_TEXT;
    ...
    function validation($data,$files)
    {
        ...
        {
            $old = $data['oldbegindate_hours[' . $i . ']'] + $data['oldbegindate_minutes[' . $i . ']'];
            $new = $data['newbegindate_hours[' . $i . ']'] + $data['newbegindate_minutes[' . $i . ']'];
            if( ($data['oldbegindate_hours[' . $i . ']'] < 0) ||
                ($data['oldbegindate_minutes[' . $i . ']'] < 0) ||
                ($old > 24 * 60 * 60) || 
                ($data['oldbegindate_minutes[' . $i . ']'] > 3600) )
            {
                $err[] = $this->dof->get_string('wrong_old_begin_date', 'schedule');
            }
            if( ($data['newbegindate_hours[' . $i . ']'] < 0) ||
                ($data['newbegindate_minutes[' . $i . ']'] < 0) ||
                ($new > 24 * 60 * 60) || 
                ($data['newbegindate_minutes[' . $i . ']'] > 3600) )
            {
                $err[] = $this->dof->get_string('wrong_new_begin_date', 'schedule');
            }
        }
        if( ! empty($err) )
        {
            $errors['caption'] = implode('<br/>', $err);
        }
        // Возвращаем ошибки, если они есть
        return $errors;
    }
    ...
    public function process()
    {
        ...
        {
            if( $this->dof->storage('schtemplates')->is_access('edit') )
            {
                $replaces = new stdClass();
                $replaces->replaces = [];
                for($i = 0; $i < $formdata->option_repeats; $i++)
                {
                    $old = $formdata->{'oldbegindate_hours['.$i.']'} + $formdata->{'oldbegindate_minutes['.$i.']'};
                    $new = $formdata->{'newbegindate_hours['.$i.']'} + $formdata->{'newbegindate_minutes['.$i.']'};
                    if( $old > 0 || $new > 0)
                    {
                        $obj = new stdClass();
                        $obj->old = $old;
                ...
                if( ! empty($replaces->replaces) && $this->ageid > 0 )
                {
                    if( $this->dof->add_todo('storage', 'schtemplates', 'change_begin_date_in_period', $this->ageid, $replaces, 2, time()) )
                    {
                        $this->dof->messages->add($this->dof->get_string('todo_was_added', 'schedule'), DOF_MESSAGE_SUCCESS);
                    } else
                    {
                        $this->dof->messages->add($this->dof->get_string('todo_was_not_added', 'schedule'), DOF_MESSAGE_ERROR);
                    }
                }
            } else 
            {
                $this->dof->messages->add($this->dof->get_string('permission_denied', 'schedule'), DOF_MESSAGE_ERROR);
            }
        }
    }
}
