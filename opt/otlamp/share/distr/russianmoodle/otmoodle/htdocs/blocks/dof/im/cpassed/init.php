...
class dof_im_cpassed implements dof_plugin_im
{
    ...
    public function config_default($code = NULL)
    {
        ...
        // Активность системы оповещения пользователей
        $obj = new stdClass();
        $obj->type = 'checkbox';
        $obj->code = 'request_notification_enable';
        $obj->value = '0';
        $config[$obj->code] = $obj;
    
        // Текст сообщения по умолчанию (подтверждено)
        $obj = new stdClass();
        $obj->type = 'textarea';
        $obj->code = 'request_notification_confirm_default';
        $obj->value = $this->dof->get_string(
            'config_cpassed_request_notification_confirm_default', 'cpassed');
        $config[$obj->code] = $obj;
        
        // Текст сообщения по умолчанию (отклонено)
        $obj = new stdClass();
        $obj->type = 'textarea';
        $obj->code = 'request_notification_reject_default';
        $obj->value = $this->dof->get_string(
            'config_cpassed_request_notification_reject_default', 'cpassed');
        $config[$obj->code] = $obj;
...
//                $rez .= "<br />";
                if ( $this->is_access('viewall', null, null, $addvars['departmentid']) )
                {//может видеть все классы
                    $path = $this->dof->url_im('cpassed','/list.php',$addvars);
                }
                //ссылка на список подписок на курсы
                $result .= "<a href=\"{$path}\">".$this->dof->get_string('list', 'cpassed').'</a>';
                if ( $this->is_access('addcpassed', null, null, $addvars['departmentid']) )
                {//может создавать подписку на курс - покажем ссылку
                    $result .= "<br />";
                    $path = $this->dof->url_im('cpassed','/edit.php',$addvars);
                    $result .= "<a href=\"{$path}\">".$this->dof->get_string('new', 'cpassed').'</a>';
                }
                break;
            default:
                break;
    ...
    public function form_listeditor($addvars = [], $options = [])
    {
        ...
        // Генерация формы
        $form = new dof_im_cpassed_listeditor_form($url, $customdata);
    ...
    * @param array $options - массив опций ['fullview'=>bool]
    ...
    public function render_cstream_cpasseds_tabs($tabname, $addvars = [])
    {
        ...
        $canuse = $this->dof->storage('cstreams')->is_access('use', $addvars['cstreamid']);
        $canviewdesk = $this->dof->storage('cstreams')->is_access('viewdesk', $addvars['cstreamid']);

        // Подписки на дисциплину
        if ( $canuse || $canviewdesk )
        {
        ...
        // Добавление подписок на дисциплины
        if ( $canuse )
        {
    ...
    /**
     * Генерация секции самоподписки в дисциплины
     * 
     * @param int $personid - ID персоны
     * 
     * @return string - HTML-код секции
     */
    public function section_selfenrol()
    {
        // Подключение класса формы
        require_once($this->dof->plugin_path('im', 'cpassed', '/form.php'));
        ...
        $this->dof->modlib('nvg')->add_css('im', 'cpassed', '/styles.css');
        
        // Определение целевой персоны
        $targetperson = $this->dof->storage('persons')->get_bu(null, true);
        
        // Таблица учебных процессов преподавателя
        $html = $this->table_selfenrol_teacher($targetperson->id);
        
        $url = $this->dof->url_im('cpassed', '/selfenrol.php');
        // Инициализация формы записи
        $customdata = new stdClass();
        $customdata->dof = $this->dof;
        $customdata->returnurl = $this->dof->modlib('nvg')->get_currentpage_url();
        $form = new dof_im_cpassed_selfenrol_form($url, $customdata);
        // Обработка формы
        $form->proccess();
        if ( $form->hascontent )
        {
    ...
    /**
     * Генерация таблицы учебных процессов преподавателя
     *
     * @param int $teacherid - ID персоны преподавателя
     *
     * @return string - HTML-код таблицы
     */
    public function table_selfenrol_teacher($teacherid)
    {
        $html = '';
        
        // Инициализация менеджера самозаписи
        $selfenrolmanager = $this->dof->modlib('learningplan')->get_manager('selfenrol');
        
        // Получение учебных процессов преподавателя с открытой самозаписью
        $teachercstreams = $selfenrolmanager->get_teacher_cstreams_by_teacherid($teacherid);
        
        // Отображение таблиц для каждой из дисциплин
        foreach ( $teachercstreams as $programmitemid => $cstreams )
        {
            // Отображение имени дисциплины
            $programmitemname = $this->dof->storage('programmitems')->
                get_field($programmitemid, 'name');
            $html .= dof_html_writer::tag('h4', $programmitemname);
            ...
            foreach ( $cstreams as $cstreamid => $cstream )
            {
                ...
                // Получение ссылки на интерфейс просмотра подписок
                $link = dof_html_writer::link(
                    $this->dof->url_im('cpassed', '/listeditor.php', ['cstreamid' => $cstreamid]),
                    $this->dof->get_string('table_selfenrol_teacher_cpassedlist_link', 'cpassed'),
                    ['class' => 'btn bytton dof_button']
                );
                $data[] = $link;
                
                // Название учебного процесса
                $data[] = $this->dof->storage('cstreams')->get_name($cstream);
                ...
                // Количество мест
                $cell = '';
                $countrequest = $selfenrolmanager->cstream_count_requests($cstream);
                if ( $countrequest )
                {// Найдены заявки на запись
                    $cell .= dof_html_writer::div(
                        $this->dof->get_string('table_selfenrol_teacher_new_requests', 'cpassed')
                    );
                }
                $leftenrols = $selfenrolmanager->cstream_count_enrols_left($cstream);
                if ( $leftenrols === false )
                {// Неограниченно
                    $cell .= $this->dof->get_string('table_selfenrol_teacher_leftenrolslots_unlimit', 'cpassed');
                } else
                {// Установлено ограничение
                    $a = new stdClass;
                    $a->left = $leftenrols;
                    $cell .= $this->dof->get_string('table_selfenrol_teacher_leftenrolslots', 'cpassed', $a);
                }
                $data[] .= $cell;
                $table->data[] = $data;
            }
