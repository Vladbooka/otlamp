...
/**
 * Роутер статусов для подписок на программы
 *
 * @package    workflow
 * @subpackage programmsbcs
...
class dof_workflow_programmsbcs implements dof_workflow
{
    ...
    public function change($id, $newstatus, $options = null)
    {
        ...
        if ( in_array($newstatus, ['onleave','completed','failed','rejected','canceled']) )
        {
            $updateobject->dateend = time();
        }    
        $result = $result AND $storage->update($updateobject);
        ...
        {// выполняем необходимые действия, связанные со сменой статуса
            ...
            case 'condactive':
                ...
                if ( $object->status == 'plan' OR $object->status == 'suspend' )
                {
                    if ( $sbclist = $this->dof->storage('cpassed')->get_records([
                            'programmsbcid'=>$id, 
                        ...
                        foreach ( $sbclist as $cpassed )
                        {
                            ...
                            switch ( $csstatus )
                            {
                                case 'active':
                                    ...
                                    $result = $result AND $this->dof->workflow('cpassed')->change($cpassed->id, 'active');
                                break;
                                case 'suspend':
                                    // если ее поток приостановлен 
                                    if ( $cpassed->status == 'plan' )
                                    {// запланированные подписки тоже приостановим
                                        $result = $result AND $this->dof->workflow('cpassed')->change($cpassed->id, 'suspend');
                                    }
                                break;
                            }
                        }
            ...
            case 'suspend':
                ...
                {// если мы получили записи - изменим их статус
                    foreach ( $sbclist as $cpassed )
                    {
                        // Получение предмето-классов по подписке 
                        $csstatus = $this->dof->storage('cstreams')->get_field($cpassed->cstreamid,'status');
                        switch ( $csstatus )
                        {
                            case 'active':
                            ...
                            case 'suspend':
                                ...
                                $result = $result & $this->dof->workflow('cpassed')->change($cpassed->id, 'suspend');
                            break;
                        }
                    }
                }
            ...
            case 'canceled':
                ...
                // должны перейти в статус "активные"
                $sbclist = $this->dof->storage('cpassed')->get_records([
                    'programmsbcid'=>$id, 
                    'status'=> [
                        'plan', 
                        'active', 
                ...
                {// если мы получили записи - изменим их статус
                    ...
                    {// пербираем все запланированные подписки на предметы, и перевозим каждую в статус "активна"
                        $result = $result AND $this->dof->storage('cpassed')->set_final_grade($sbc->id);
                    }
                }                
                break;
        }
