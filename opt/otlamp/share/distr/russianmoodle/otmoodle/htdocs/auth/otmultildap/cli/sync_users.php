...
if (!is_enabled_auth('otmultildap')) {
    error_log('[AUTH otmultildap] '.get_string('pluginnotenabled', 'auth_otmultildap'));
    die;
}

cli_problem('[AUTH otmultildap] The users sync cron has been deprecated. Please use the scheduled task instead.');

// Abort execution of the CLI script if the auth_otmultildap\task\sync_task is enabled.
$taskdisabled = \core\task\manager::get_scheduled_task('auth_otmultildap\task\sync_task');
if (!$taskdisabled->get_disabled()) {
    cli_error('[AUTH otmultildap] The scheduled task sync_task is enabled, the cron execution has been aborted.');
}

$ldapauth = get_auth_plugin('otmultildap');
