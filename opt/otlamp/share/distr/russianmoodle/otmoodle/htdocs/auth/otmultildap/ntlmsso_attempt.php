...
if (!in_array('otmultildap', $authsequence, true)) {
    print_error('ldap_isdisabled', 'auth');
}

$authplugin = get_auth_plugin('otmultildap');
if (empty($authplugin->config->ntlmsso_enabled)) {
    print_error('ntlmsso_isdisabled', 'auth_otmultildap');
}
...
$msg = '<p>'.get_string('ntlmsso_attempting', 'auth_otmultildap').'</p>'
    . '<img width="1", height="1" '
    . ' src="' . $CFG->wwwroot . '/auth/otmultildap/ntlmsso_magic.php?sesskey='
    . $sesskey . '" />';
redirect($CFG->wwwroot . '/auth/otmultildap/ntlmsso_finish.php', $msg, 3);
