         ...
         if ($newinfo = $this->get_userinfo($username)) {
             ...
             if (!empty($updatekeys)) {
                 ...
                 foreach (profile_get_user_fields_with_data($userid) as $formfield) {
                     $profilefields[$formfield->field->shortname] = $formfield;
                 }
                 $newprofilefields = [];
                 ...
                 foreach ($updatekeys as $key) {
                     ...
                     if (!empty($this->config->{'field_updatelocal_' . $key})) {
                         if (preg_match('/^profile_field_(.*)$/', $key, $match)) {
                             // Custom field.
                             $fieldshortname = $match[1];
                             
                             if (array_key_exists($fieldshortname, $profilefields))
                             {
                                 /** @var profile_field_base $profilefield */
                                 $profilefield = $profilefields[$fieldshortname];
                                 
                                 $currentvalue = $profilefield->data ?? null;
                                 $currentvalue = is_null($currentvalue) ? null : (string)$currentvalue;
                                 
                                 $newvalue = $profilefield->edit_save_data_preprocess($newvalue, new stdClass);
                                 $newvalue = $newvalue ?? $profilefield->field->defaultdata;
                                 $newvalue = is_null($newvalue) ? null : (string)$newvalue;
                             }
                             
                             $newprofilefields[$fieldshortname] = $newvalue;
                             
                         } else {
                             // Standard field.
                             $currentvalue = $user->$key ?? null;
