 ...
 */
class auth_plugin_otmultildap extends auth_plugin_base {
    
    /**
     * Код плагина для получения конфигов
     * 
     * @var string
     */
    ...
    /**
     * Установка конфигов и значений по умолчанию для конфигурационного файла
     * 
     * @param string $code
     */
    function postinit_plugin($code) {
        $this->config = $this->prepare_config_by_ldapcode(get_config($this->pluginconfig), $code);
    ...
    public function __construct() {
        $this->authtype = 'otmultildap';
        $this->roleauth = 'auth_otmultildap';
        $this->errorlogtag = '[AUTH OTMULTILDAP] ';
     ...
     */
    public function auth_plugin_otmultildap() {
    ...
    function user_login($username, $password) {
        if (! function_exists('ldap_bind')) {
            print_error('auth_otmultildapnotinstalled', 'auth_otmultildap');
            return false;
        }
        ...
        $code = $this->get_user_code($user);
        if (!$code) {
            return false;
        }
        // установка конфигурации LDAP-сервера
        $this->postinit_plugin($code);
        ...
        if (!empty($this->config->ntlmsso_enabled) && $key === $password) {
            ...
            if ($username === $sessusername) {
                ...
                $validuser = false;
                $ldapconnection = $this->ldap_connect($code);
     ...
     */
    function get_userinfo($username, $code = '') {
        ...
        $attrmap = $this->ldap_attributes($code);
     ...
     */
    function get_userinfo_asobj($username, $code) {
        $user_array = $this->get_userinfo($username, $code);
        if ($user_array == false) {
    ...
    function get_userlist() {
        $code = $this->get_configuration_code_activeregister();
        if (!$code) {
            return false;
        }
        $this->postinit_plugin($code);
    ...
    function user_create($userobject, $plainpass) {
        ...
        // получение кода конфигурации сервера LDAP
        $code = $this->get_configuration_code_activeregister();
        if (!$code) {
            return false;
        }
        ...
        switch ($this->config->user_type)  {
            ...
            case 'ad':
                ...
                if (!function_exists('mb_convert_encoding')) {
                    print_error('auth_otmultildap_no_mbstring', 'auth_otmultildap');
                }

                // Check for invalid sAMAccountName characters.
                if (preg_match('#[/\\[\]:;|=,+*?<>@"]#', $extusername)) {
                    print_error ('auth_otmultildap_ad_invalidchars', 'auth_otmultildap');
                }
                ...
                if (!ldap_add($ldapconnection, $userdn, $newuser)) {
                    print_error('auth_otmultildap_ad_create_req', 'auth_otmultildap');
                }
                ...
                break;
            default:
               print_error('auth_otmultildap_unsupportedusertype', 'auth_otmultildap', '', $this->config->user_type_name);
        }
        $this->ldap_close();
        return $uadd;
    }
    ...
    function user_signup($user, $notify=true) {
        ...
        if ($this->user_exists($user->username)) {
            print_error('auth_otmultildap_user_exists', 'auth_otmultildap');
        }
        ...
        if (! $this->user_create($user, $plainslashedpassword)) {
            print_error('auth_otmultildap_create_error', 'auth_otmultildap');
        }
        ...
        if (! send_confirmation_email($user)) {
            print_error('noemail', 'auth_otmultildap');
        }

        if ($notify) {
            $emailconfirm = get_string('emailconfirm');
            $PAGE->set_url('/auth/otmultildap/auth.php');
    ...
    function password_expire($username) {
        ...
        // получение кода конфигурации сервера LDAP, по которому пользователь синхронизирован в СДО
        $code = $this->get_user_code(core_user::get_user_by_username($username));
        if (!$code) 
        {
            return false;
        }
        ...
        if ($sr)  {
            ...
            error_log($this->errorlogtag.get_string('didtfindexpiretime', 'auth_otmultildap'));
        }
    ...
    function sync_users($do_updates=true) {
        ...
        $codes = $this->get_ldap_codes(get_config($this->pluginconfig));
        
        $dbman = $DB->get_manager();
        
        // синхронизация LDAP по каждой конфигурации
        foreach ($codes as $code) {
            try
            {
                $this->postinit_plugin($code);
                print_string('connectingldap', 'auth_otmultildap', $code);
                echo "\n";
                
                try {
                    // попытка открыть соединение с LDAP сервером
                    $ldapconnection = $this->ldap_connect($code);
                } catch ( Exception $e ) 
                {
                    echo  $e->getMessage(), "\n";
                    // неудачная попытка, пропустим конфигурацию
                    continue;
                }
            ...
            /// Define table user to be created
                ...
                print_string('creatingtemptable', 'auth_otmultildap', 'tmp_extuser');
                echo "\n";
                ...
                if ($count < 1) {
                    print_string('didntgetusersfromldap', 'auth_otmultildap');
                    echo "\n";
                    $dbman->drop_table($table);
                    $this->ldap_close();
                    continue;
                } else {
                    print_string('gotcountrecordsfromldap', 'auth_otmultildap', $count);
                    echo "\n";
                }
        ...
        /// User removal
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                    $sql = "SELECT u.*
                         ...
                         LEFT JOIN {user_preferences} up ON u.id = up.userid
                             WHERE u.auth = :auth
                                   ...
                                   AND up.name = 'ldap_configuration_code'
                                   AND up.value = :ldap_configuration_code";
                    $remove_users = $DB->get_records_sql($sql, array('auth'=>$this->authtype, 'ldap_configuration_code' => $code));
        
                    if (!empty($remove_users)) {
                        print_string('userentriestoremove', 'auth_otmultildap', count($remove_users));
                        echo "\n";
                        foreach ($remove_users as $user) {
                            try {
                                $string = "\t" . get_string('auth_dbdeleteuser', 'auth_db', array('name'=>$user->username, 'id'=>$user->id)) . "\n";
                                echo $string;
                                delete_user($user);
                            } catch ( Exception $e )
                            {
                                $errstring = ' - ' . get_string('auth_dbrecord_fail', 'auth_otmultildap', $e->getMessage()) . "\n";
                                echo $errstring;
                                $errdeleteusers[$user->username] = $string . $errstring;
                            }
                        }
                    } else {
                        print_string('nouserentriestoremove', 'auth_otmultildap');
                        echo "\n";
                    }
                    ...
                    if (!empty($remove_users)) {
                        ...
                        foreach ($remove_users as $user) {
                            try {
                                $string = "\t" . get_string('auth_dbsuspenduser', 'auth_db', array('name'=>$user->username, 'id'=>$user->id)) . "\n";
                                echo $string;
                                ...
                                echo ' - ' . get_string('auth_dbrecord_success', 'auth_otmultildap', $user->id) . "\n";
                            } catch ( Exception $e )
                            {
                                $errstring = ' - ' . get_string('auth_dbrecord_fail', 'auth_otmultildap', $user->id) . "\n";
                                echo $errstring;
                                $errtosuspendusers[$user->username] = $string . $errstring;
                            }
                        }
                    } else {
                        print_string('nouserentriestoremove', 'auth_otmultildap');
        ...
        /// Revive suspended users
                if (!empty($this->config->removeuser) and $this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    $sql = "SELECT u.id, u.username
                              ...
                              LEFT JOIN {user_preferences} up ON u.id = up.userid
                             WHERE (u.auth = 'nologin' OR (u.auth = :auth AND u.suspended = 1)) AND u.deleted = 0 
                                   AND up.name = 'ldap_configuration_code'
                                   AND up.value = :ldap_configuration_code";
                    // Note: 'nologin' is there for backwards compatibility.
                    $revive_users = $DB->get_records_sql($sql, array('auth'=>$this->authtype, 'ldap_configuration_code' =>$code));
        
                    if (!empty($revive_users)) {
                        print_string('userentriestorevive', 'auth_otmultildap', count($revive_users));
                        echo "\n";
        
                        foreach ($revive_users as $user) {
                            try {
                                $string = "\t" . get_string('auth_dbreviveduser', 'auth_db', array('name'=>$user->username, 'id'=>$user->id)) . "\n";
                                echo $string;
                                $updateuser = new stdClass();
                                $updateuser->id = $user->id;
                            ...
                            } catch ( Exception $e )
                            {
                                $errstring =  ' - ' . get_string('auth_dbrecord_fail', 'auth_otmultildap', $user->id) . "\n";
                                echo $errstring;
                                $errtoreviveusers[$user->username] = $string . $errstring;
                            }
                        }
                    } else {
                        print_string('nouserentriestorevive', 'auth_otmultildap');
                        echo "\n";
                    }
        ...
        /// User Updates - time-consuming (optional)
                if ($do_updates) {
                	...
                	print_string('noupdatestobedone', 'auth_otmultildap');
            	}
                ...
                if ($do_updates and !empty($updatekeys)) { // run updates only if relevant
                    $users = $DB->get_records_sql("SELECT u.username, u.id
                                                     ...
                                                     WHERE u.deleted = 0 AND u.auth = ? AND u.mnethostid = ?
                                                         AND up.name = 'ldap_configuration_code'
                                                         AND up.value = ?",
                                array($this->authtype, $CFG->mnet_localhost_id, $code));
                    if (!empty($users)) {
                        print_string('userentriestoupdate', 'auth_otmultildap', count($users));
                        echo "\n";
                        ...
                        foreach ($users as $user) {
                            try {
                                $string = "\t" . get_string('auth_dbupdatinguser', 'auth_db', array('name'=>$user->username, 'id'=>$user->id));
                                $transaction = $DB->start_delegated_transaction();
    							$userinfo = $this->get_userinfo($user->username);
                                if (!$this->update_user_record($user->username, $updatekeys, true, $this->is_user_suspended((object) $userinfo))) {
                                    $string .= ' - '.get_string('skipped');
                                }
                                echo $string;
                                $xcount++;
                            ...
                            } catch ( Exception $e )
                            {
                                ...
                                $errstring =  ' - ' . get_string('auth_dbrecord_fail', 'auth_otmultildap', $e->getMessage()) . "\n";
                                echo $errstring;
                                $errupdateusers[$user->username] = $string . $errstring;
                            }
                        }
        ...
        /// User Additions
                ...
                if (!empty($add_users)) {
                    print_string('userentriestoadd', 'auth_otmultildap', count($add_users));
                    echo "\n";
        
        
                    foreach ($add_users as $user) {
                        try {
                            $string = "\t" . get_string('auth_dbinsertuser', 'auth_otmultildap', $user->username);
                            $transaction = $DB->start_delegated_transaction();
                            $user = $this->get_userinfo_asobj($user->username, $code);
    						...
    						// установка кода конфигурации в свойства пользователя, чтобы знать
                            // с какого сервера был синхронизирован пользователь и корректно его обрабатывать
                            set_user_preference('ldap_configuration_code', $code, $id);
                        ...
                        } catch ( Exception $e )
                        {
                            ...
                            $errstring = ' - ' . get_string('auth_dbrecord_fail', 'auth_otmultildap', $e->getMessage()) . "\n";
                            echo $errstring;
                            $erraddusers[$user->username] = $string . $errstring;
                        }
                    }
                    unset($add_users); // free mem
                } else {
                    print_string('nouserstobeadded', 'auth_otmultildap');
                    echo "\n";
                }
                ...
                $errupdateusers = [];
                $erraddusers = [];
                // вывод ошибок по текущей конфигурации
                if ( ! empty($errdeleteusers) ||
                        ! empty($errtosuspendusers) || 
                        ! empty($errtoreviveusers) || 
                        ! empty($errupdateusers) || 
                        ! empty($erraddusers) ) 
                {
                    // во время выполнения были ошибки
                    print_string('errors_while_sync', 'auth_otmultildap');
                    echo "\n"; 
                    
                    if ( ! empty($errdeleteusers) )
                    {
                        // ошибки во время удаления пользователей
                        print_string('errors_while_delete_users', 'auth_otmultildap');
                        echo "\n";
                        foreach ( $errdeleteusers as $errmessage )
                        {
                            echo $errmessage;
                        }
                    }
                    if ( ! empty($errtosuspendusers) )
                    {
                        // ошибки во время приостановки пользователей
                        print_string('errors_while_suspend_users', 'auth_otmultildap');
                        echo "\n";
                        foreach ( $errtosuspendusers as $errmessage )
                        {
                            echo $errmessage;
                        }
                    }
                    if ( ! empty($errtoreviveusers) )
                    {
                        // ошибки во время перевода пользователей из приостановленного в активный
                        print_string('errors_while_activate_users', 'auth_otmultildap');
                        echo "\n";
                        foreach ( $errtoreviveusers as $errmessage )
                        {
                            echo $errmessage;
                        }
                    }
                    if ( ! empty($errupdateusers) )
                    {
                        // ошибки во время обновления пользователей
                        print_string('errors_while_update_users', 'auth_otmultildap');
                        echo "\n";
                        foreach ( $errupdateusers as $errmessage )
                        {
                            echo $errmessage;
                        }
                    }
                    if ( ! empty($erraddusers) )
                    {
                        // ошибки во время обновления пользователей
                        print_string('errors_while_add_users', 'auth_otmultildap');
                        echo "\n";
                        foreach ( $erraddusers as $errmessage )
                        {
                            echo $errmessage;
                        }
                    }
     ...
     */
     function update_user_record($username, $updatekeys = false, $triggerevent = false, $suspenduser = false) 
     {
        ...
        return parent::update_user_record($username, $updatekeys, $triggerevent, $suspenduser);
    }
    ...
    function ldap_bulk_insert($username) {
        ...
        $record = $DB->get_record('tmp_extuser', ['username' => $username, 'mnethostid' => $CFG->mnet_localhost_id]);
        if ( ! empty($record) )
        {
            return;
        }
    ...
    function user_activate($username) {
        ...
        switch ($this->config->user_type)  {
            ...
            case 'ad':
                ...
                break;
            default:
                print_error('user_activatenotsupportusertype', 'auth_otmultildap', '', $this->config->user_type_name);
        }
    ...
    function user_update($olduser, $newuser) {
        ...
        // получение кода конфигурации сервера LDAP, по которому пользователь синхронизирован в СДО
        $code = $this->get_user_code($olduser);
        if (!$code) 
        {
            return false;
        }
        ...
        if (isset($olduser->username) and isset($newuser->username) and $olduser->username != $newuser->username) {
            error_log($this->errorlogtag.get_string('renamingnotallowed', 'auth_otmultildap'));
            return false;
        }
        ...
        if ($user_info_result) {
            ...
            if (empty($user_entry)) {
                ...
                error_log($this->errorlogtag.get_string('updateusernotfound', 'auth_otmultildap',
                                                          array('userdn'=>$user_dn,
                                                                'attribs'=>$attribs)));
                return false; // old user not found!
            } else if (count($user_entry) > 1) {
                error_log($this->errorlogtag.get_string('morethanoneuser', 'auth_otmultildap'));
                return false;
            }
                ...
                if ($newvalue !== null and $newvalue !== $oldvalue and !empty($this->config->{'field_updateremote_' . $key})) {
                    ...
                    foreach ($ldapkeys as $ldapkey) {
                        ...
                        if (!isset($user_entry[$ldapkey][0])) {
                            $success = false;
                            error_log($this->errorlogtag.get_string('updateremfailfield', 'auth_otmultildap',
                        ...
                        if (!$ambiguous) {
                            ...
                            if ($nuvalue !== $ldapvalue) {
                                ...
                                if (@ldap_modify($ldapconnection, $user_dn, array($ldapkey => $nuvalue))) {
                                    ...
                                    $success = false;
                                    error_log($this->errorlogtag.get_string ('updateremfail', 'auth_otmultildap',
                    ...
                    if ($ambiguous and !$changed) {
                        $success = false;
                        error_log($this->errorlogtag.get_string ('updateremfailamb', 'auth_otmultildap',
                                                                 array('key'=>$key,
                                                                       'ouvalue'=>$ouvalue,
                                                                       'nuvalue'=>$nuvalue)));
                    }
            ...
            error_log($this->errorlogtag.get_string ('usernotfound', 'auth_otmultildap'));
            $success = false;
        }
    ...
    function user_update_password($user, $newpassword) {
        ...
        if (!$user_dn) {
            error_log($this->errorlogtag.get_string('nodnforusername', 'auth_otmultildap', $user->username));
            return false;
        }
        ...
        switch ($this->config->user_type) {
            case 'edir':
                ...
                if (!$result) {
                    error_log($this->errorlogtag.get_string ('updatepasserror', 'auth_otmultildap',
                                                               array('errno'=>ldap_errno($ldapconnection),
                                                                     'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                }
                ...
                if ($sr) {
                    ...
                    if (!empty($info[$this->config->expireattr][0])) {
                        ...
                        if (!$result) {
                            error_log($this->errorlogtag.get_string ('updatepasserrorexpiregrace', 'auth_otmultildap',
                                                                       array('errno'=>ldap_errno($ldapconnection),
                                                                             'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                        }
                    }
                    ...
                    error_log($this->errorlogtag.get_string ('updatepasserrorexpire', 'auth_otmultildap',
                                                             array('errno'=>ldap_errno($ldapconnection),
                                                                   'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                }
                break;
            ...
            case 'ad':
                ...
                // double quotes. See http://support.microsoft.com/?kbid=269190
                if (!function_exists('mb_convert_encoding')) {
                    error_log($this->errorlogtag.get_string ('needmbstring', 'auth_otmultildap'));
                    return false;
                }
    ...
    function ldap_expirationtime2unix ($time, $ldapconnection, $user_dn) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'ad':
                ...
                break;
            default:
                print_error('auth_otmultildap_usertypeundefined', 'auth_otmultildap');
        }
        return $result;
    }
    ...
    function ldap_unix2expirationtime($time) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'rfc2307bis':
                ...
                break;
            default:
                print_error('auth_otmultildap_usertypeundefined2', 'auth_otmultildap');
        }
        return $result;
    ...
    function ldap_attributes($code = '') 
    {
        $moodleattributes = array();
        // If we have custom fields then merge them with user fields.
        $customfields = $this->get_custom_user_profile_fields();
        $userfields = $this->get_user_profile_fields();
        ...
        $newmoodleaatributes = [];
        foreach ( $moodleattributes as $field => $val )
        {
            $fieldcheck = explode('___', $field);
            if ( array_key_exists(1, $fieldcheck) )
            {
                $newmoodleaatributes[$fieldcheck[0]] = $val;
            } else 
            {
                $newmoodleaatributes[$field] = $val;
            }
        }
        $newmoodleaatributes['username'] = core_text::strtolower(trim($this->config->user_attribute));
        $newmoodleaatributes['suspended'] = core_text::strtolower(trim($this->config->suspended_attribute));
        return $newmoodleaatributes;
    }
    ...
    function loginpage_hook() {
        ...
        if (($_SERVER['REQUEST_METHOD'] === 'GET'         // Only on initial GET of loginpage
            ...
            if($this->config->ntlmsso_ie_fastpath == AUTH_NTLM_FASTPATH_YESATTEMPT ||
                ...
                if (core_useragent::is_ie()) {
                    $sesskey = sesskey();
                    redirect($CFG->wwwroot.'/auth/otmultildap/ntlmsso_magic.php?sesskey='.$sesskey);
                } else if ($this->config->ntlmsso_ie_fastpath == AUTH_NTLM_FASTPATH_YESFORM) {
                    redirect($CFG->httpswwwroot.'/login/index.php?authldap_skipntlmsso=1');
                }
            }
            redirect($CFG->wwwroot.'/auth/otmultildap/ntlmsso_attempt.php');
        }
        ...
        if (empty($SESSION->wantsurl)
            && (get_local_referer() == $CFG->wwwroot.'/auth/otmultildap/ntlmsso_finish.php')) {
        ...
        if (isset($_SERVER['REMOTE_USER']) && !empty($_SERVER['REMOTE_USER'])) {
            ...
            switch ($this->config->ntlmsso_type) {
                ...
                case 'kerberos':
                    ...
                    break;
                default:
                    error_log($this->errorlogtag.get_string ('ntlmsso_unknowntype', 'auth_otmultildap'));
    ...
    /**
     * Метод получение списка конфигураций/формы настроек
     *
     ...
     */
    function config_form($fullconfig, $err, $user_fields) {
        global $CFG, $OUTPUT;

        if (!function_exists('ldap_connect')) { // Is php-ldap really there?
            echo $OUTPUT->notification(get_string('auth_otmultildap_noextension', 'auth_otmultildap'));
            return;
        }
        
        // код конфигурации
        $ldapcodedirty = optional_param('ldapcode', null, PARAM_RAW);
        $ldapcode = optional_param('ldapcode', null, PARAM_ALPHANUMEXT);
        // флаг удаления конфигурации
        $deleteldapcode = optional_param('delete', null, PARAM_BOOL);
        
        if ( (strlen($ldapcode) === 0 && ! is_null($ldapcodedirty)) || $ldapcode != $ldapcodedirty )
        {
            $ldapcode = null;
            // передан неверный код LDAP конфига
            echo $OUTPUT->notification(get_string('invalid_ldapcode', 'auth_otmultildap'));
        }
        
        if (is_null($ldapcode))
        {// передан пустой код, отобразим список существующих конфигураций
            
            // получение основной конфигурации, по которой ведется регистрация пользователей из moodle в LDAP
            $activecode = $this->get_configuration_code_activeregister();
            $this->postinit_plugin($activecode);
            
            foreach($this->get_ldap_codes($fullconfig) as $ldapcode)
            {
                $settingsurl = new moodle_url('/admin/auth_config.php', [
                    'auth' => 'otmultildap',
                    'ldapcode' => $ldapcode
                ]);
                $settingslink = html_writer::link($settingsurl, $ldapcode);
                $deletesettingsurl = new moodle_url('/admin/auth_config.php', [
                    'auth' => 'otmultildap',
                    'ldapcode' => $ldapcode,
                    'delete' => true
                ]);
                $deletesettingslink = html_writer::link(
                        $deletesettingsurl,
                        html_writer::img($OUTPUT->pix_url('t/delete'),get_string('auth_otmultildap_delete_ldapcode_settings_desc', 'auth_otmultildap')));
                $add = '';
                ...
                echo html_writer::div($settingslink.' '.$deletesettingslink . $add, 'auth_otmultildap_settings_link');
            }
            
            // элементы для создания новой конфигурации
            echo html_writer::div(get_string('auth_otmultildap_new_ldap_code_desc', 'auth_otmultildap'));
            echo html_writer::empty_tag('input', [
                'name' => 'new_ldap_code',
                'id' => 'new_ldap_code',
                'type' => 'text'
            ]);
            if (isset($err['new_ldap_code'])) 
            { 
                echo $OUTPUT->error_text($err['new_ldap_code']); 
            }
        } elseif (!empty($deleteldapcode)) 
        {
            
            echo html_writer::div(get_string('auth_otmultildap_confirm_delete_ldap_code', 'auth_otmultildap', $ldapcode));
        ...
        {
            $this->postinit_plugin($ldapcode);
            $config = $this->config;
            include($CFG->dirroot.'/auth/otmultildap/config.html');
        }
    }
    
    /**
     * Метод получение списка конфигураций LDAP серверов
     * 
     * @param stdClass $config
     * 
     * @return mixed[]
     */
    public function get_ldap_codes($config)
    {
        $knowncodes = [];
        if (!empty($config)) {
            foreach($config as $code=>$value)
            {
                $codeparts = explode('___', $code);
                if(isset($codeparts[0]) && $codeparts[0] == 'host_url' && count($codeparts) > 1)
                {
                    $knowncodes[] = $codeparts[count($codeparts)-1];
                }
            }
        }
    ...
    /**
     * Формирование объекта конфигов конкретной конфигурации
     * 
     * @param stdClass $config
     * @param string $ldapcode
     * 
     * @return stdClass
     */
    protected function prepare_config_by_ldapcode($config, $ldapcode)
    {
        $ldapcodeconfig = new stdClass();
        
        foreach($config as $code=>$value)
        {
            $codeparts = explode('___', $code);
            if (!array_key_exists(1, $codeparts) || $codeparts[1] !== $ldapcode) 
            {
                continue;
            }
            if ( ! empty($codeparts) )
            {
                $ldapcodeconfig->{$codeparts[0]} = $value;
            }
        }
        
        return $ldapcodeconfig;
    }
    
    /**
     * Метод удаления конфигов по коду конфигурации
     * 
     * @param string $ldapcode
     * 
     * @return void
     */
    public function delete_configs_with_ldapcode($ldapcode)
    {
        $config = get_config($this->pluginconfig);
        
        foreach($config as $code=>$value)
        {
            if (substr($code, -strlen($ldapcode)-3) == '___'.$ldapcode)
            {
                unset_config($code, 'auth_otmultildap');
            }
        }
    }
    ...
    function process_config($config) {
        ...
        if (isset($config->new_ldap_code))
        {
            redirect(new moodle_url('/admin/auth_config.php', [
                'auth' => 'otmultildap',
                'ldapcode' => $config->new_ldap_code
            ]));
        }
        if (isset($config->ldap_code_to_delete))
        {
            $this->delete_configs_with_ldapcode($config->ldap_code_to_delete);
            redirect(new moodle_url('/admin/auth_config.php', ['auth' => 'otmultildap']));
        }
        
        // Set to defaults if undefined
        if (!isset($config->ldapcode)) {
            $config->ldapcode = 'default';
        }        
        ...
        if (!empty($config->auth_user_create)) {
            // метка, что синхронизация новых пользователей в LDAP будет в эту конфигурацию
            $allconfigs = get_config($this->pluginconfig);
            foreach($allconfigs as $configname => $val) {
                if(strpos($configname, 'auth_user_create___') !== false && !empty($val)) {
                    set_config($configname, 0, $this->pluginconfig);
                }
            }
        }

        // Save settings
        set_config('host_url___'.$config->ldapcode, trim($config->host_url), $this->pluginconfig);
        set_config('start_tls___'.$config->ldapcode, $config->start_tls, $this->pluginconfig);
        set_config('ldapencoding___'.$config->ldapcode, trim($config->ldapencoding), $this->pluginconfig);
        set_config('pagesize___'.$config->ldapcode, (int)trim($config->pagesize), $this->pluginconfig);
        set_config('contexts___'.$config->ldapcode, $config->contexts, $this->pluginconfig);
        set_config('user_type___'.$config->ldapcode, core_text::strtolower(trim($config->user_type)), $this->pluginconfig);
        set_config('user_attribute___'.$config->ldapcode, core_text::strtolower(trim($config->user_attribute)), $this->pluginconfig);
        set_config('suspended_attribute___'.$config->ldapcode, core_text::strtolower(trim($config->suspended_attribute)), $this->pluginconfig);
        set_config('sync_suspended___'.$config->ldapcode, $config->sync_suspended, $this->pluginconfig);
        set_config('search_sub___'.$config->ldapcode, $config->search_sub, $this->pluginconfig);
        set_config('opt_deref___'.$config->ldapcode, $config->opt_deref, $this->pluginconfig);
        set_config('preventpassindb___'.$config->ldapcode, $config->preventpassindb, $this->pluginconfig);
        set_config('bind_dn___'.$config->ldapcode, trim($config->bind_dn), $this->pluginconfig);
        set_config('bind_pw___'.$config->ldapcode, $config->bind_pw, $this->pluginconfig);
        set_config('ldap_version___'.$config->ldapcode, $config->ldap_version, $this->pluginconfig);
        set_config('objectclass___'.$config->ldapcode, trim($config->objectclass), $this->pluginconfig);
        set_config('memberattribute___'.$config->ldapcode, core_text::strtolower(trim($config->memberattribute)), $this->pluginconfig);
        set_config('memberattribute_isdn___'.$config->ldapcode, $config->memberattribute_isdn, $this->pluginconfig);
        set_config('creators___'.$config->ldapcode, trim($config->creators), $this->pluginconfig);
        set_config('create_context___'.$config->ldapcode, trim($config->create_context), $this->pluginconfig);
        set_config('expiration___'.$config->ldapcode, $config->expiration, $this->pluginconfig);
        set_config('expiration_warning___'.$config->ldapcode, trim($config->expiration_warning), $this->pluginconfig);
        set_config('expireattr___'.$config->ldapcode, core_text::strtolower(trim($config->expireattr)), $this->pluginconfig);
        set_config('gracelogins___'.$config->ldapcode, $config->gracelogins, $this->pluginconfig);
        set_config('graceattr___'.$config->ldapcode, core_text::strtolower(trim($config->graceattr)), $this->pluginconfig);
        set_config('auth_user_create___'.$config->ldapcode, $config->auth_user_create, $this->pluginconfig);
        set_config('forcechangepassword___'.$config->ldapcode, $config->forcechangepassword, $this->pluginconfig);
        set_config('stdchangepassword___'.$config->ldapcode, $config->stdchangepassword, $this->pluginconfig);
        set_config('passtype___'.$config->ldapcode, $config->passtype, $this->pluginconfig);
        set_config('changepasswordurl___'.$config->ldapcode, trim($config->changepasswordurl), $this->pluginconfig);
        set_config('removeuser___'.$config->ldapcode, $config->removeuser, $this->pluginconfig);
        set_config('ntlmsso_enabled___'.$config->ldapcode, (int)$config->ntlmsso_enabled, $this->pluginconfig);
        set_config('ntlmsso_subnet___'.$config->ldapcode, trim($config->ntlmsso_subnet), $this->pluginconfig);
        set_config('ntlmsso_ie_fastpath___'.$config->ldapcode, (int)$config->ntlmsso_ie_fastpath, $this->pluginconfig);
        set_config('ntlmsso_type___'.$config->ldapcode, $config->ntlmsso_type, 'auth/otmultildap');
        set_config('ntlmsso_remoteuserformat___'.$config->ldapcode, trim($config->ntlmsso_remoteuserformat), 'auth/otmultildap');
        
        // save field lock configuration
        foreach ($config as $name => $value) {
            if (preg_match('/^lockconfig_(.+?)$/', $name, $matches)) {
                set_config($matches[1].'___'.$config->ldapcode, $value, 'auth/otmultildap');
            }
        }
    ...
    function ldap_get_ad_pwdexpire($pwdlastset, $ldapconn, $user_dn){
        ...
        if (!function_exists('bcsub')) {
            error_log($this->errorlogtag.get_string ('needbcmath', 'auth_otmultildap'));
            return 0;
        }
        ...
        $sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)',
                        array('userAccountControl'));
        if (!$sr) {
            error_log($this->errorlogtag.get_string ('useracctctrlerror', 'auth_otmultildap', $user_dn));
        ...
        if (!$sr) {
            error_log($this->errorlogtag.get_string ('rootdseerror', 'auth_otmultildap'));
            return 0;
        }
     ...
     */
    function ldap_connect($ldapcode ='default') {
        ...
        print_error('auth_otmultildap_noconnect_all', 'auth_otmultildap', '', $debuginfo);
    }
    ...
    function validate_form($form, &$err) {
        if (isset($form->new_ldap_code))
        {
            if (trim($form->new_ldap_code) == '')
            {
                $err['new_ldap_code'] = get_string('auth_otmultildap_ldapcode_is_empty', 'auth_otmultildap');
            }
            return;
        }
        if(isset($form->ldap_code_to_delete))
        {
            return;
        }
        if ($form->ntlmsso_type == 'ntlm') {
            ...
            if (!empty($format) && !preg_match('/%username%/i', $format)) {
                $err['ntlmsso_remoteuserformat'] = get_string('auth_ntlmsso_missing_username', 'auth_otmultildap');
            }
        }
    }
    ...
    protected function get_ntlm_remote_user($remoteuser) {
         ...
         * an empty username.
         */
        error_log($this->errorlogtag.get_string ('auth_ntlmsso_maybeinvalidformat', 'auth_otmultildap'));
        return '';
    }
    ...
    /**
     * Получение кода конфигурации, по которой синхронизирован пользователь
     * 
     * @return false|int
     */
    protected function get_user_code(stdClass $user)
    {
        // получение кода конфигурации сервера LDAP, по которому пользователь синхронизирован в СДО
        $code = get_user_preferences('ldap_configuration_code', null, $user);
        $availablecodes = $this->get_ldap_codes(get_config($this->pluginconfig));
        if (!in_array($code, $availablecodes)) {
            // отсутствует код LDAP-апа или такая конфигурация LDAP-а отсутствует в системе
            return false;
        }
        $this->postinit_plugin($code);
    ...
    /**
     * Получение кода, являющегося активным при регистрации пользователей из Moodle в LDAP сервер
     * 
     * @return string|false
     */
    public function get_configuration_code_activeregister()
    {
        $configs = get_config($this->pluginconfig);
        foreach ($configs as $config => $value) {
            if (strpos($config, 'auth_user_create___') !== false && !empty($value)) {
                return str_replace('auth_user_create___', '', $config);
            }
        }
     ...
     * @return array
     */
    public function get_user_profile_fields($code = null) : array
    {
        if ( is_null($code) )
        {
        ...
        $newfields = [];
        foreach ( $this->userfields as $fieldname )
        {
            $newfields[] = $fieldname . '___' . $code;
        }
     ...
     */
    public function get_custom_user_profile_fields($code = null) : array
    {
        global $DB;
        ...
        if ($proffields = $DB->get_records('user_info_field')) {
            foreach ($proffields as $proffield) {
                if ( ! is_null($code) )
                {
                    $fields[] = 'profile_field_'.$proffield->shortname . '___' . $code;
                } else 
                {
                    $fields[] = 'profile_field_'.$proffield->shortname;
                }
            }
        }
        unset($proffields);
